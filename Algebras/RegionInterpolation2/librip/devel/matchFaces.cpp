/*
 ...
 vector<Matches> fn(vector<Face> *src, vector<Face> *dst,
...
vector<Matches> _matchFacesLua(vector<Face> *src,
    ...
    while (i < src->size() && j < dst->size()) {
        ...
        if ((*src)[i].isEmpty()) {
            ...
            if (p1 == p2) {
                (*src)[i].used = 1;
                (*dst)[j].used = 1;
                ret.push_back(Matches(&(*src)[i], &(*dst)[j]));
                i++;
                j++;
            } else if (p1 < p2) {
                i++;
...
*/
static vector<Matches> matchFacesCriterion(vector<Face> *src,
    ...
    while (it != mtab.end()) {
        if (!it->s->used && !it->d->used) {
            it->s->used = 1;
            it->d->used = 1;
            ret.push_back(Matches(it->s, it->d));
        }
        it++;
    }
...
*/
static vector<Matches> matchFacesOverlap(vector<Face> *src,
        vector<Face> *dst, int depth, string args) {
...
*/
vector<Matches> matchFacesMW(vector<Face> *src,
        vector<Face> *dst, int depth, string args) {
    if (depth == 0) {
...
vector<Matches> matchFaces(
    ...
    for (unsigned int i = 0; i < src->size(); i++) {
        if (!(*src)[i].used) {
            ret.push_back(Matches(&(*src)[i], NULL));
        }
    }
    for (unsigned int i = 0; i < dst->size(); i++) {
        if (!(*dst)[i].used) {
            ret.push_back(Matches(NULL, &(*dst)[i]));
        }
    }
