...
Copyright (C) 2020, University in Hagen, Faculty of Mathematics and
...
//[$][\$]
 ...
 Version 1.1 - Added ~checkCellOriginal~ to check if the attributes Original and or Cell exists in Value Mapping - D. Selenyi - 25.07.2020
    ...
    bool DRelHelpers::isDRelDescr( ListExpr arg, ListExpr& drelType, 
        ...
        if( !DRel::checkType( type )
         && !DFRel::checkType( type ) ) {
            #ifdef DRELDEBUG
            cout << "nl->toString(type): " << nl->ToString(type) << endl;
...
get the corresponding drelType (drel or dfrel), the relation type
(for example ( rel( tuple( ( PLZ int ) ) ) )  ), the distribution type
as a list, the value (for example the name of the drel and the Type of the d[f]array.
...
*/
    bool DRelHelpers::replaceDRELFUNARG( 
        ...
        cout << endl <<"replaceDRELFUNARG" << endl;
...
search.
Additional audit. If searched Attribute is out of boundary, then return -1 to inform user.
...
/*
1.22 ~removePartitionAttributes~
...
/*
1.23 ~addPartitionAttributes~

Add all partition attributes from the attrList. The partition attributes 
may be Original and Cell.
...
/*
1.25 Value Mapping ~createRepartitionQuery~
...
the "elem" in the query has to be unique. Therefor the argument elem, elem2 
    ...
    bool DRelHelpers::createRepartitionQuery( 
        ...
        if( targetDistType == range ) {
            ...
            if( !parti->repartition2DFMatrix( ) ) {
                cout << "repartition (DrelHelpers 1) failed!!" << endl;
                return false;
            }
            ...
            if( !matrix || !matrix->IsDefined( ) ) {
                cout << "repartition (DrelHelpers 2) failed!!" << endl;
                return false;
            }
...
/*
1.27 ~checkCellOriginal~

Check if Cell and Original exists in the Attribute List

*/

bool  DRelHelpers::checkCellOriginal( 
int *checkOriginal, int *checkCell, ListExpr *attrList, ListExpr *relType) {
        ...
        *checkCell = 0;
        ...
        while(!nl->HasLength(attrListCheck,0)){
                #ifdef DRELDEBUG
                cout << endl << "attrListCheck: ";
                cout <<  nl->ToString(attrListCheck) << endl;
                cout << "nl->First(nl->First(attrList)): " 
                cout << attrListcheck << endl;
                #endif 

                if(attrListcheck == "Original")
                {
                        *checkOriginal = 1;
                }
                if(attrListcheck == "Cell")
                {
                         *checkCell= 1;
                }
        attrListCheck = nl->Rest(attrListCheck);
        }

        #ifdef DRELDEBUG
        cout << "checkOriginal: " << *checkOriginal << endl;
        cout << "checkCell: " << *checkCell << endl;
        cout << endl << "attrList before remove: "; 
