...
drelprojectextend, lsortby, drelgroupby, lsort, lrdup, 
rename, head and drelextend

This operators have the same value mapping which calls the dmap operator of 
...
#include "Algebras/DRel/Partitioner.hpp"
#include "Algebras/DRel/BoundaryCalculator.hpp"
#include "Algebras/DRel/drelport.h"
...
*/
    ListExpr drelpartitionTM( ListExpr args ) {
        ...
        if( !( nl->AtomType( nl->Second( args ) ) == SymbolType ) ) {
            return listutils::typeError( err +
                ": second argument is not an attribute" );
        }
        ...
        if( nl->HasLength( args, 3 ) ) {
            if( Vector::checkType( nl->Third( args ) ) ) {
                if( !nl->Equal( nl->Second( nl->Third( args ) ), attrType ) ) {
                    return listutils::typeError(err + 
                        ": attr type does not fit to the vector type" );
                }
            }
                ...
                if( !nl->HasLength( nl->Third( nl->Third( args ) ), 4 ) ) {
                    return listutils::typeError(err + 
                        ": third argument is a d[f]rel, but it is "
                ...
                if( !nl->Equal( 
                    ...
                    attrType ) ) {
                    return listutils::typeError(err + 
                        ": d[f]rel argument is partitioned by range, but"
...
*/
    ...
    int drelpartitionVMT( Word* args, Word& result, int message,
        ...
        if( !parti->repartition2DFMatrix( ) ) {
            std::cout << "repartition (drelpartition 1) failed!!" << std::endl;
            result = qp->ResultStorage( s );
            ( ( DFRel* )result.addr )->makeUndefined( );
            return 0;
        }
        ...
        if( !matrix || !matrix->IsDefined( ) ) {
            std::cout << "repartition (drelpartition 2) failed!!" << std::endl;
            result = qp->ResultStorage( s );
            ( ( DFRel* )result.addr )->makeUndefined( );
            return 0;
        }
    ...
    int drelpartitionSelect( ListExpr args ) {
        ...
        int x = nl->HasLength( args, 3 ) 
             && ( Vector::checkType( nl->Third( args ) ) 
                ...
                || DRel::checkType( nl->Third( args ) ) ) ? 2 : 0;
