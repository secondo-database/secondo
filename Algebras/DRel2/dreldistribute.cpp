...
//[$][\$]
 ...
 Version 1.1 - Added dreldistributevmtchash for consistent Hashing and updated the informations - D. Selenyi - 05.07.2020
 Version 1.2 - Small improvements - D. Selenyi - 19.08.2020
...
fields which will be used to distribute the stream. For the distribution
...
*/
    ...
    ListExpr distributeTM( ListExpr args ) {
        ...
        if( !Relation::checkType( relType ) ) {
            return listutils::typeError( err + 
                ": first parameter is not a relation" );
        }
        ...
        if( !( isWorkerRelDesc( workerRelType, positions, types, errmsg ) ) ) {
            return listutils::typeError(err + 
                ": relation is not a worker relation" );
        }
        ...
        if( !nl->IsAtom( reqDistType ) ) {
            return listutils::typeError( err + 
                ": fourth parameter is not distribution type" );
        }
        ...
        // or spatial3d for 6 arguments
            ...
            if( !( nl->AtomType( nl->Fourth( args ) )
                    == SymbolType ) ) {
                return listutils::typeError( err + 
                    ": fifth parameter is not an attribute" );
            }
            ...
            // create result type and append list for range distribution
                ...
                if( !DistTypeRange::allowedAttrType( attrType ) ) {
                    return listutils::typeError( err + 
                    ": attribute type is not supported for range distribution");
                }
            ...
            // create result type and append list for spatial2d distribution
                ...
                if( !DistTypeSpatial<CellGrid2D>
                    ...
                    return listutils::typeError( err + 
                    ": attribute type is not supported for spatial2d " +
                    "distribution" );
                }
            ...
            // create result type and append list for spatial3d distribution
                ...
                if( !DistTypeSpatial<CellGrid<3>>
                    ...
                    return listutils::typeError( err + 
                        ": attribute type is not supported for spatial3d " +
                        "distribution" );
                }
                ...
                return listutils::typeError(err + 
                    ": for this number of arguments only range, spatial2d " +
                    " or spatial3d is supported" );
            }

        }
        else {
            return listutils::typeError(err + 
                ": distribution type does not fit to the number of arguments" );
        }
...
/*
1.7 ~distributeVMTchash~

Value mapping of the distribute operator to distribute by consistent hash.

*/
    template<class RType, class AType, class DType, class HType, class CType>
    int distributeVMTchash( Word* args, Word& result, int message,
        Word& local, Supplier s ) {
        ...
        //Number of Workers.
        CcInt* workerscount = new CcInt( workers->GetNoTuples( ) );
        ...
        ListExpr fundef = nl->ThreeElemList(
            ...
            nl->ThreeElemList(
                nl->SymbolAtom( "attr" ),
                nl->SymbolAtom( "t" ),
                nl->SymbolAtom( attrName ) ), // Attributname for chashfunction
            nl->IntAtom( 3600000 ) ); 
        ...
        CcInt* slots = ( ( CcInt* )args[ 4 ].addr );

        //if slots are 0 then depending on the workerscount else user input
        if (slots->GetValue() == 0)
        {
        //CcInt* slots2 = new CcInt(workerscount->GetValue( )*4);
        slots = new CcInt(workerscount->GetValue( ));
        }        

        // new argument vector for distribute5VMT with the OpTree passed to 
        // the valuemapping of the distribute3 operator.
        ArgVector argVec = { 
            stream, 
            args[ 1 ].addr, 
        ...
        //distribute4 changed to 5 for chashing.
        distribute5VMT<AType, DType, HType, CType>( argVec,
            result, message, local, s );
        ...
        RType* drel = ( RType* )result.addr;
        if( drel->IsDefined( ) ) {
            drel->setDistType( new DistTypeCHash( chash, pos - 1) );
        }
    ...
    OperatorSpec drelfdistributeSpec(
        ...
        "are RANDOM, HASH, RANGE, SPATIAL2D SPATIAL3D, REPLICATED and CHASH. "
        ...
        "RANGE, CHASH, SPATIAL2D and SPATIAL3D and controls in which slot of "
        "the resulting array is the corresponding tuple inserted. The fifth "
    ...
    ValueMapping drelfdistributeVM[ ] = {
        ...
        distributeVMTchash<DFRel, DFArray, FRelCopy, CcString, CcString>,
        distributeVMTchash<DFRel, DFArray, FRelCopy, CcString, FText>,
        distributeVMTchash<DFRel, DFArray, FRelCopy, FText, CcString>,
        distributeVMTchash<DFRel, DFArray, FRelCopy, FText, FText>,
    };
    ...
    ValueMapping dreldistributeVM[ ] = {
        ...
        distributeVMTchash<DRel, DArray, RelFileRestorer, CcString, CcString>,
        distributeVMTchash<DRel, DArray, RelFileRestorer, CcString, FText>,
        distributeVMTchash<DRel, DArray, RelFileRestorer, FText, CcString>,
        distributeVMTchash<DRel, DArray, RelFileRestorer, FText, FText>
    };
