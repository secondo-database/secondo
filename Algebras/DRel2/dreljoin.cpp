...
//[$][\$]
 ...
 Version 1.1 - Small improvements. New Flags for Repl/range implemented. - D. Selenyi - 16.08.2020
...
*/
    ...
    ListExpr drelsimpleJoinTM( ListExpr args ) {
        ...
        if( rel1Flag && rel2Flag && compatFlag ) {
            ...
            int size1test = DRelHelpers::countDRel( 
                nl->SymbolValue( drel1Value ) );
    ...
    ListExpr hashPartition( ListExpr drelType, void* ptr, std::string attr, 
        ...
        if( type == spatial2d || type == spatial3d ) {
            ...
            queryS = "(collect2 (partitionF " + nl->ToString( 
                ...
                //+ attr + ") 99999)) 0) ( \"\" )"+getDRelPortString()+")";
                + attr + ") 99999)) 0)  \"\" "+getDRelPortString()+")";

                //Selenyi added checkoriginalcell
                int checkOriginal, checkCell;
                ListExpr relType = nl->TheEmptyList( );
                ListExpr attrList = nl->Second(
                        nl->Second(nl->Second(drelType)));
                DRelHelpers::checkCellOriginal(
                        &checkOriginal, &checkCell, &attrList, &relType);

                if (checkOriginal == 0 && checkCell == 0){
                    queryS = "(collect2 (partitionF " + nl->ToString( 
                    DRelHelpers::createdrel2darray( drelType, ptr ) ) + " \"\" "
                    "(fun (elem" + elem1str + "_1 FFR) (elem" + elem2str + "_2 "
                    "FFR)  (feed elem" + elem1str + "_1) )"
                    " (fun (elem" + elem1str + "_4 FFR) (elem" + 
                    elem2str + "_5 FFR) (hashvalue(attr elem" + elem2str + "_5 "
                    + attr + ") 99999)) 0)  \"\" "+getDRelPortString()+")";
            ...
            queryS = "(collect2 (partition " + nl->ToString(
                    ...
                    "(attr elem_" + elemstr + " " + attr + ") 99999)) 50) \"\" "
                    + getDRelPortString()+")";
        }
...
*/
    ...
    int drelsimpleJoinVMT( Word* args, Word& result, int message,
        ...
        else if ( dType == range) {  // must be range
