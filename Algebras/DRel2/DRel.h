...
//[$][\$]
 ...
 Version 1.1 - CHash integrated - D. Selenyi - 01.07.2020
...
*/
    ...
    DistTypeBasic* DRelT<T>::getDistType( ) {
        ...
        if(!distType)
        {
            distType =  new DistTypeBasic( chash ) ;
        }
        distType->print( );
        #endif
...
*/
    ...
    ListExpr DRelT<T>::toListExpr( ListExpr typeInfo ) const {
        ...
        #ifdef DRELDEBUG
        std::cout << std::endl << "DRelT::toListExpr" << std::endl;
        std::cout << "typeInfo" << std::endl;
        std::cout << nl->ToString( typeInfo ) << std::endl;
        std::cout << "nl->Third( typeInfo ): ";
        std::cout << nl->ToString(nl->Third( typeInfo )) << std::endl;
        #endif
        ...
        std::cout << "distType->toListExpr( nl->Third( typeInfo ) ): ";
        std::cout << nl->ToString(distType->toListExpr(
                             nl->Third( typeInfo ) )) << std::endl;
...
*/
  ...
  distributed2::DArrayT<T>* darray = 
    ...
    DistTypeBasic* DRelT<T>::readDistTypeFrom( 
        ...
        if( nl->HasLength( list, 2 ) && type == chash ) {
            return new DistTypeCHash( type, attrPos);
        }
...
*/
    ...
    bool DRelT<T>::Open( 
        ...
        if( type == chash) {
            ...
            rel->setDistType( new DistTypeCHash( type, attr) );
            return true;
        }
