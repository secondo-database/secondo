...
*/
  ...
  void OutputToDotFile( void* rootNode, const char* filePath);

  void BuildDotDescr( void* node, DotSpec& dot );
/*
Builds a graph representation of the operator subtree with ~node~ 
as root node. ~BuildDotDescr~ is called recursively for each child
node of ~node~. The result is written to a dot specification ~dot~ 
which also allows to change the formatting of the graphs nodes. 

~OutputToDotFile~ calls ~BuildDotDescr~ and writes the content to 
a file specified by the full path ~filePath~ 

It is possible to convert the dot file to a graphical representation
by calling "dot -Tpng <pathToDotfile> -o <outputFilename>.png" on 
the terminal  
...
*/
  ...
  Supplier RemoveSon( const Supplier s, const int i );
/*
Deletes the connection to the son with index ~i~ (zero based) of the operator 
node ~s~. This splits the operator tree. The orphaned child is returned by the 
function and the caller is responsible to keep a reference to the removed node 
subtree and call destroy to free its resources.
...
/*
Return the number of possible instances of the par-operator. This value is a 
...
*/
  ...
  parthread::ParallelQueryOptimizer m_parallelQueryOptimizer;
