...
/** Prints out all nodes of the given tree **/
  ...
  */
  ...
  void 
  QueryProcessor::OutputToDotFile(void* root_node, const char* filePath) 
  {
    //print query plan as graph
    DotSpec dot("optree");
    ...
    std::ofstream of;
    of.open(filePath, std::ios::trunc);
    BuildDotDescr(root_node, dot);
...
QueryProcessor::SubtreeX( const ListExpr expr, const datetime::DateTime& qt )
{
  ...
  if ( RTFlag::isActive("QP:OpTree2SVG") ) {
    ...
    //save existing dot file before overwriting the file
    rename(cfp, ofp);

    OutputToDotFile(resultTree, cfp);
  }
...
void QueryProcessor::AddSon(const Supplier parent, const Supplier son)
{
  ...
  if ( tree->nodetype != Operator )
  {
    throw qp_error("The given supplier parent is no operator node");
  }
...
Supplier QueryProcessor::RemoveSon( const Supplier s, const int i )
{
  Supplier orphanedChild = NULL;
  OpTree tree = (OpTree) s;
  if ( tree->nodetype != Operator )
  {
    throw qp_error("The given supplier s is no operator node");
  ...
  //save the pointer to the cut node
  orphanedChild = tree->u.op.sons[i].addr;

  //move all sons, following the removed node, one index forward in array
  for (int j = i+1; j < numSons; j++)
  {
    tree->u.op.sons[j-1] = tree->u.op.sons[j];
  }
  tree->u.op.sons[numSons-1].addr = 0;

  //adjust reduced number of sons
  tree->u.op.noSons--;
                                    ...
                                    std::map<int, int> &valueInfoMapping)
{
  ...
  copiedNode->id = OpNodeIdCtr++;
  OpNodeAddr2Id[copiedNode] = copiedNode->id;

  if (copiedNode == NULL)
  {
    throw qp_error("copying the node failed");
  }

  switch (copiedNode->nodetype)
  {
  case IndirectObject:
  ...
  case Operator:
    ...
    if (tree->u.op.costEstimation)
    {
      //create a new cost estimation object to avoid problems with
      //concurrent access
      int funId = tree->u.op.opFunId  / 65536;
      CreateCostEstimation createCE =
          tree->u.op.theOperator->getCreateCostEstimation(funId);
      if (createCE)
      {
        copiedNode->u.op.costEstimation = createCE();
        copiedNode->u.op.costEstimation->setSupplier(copiedNode);
        copiedNode->u.op.supportsProgress = false;
        copiedNode->u.op.argsAvailable = tree->u.op.argsAvailable;
      }
    }

    if (copiedNode->u.op.isFun)
    {
      //translate the existing funNo to the funNo of the copied node
      //all indirect objects in subtrees of this node are already processed,
      //so the mapping must exists
      std::map<int, int>::iterator find = funNoMapping.find(tree->u.op.funNo);
      assert(find != funNoMapping.end());
      int funNo = find->second;

      copiedNode->u.op.funNo = funNo;
      copiedNode->u.op.funArgs = argVectors[funNo - 1];
    }

    //copy result storage
    if (copiedNode->u.op.theOperator != NULL)
    {
      ReInitResultStorage(copiedNode);
    }
    break;
  case Pointer:
  case Object:
                                        ...
                                        int partitionAttributeIdx)
{
  ...
  OpTree parentNode = static_cast<OpTree>(s);
      ...
      resultsTuple)
  {
    ...
    ListExpr parDesc = nl->Empty();
    if (GetCatalog()->IsOperatorName(parName))
    {
      ...
      if(nl->IsEmpty(opIdList) || nl->HasMinLength(opIdList, 2))
      {
        ...
        errorMessage << "Ambiguous operator name for " << parName << ": "
        << opIdListString;
...
size_t 
QueryProcessor::GetParOperatorsNumberOfInstances(const Supplier s)
{
  OpTree tree = (OpTree)s;
  const char* parName = parthread::ParallelQueryOptimizer::ParOperatorName;
  ...
  if (tree->nodetype != Operator ||
      tree->u.op.theOperator->GetName() != parName)
  {
    throw qp_error("The given supplier parent is no par-operator");
  }

  //get the second child. It should be a const value
  OpTree childNode = static_cast<OpTree>(GetSon(s, 1));
  if (childNode->nodetype != Object ||
      !childNode->u.dobj.isConstant)
  {
    throw qp_error("The given par-operator node has no constant value for \
                    number of instances");
  }
  
  //return the integer value of the constant node
  int numOfInstances = 
                static_cast<CcInt *>(childNode->u.dobj.value.addr)->GetIntval();

  if (numOfInstances < 1)
  {
    throw qp_error("The number of instances must be greater than 0");
  }
...
void 
QueryProcessor::ChangeParOperatorsNumberOfInstances(const Supplier s, 
                                                    size_t numInstances)
{
  if (GetParOperatorsNumberOfInstances(s) == numInstances)
  {
    return;
  }
  //get the second child. It should be a const value
  ...
  //delete the old constant object
  ValueInfo &originalValueInfo = values[childNode->u.dobj.valNo];
  (algebraManager->DeleteObj
             ( originalValueInfo.algId, originalValueInfo.typeId ))
             ( originalValueInfo.typeInfo, originalValueInfo.value );
  
  //create a new constant with the newInstances value
  int errorPos = 0;
  bool correct = true;
  ListExpr &errorInfo = nl->GetErrorList();
  Word value = GetCatalog()->InObject(nl->SymbolAtom(CcInt::BasicType()),
                                      nl->IntAtom(numInstances), errorPos, 
                                      errorInfo, correct);

  if (!correct)
  {
    throw qp_error("Error while creating int-value for given par-operator: " 
                   + nl->ToString(errorInfo));
  }

  originalValueInfo.value = value;
  childNode->u.dobj.value = value;
}

int 
QueryProcessor::GetParOperatorsPartitoningAttrIdx(const Supplier s)
{
  const char* parName = parthread::ParallelQueryOptimizer::ParOperatorName;
  OpTree tree = (OpTree)s;
