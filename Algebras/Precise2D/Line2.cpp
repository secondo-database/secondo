...
SegmentData::SegmentData(int xl, int yl, int xr, int yr,
		bool l, int edgeNo/*=-999999*/) :
		xLeft(xl), yLeft(yl), xRight(xr), yRight(yr),
		xLeftPos(0), yLeftPos(0),
		xRightPos(0), yRightPos(0),
		xLeftNumOfChars(0), yLeftNumOfChars(0),
		xRightNumOfChars(0), yRightNumOfChars(0),
		ldp(l), cycleno(-999999), edgeno(edgeNo),
		coverageno(-999999), insideAbove(false),
		partnerno(-999999) {
}

SegmentData::SegmentData(Point2* lp, Point2* rp, bool ldp,
		int edgeno,
		...
		xLeftNumOfChars(0), yLeftNumOfChars(0),
		xRightNumOfChars(0), yRightNumOfChars(0),
		ldp(ldp), cycleno(-999999), edgeno(edgeno),
		coverageno(-999999), insideAbove(false),
		partnerno(-999999) {
...
SegmentData::SegmentData(SegmentData& d) :
		xLeft(d.xLeft), yLeft(d.yLeft), xRight(d.xRight),
		yRight(d.yRight), xLeftPos(d.xLeftPos),
		yLeftPos(d.yLeftPos), xRightPos(d.xRightPos),
		yRightPos(d.yRightPos),
		xLeftNumOfChars(d.xLeftNumOfChars),
		yLeftNumOfChars(d.yLeftNumOfChars),
		xRightNumOfChars(d.xRightNumOfChars),
		yRightNumOfChars(d.yRightNumOfChars),
		ldp(d.ldp), cycleno(d.cycleno),
		edgeno(d.edgeno), coverageno(d.coverageno),
		insideAbove(d.insideAbove),
		partnerno(d.partnerno) {
}
;

mpq_class SegmentData::getPreciseLeftX
	...
	if (sz == 0) {
		...
		bool ok = preciseCoordinates->read(s,
				xLeftNumOfChars, xLeftPos);
		assert(ok);
		s[xLeftNumOfChars] = 0;
...
mpq_class SegmentData::getPreciseLeftY
	(const Flob* preciseCoordinates) const {
...
mpq_class SegmentData::getPreciseRightX
	(const Flob* preciseCoordinates) const {
...
mpq_class SegmentData::getPreciseRightY
	(const Flob* preciseCoordinates) const {
...
void SegmentData::SetPreciseLeftX(Flob* preciseCoordinates,
		mpq_class lx) {
	SmiSize sz = preciseCoordinates->getSize();
	...
	theStream << lx;
	theStream << '\0';
	char* s = new char[theStream.width() + 1];
	...
	bool ok = preciseCoordinates->write(s,
			xLeftNumOfChars + 1, xLeftPos);
	assert(ok);

}

void SegmentData::SetPreciseLeftY(Flob* preciseCoordinates,
		mpq_class ly) {
	SmiSize sz = preciseCoordinates->getSize();
	...
	bool ok = preciseCoordinates->write(s,
			yLeftNumOfChars + 1, yLeftPos);
	assert(ok);

}

void SegmentData::SetPreciseRightX(Flob* preciseCoordinates,
		mpq_class rx) {
	SmiSize sz = preciseCoordinates->getSize();
...
void SegmentData::SetPreciseLeftCoordinates(Point2* lp,
	...
	SmiSize newSize;
	if (sz == 0) {
		newSize = (lp->GetFLOB(0))->getSize() + 1;
	} else {
		newSize = sz + (lp->GetFLOB(0))->getSize();
	}
	preciseCoordinates->resize(newSize);
	char* lx = lp->getPreciseXAsString();
	xLeftPos = sz;
	xLeftNumOfChars = strlen(lx);
	char* ly = lp->getPreciseYAsString();
	yLeftPos = xLeftPos + xLeftNumOfChars;
	yLeftNumOfChars = strlen(ly);
	preciseCoordinates->write(lx, xLeftNumOfChars,
			xLeftPos);
	preciseCoordinates->write(ly, yLeftNumOfChars,
			yLeftPos);
}
...
void SegmentData::SetPreciseRightCoordinates(Point2* rp,
	...
	SmiSize newSize;
	if (sz == 0) {
		newSize = (rp->GetFLOB(0))->getSize() + 1;
	} else {
		newSize = sz + (rp->GetFLOB(0))->getSize();
	}
	preciseCoordinates->resize(newSize);
	char* rx = rp->getPreciseXAsString();
	xRightPos = sz;
	xRightNumOfChars = strlen(rx);
	char* ry = rp->getPreciseYAsString();
	yRightPos = xRightPos + xRightNumOfChars;
	yRightNumOfChars = strlen(ry);
	preciseCoordinates->write(rx, xRightNumOfChars,
			xRightPos);
	preciseCoordinates->write(ry, yRightNumOfChars,
			yRightPos);
}

void SegmentData::addPreciseValues(Flob* preciseCoordinates,
		mpq_class pLeftX,
		mpq_class pLeftY, mpq_class pRightX,
		mpq_class pRightY) {
//TODO ggf ersetzen durch eine fkt, die alle mpqs in eins
//ins Flob scheibt.
...
inline const Rectangle<2> SegmentData::BoundingBox(
		const Geoid* geoid /*=0*/) const {
	assert(!geoid);

	double minx = getLeftX() < getRightX() ?
			getLeftX() : getRightX();
	double maxx = getLeftX() < getRightX() ?
			getRightX() + 1 : getLeftX() + 1;
	double miny = getLeftY() < getRightY() ?
			getLeftY() : getRightY();
	double maxy = getLeftY() < getRightY() ?
			getRightY() + 1 : getLeftY() + 1;
...
Line2::Line2(const bool def, bool ldp, int xl, int yl,
		int xr, int yr,
...
Line2::Line2(const Line2& l) :
		...
		preciseCoordinates(
				l.preciseCoordinates.getSize()),
				segmentData(l.segmentData.Size()),
				bbox(l.bbox)
{
		preciseCoordinates.copyFrom(
		l.preciseCoordinates);
...
void Line2::addSegment(bool ldp, int leftX, int leftY,
...
void Line2::addSegment(bool ldp, Point2* lp, Point2* rp,
		int edgeno) {
...
inline const Rectangle<2> Line2::
	BoundingBox(const Geoid* geoid /*=0*/) const {
	if (geoid) {
		// spherical geometry case:
		...
		for (int i = 0; i < Size(); i++) {
			...
			if (sd.IsLeftDomPoint()) {
				if (!geobbox.IsDefined()) {
					geobbox = sd.BoundingBox(geoid);
				} else {
					geobbox = geobbox.
					Union(sd.BoundingBox(geoid));
				}
			}
		}
		return geobbox;
...
void Line2::collectFace(int faceno, int startPos,
		DbArray<bool>& used) {
...
int Line2::getUnusedExtension(int startPos,
		const DbArray<bool>& used) const {
	SegmentData hs, hs2;
	...
	while (pos >= 0 && !done) {
		...
		|| cmp(hs.GetDomPreciseXCoord(&preciseCoordinates),
				hs2.
				GetDomPreciseXCoord(&preciseCoordinates)) != 0
		|| cmp(hs.GetDomPreciseYCoord(&preciseCoordinates),
			hs2.
			GetDomPreciseYCoord(&preciseCoordinates)) != 0) {
			done = true;
		} else {
			used.Get(pos, u);
			if (!u) {
...
bool Line2::SegmentIsVertical(int lx, mpq_class plx, int rx,
		mpq_class prx) {
	if (lx != rx) {
		return false;
	}
...
*/
int Line2::CompareSegment2(const SegmentData& seg1,
...
void Line2::SetPartnerNo() {
	...
	for (int i = 0; i < segmentData.Size(); i++) {
		...
		if (seg1.IsLeftDomPoint()) {
			if (lpp >= 0) {
				// error case, segment already registered
				cerr << "wrong order in halfsegment array"
						<< endl;
			...
			if (lpp < 0) { // error case left segment not found
				cerr << "Error in halfsegment array detected"
						<< endl;
				cerr << "may be a wrong ordering or "
						"wrong set edgeno's"
						<< endl;
				TMP.Put(seg1.GetEdgeno(), i);
			} else {
...
void Line2::intersection(Line2& l2, Line2& result,
		const Geoid* geoid/*=0*/) {
...
void Line2::crossings(Line2& l2, Points2& result,
		const Geoid* geoid/*=0*/) {
...
void convertLineIntoLine2(Line& l, Line2& result) {
	...
	for (int i = 0; i < l.Size(); i++) {
		...
		if (hs.IsLeftDomPoint()) {
			...
			mpq_class plx = computeMpqFromDouble(
					hs.GetDomPoint().GetX() - glx);
			mpq_class ply = computeMpqFromDouble(
					hs.GetDomPoint().GetY() - gly);

			int grx = static_cast<int>(hs.GetSecPoint().GetX());
			int gry = static_cast<int>(hs.GetSecPoint().GetY());
			mpq_class prx = computeMpqFromDouble(
					hs.GetSecPoint().GetX() - grx);
			mpq_class pry = computeMpqFromDouble(
					hs.GetSecPoint().GetY() - gry);

			result.addSegment(true, glx, gly, grx, gry,
					plx, ply, prx, pry, edgeNo);
			result.addSegment(false, grx, gry, glx, gly,
					prx, pry, plx, ply, edgeNo);
			edgeNo++;
		}
	}
...
inline void Line2::CopyFrom(const Attribute* right) {
	const Line2* l = (const Line2*) right;
