...
1.1 Implementation file "GTAF[_]FileNode.cpp"[4]

January-February 2008, Mirko Dibbert
\\[3ex]
This file implements the "FileNode"[4] class.
...
FileNode::getNodeId()
{
    ...
    {
        SmiRecord record;
        m_supp->file()->AppendRecord(m_nodeId, record);
    }
...
*/
        if (m_extPageCnt + 1 < m_node->m_pagecount)
        {
            ...
            {
                m_supp->file()->DeleteRecord(rec_no);
            }
    ...
    m_supp->file()->SelectRecord(m_nodeId, record, SmiFile::Update);
...
FileNode::get(SmiRecordId nodeId)
{
    ...
    SmiRecord record;
    m_supp->file()->SelectRecord(nodeId, record, SmiFile::ReadOnly);
...
FileNode::drop()
{
    ...
    // remove header page
    m_supp->file()->DeleteRecord(m_nodeId);
    m_nodeId = 0;
}
