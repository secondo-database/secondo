...
DistfunInfo::DistfunInfo()
    : m_name("undef"), m_descr("undef"), m_distfun(0), m_distdataInfo(),
      m_flags(0) {}
...
*/
DistfunInfo::DistfunInfo(const string &name, const string &descr,
...
void DistfunReg::addInfo(DistfunInfo info) {
  ostringstream osId;
  osId << info.data().algebraId() << "#" << info.data().typeId() << "#"
       << info.name() << "#" << info.data().distdataId();
  distfunInfos[osId.str()] = info;

  if (info.isDefault()) {
    assert(info.data().isDefined());
    defaultNames[info.data().typeName()] = info.name();
    DistDataReg::defaultNames[info.data().typeName()] = info.data().name();
  }
}
...
string DistfunReg::defaultName(const string &typeName) {
  ...
  map<string, string>::iterator iter = defaultNames.find(typeName);
...
*/
DistfunInfo &DistfunReg::getInfo(const string &distfunName, DistDataId id) {
  if (!initialized)
    initialize();
  ...
  if (distfunName == DFUN_DEFAULT) {
    ...
    osId << id.algebraId() << "#" << id.typeId() << "#" << defaultName(typeName)
         << "#" << id.distdataId();
  } else {
    osId << id.algebraId() << "#" << id.typeId() << "#" << distfunName << "#"
         << id.distdataId();
  }
...
string DistfunReg::printDistfuns() {
  ...
  result << endl << "Defined distance functions:" << seperator;
  list<DistfunInfo> distfunInfos;
  DistfunReg::getInfoList(distfunInfos);
  list<DistfunInfo>::iterator iter = distfunInfos.begin();

  while (iter != distfunInfos.end()) {
    result << "name             : " << iter->name() << endl
           << "description      : " << iter->descr() << endl
           << "type constructor : " << iter->data().typeName() << endl
           << "accepted distdata: ";

    string curName = iter->name();
    string curType = iter->data().typeName();
    while ((iter != distfunInfos.end()) && (iter->name() == curName) &&
           (iter->data().typeName() == curType)) {
      result << iter->data().name();
      ++iter;

      if ((iter != distfunInfos.end()) && (iter->name() == curName) &&
          (iter->data().typeName() == curType))
        result << ", ";
    }
    result << seperator;
...
*/
void DistfunReg::EuclideanInt(const DistData *data1, const DistData *data2,
                              double &result) {
...
*/
void DistfunReg::EuclideanReal(const DistData *data1, const DistData *data2,
                               double &result) {
  SEC_STD_REAL val1 = *static_cast<const SEC_STD_REAL *>(data1->value());
  SEC_STD_REAL val2 = *static_cast<const SEC_STD_REAL *>(data2->value());
  result = abs(val1 - val2);
}

void DistfunReg::euclidPoint(const DistData *data1, const DistData *data2,
                             double &result) {
...
*/
void DistfunReg::specialPoints(const DistData *data1, const DistData *data2,
                               double &result) {
...
*/
void DistfunReg::EuclideanHPoint(const DistData *data1, const DistData *data2,
                                 double &result) {
  result = 0.0;
  ...
  if (dim1 != dim2) {
    cmsg.error() << "Tried to compute Euclidean distance between hpoints "
  ...
  const GTA_SPATIAL_DOM *coords1 =
      reinterpret_cast<const GTA_SPATIAL_DOM *>(buf1 + sizeof(unsigned));
  const GTA_SPATIAL_DOM *coords2 =
      reinterpret_cast<const GTA_SPATIAL_DOM *>(buf2 + sizeof(unsigned));
...
*/
void DistfunReg::EditDistance(const DistData *data1, const DistData *data2,
                              double &result) {
  ...
  for (int i = 1; i <= len1; ++i) {
    for (int j = 1; j <= len2; ++j) {
      ...
      //         d(i-1 , j-1) + dist }
      d[i][j] =
          min(d[i - 1][j] + 1, min((d[i][j - 1]) + 1, d[i - 1][j - 1] + dist));
    }
  }
...
void DistfunReg::MPointDistanceAvg(const DistData *data1, const DistData *data2,
  ...
  result = temporalalgebra::DistanceComputation<
      temporalalgebra::MPoint, temporalalgebra::UPoint>::DistanceAvg(mp1, mp2,
                                                                     duration,
                                                                     true);
}
...
*/
void DistfunReg::euclidFVector(const DistData *data1, const DistData *data2,
                               double &result) {
  // handle undefined values
  if (data1->size() == 0 && data2->size() == 0) {
    result = 0;
...
void DistfunReg::sqfdFeatureSignature(const DistData *data1,
  ...
  while (o1 < data1->size()) {
    ...
    memcpy((void *)&fst, (char *)data1->value() + o1,
  ...
  while (o2 < data2->size()) {
    FeatureSignatureTuple fst;
    memcpy((void *)&fst, (char *)data2->value() + o2,
           sizeof(FeatureSignatureTuple));
...
void DistfunReg::emdFeatureSignature(const DistData *data1,
                                     const DistData *data2, double &result) {
  if (data1->size() == 0 && data2->size() == 0) {
    result = 0;
    return;
...
All avaliable distance functions must be registered in this method by calling
the "addInfo"[4] method whith the respective "DistfunInfo" object. The parameter
of the "DistfunInfo"[4] constructor have the following meaning:
...
If more than one distance function for a specific type constructer has been
defined as default, the last registered one will be used as default.

Constants for the name and description for the distance functions are defined in
the "DistfunSymbols.h"[4] headerfile.
...
void DistfunReg::initialize() {
  ...
  addInfo(DistfunInfo(DFUN_EUCLID, DFUN_EUCLID_DESCR, EuclideanInt,
                      DistDataReg::getInfo(CcInt::BasicType(), DDATA_NATIVE),
                      DFUN_IS_METRIC | DFUN_IS_DEFAULT));

  addInfo(DistfunInfo(DFUN_EUCLID, DFUN_EUCLID_DESCR, EuclideanReal,
                      DistDataReg::getInfo(CcReal::BasicType(), DDATA_NATIVE),
                      DFUN_IS_METRIC | DFUN_IS_DEFAULT));

  addInfo(DistfunInfo(DFUN_EUCLID, DFUN_EUCLID_DESCR, euclidPoint,
                      DistDataReg::getInfo(Point::BasicType(), DDATA_NATIVE),
                      DFUN_IS_METRIC | DFUN_IS_DEFAULT));

  addInfo(DistfunInfo(DFUN_SPECIALPOINTS, DFUN_SPECIALPOINTS_DESCR,
                      specialPoints,
                      DistDataReg::getInfo(Points::BasicType(), DDATA_NATIVE),
                      DFUN_IS_METRIC | DFUN_IS_DEFAULT));

  addInfo(DistfunInfo(DFUN_EUCLID, DFUN_EUCLID_DESCR, EuclideanHPoint,
                      DistDataReg::getInfo("hpoint", DDATA_NATIVE),
                      DFUN_IS_METRIC | DFUN_IS_DEFAULT));

  addInfo(DistfunInfo(DFUN_EDIT_DIST, DFUN_EDIT_DIST_DESCR, EditDistance,
                      DistDataReg::getInfo(CcString::BasicType(), DDATA_NATIVE),
                      DFUN_IS_METRIC | DFUN_IS_DEFAULT));

  addInfo(DistfunInfo(
      DFUN_MPOINT_DISTAVG, DFUN_MPOINT_DISTAVG_DESCR, MPointDistanceAvg,
      DistDataReg::getInfo(temporalalgebra::MPoint::BasicType(), DDATA_NATIVE),
      DFUN_IS_METRIC | DFUN_IS_DEFAULT));
  ...
  addInfo(
      DistfunInfo(DFUN_SYMTRAJ_DIST1, DFUN_SYMTRAJ_DIST1_DESCR,
  ...
  addInfo(DistfunInfo(DFUN_TUPLE_DIST1, DFUN_TUPLE_DIST1_DESCR, tupleDistance,
...
//---------------------------------------------------
  ...
  addInfo(DistfunInfo(
      DFUN_EMD, DFUN_EMD_DESCR, emdFeatureSignature,
      DistDataReg::getInfo(FeatureSignaturealg::FeatureSignature::BasicType(),
                           DDATA_NATIVE),
      DFUN_IS_METRIC | DFUN_IS_DEFAULT));
#endif
