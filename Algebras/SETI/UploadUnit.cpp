                    ...
                    const int errorPos, ListExpr& errorInfo, bool& correct )
{
        ...
        nl->AtomType( updatetime ) == StringType )
  {
    ...
    if ( !ok )
    {
      cmsg.inFunError("The time stamp is invalid!");
      return result;
    }
  }
  else
  {
    cmsg.inFunError("Thet time stamp must be of string type!");
  }
...
ListExpr UploadUnit::Property()
{
  return (nl->TwoElemList(
         ...
         nl->FiveElemList(nl->StringAtom("-> DATA"),
                          ...
                          nl->StringAtom("(<id> <instant> (<x> <y>))"),
                          nl->StringAtom("(9 \"2010-10-12\" (8.2 1.6)"),
                          nl->StringAtom("x and y must be of type double."))));
}
