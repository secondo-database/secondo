...
*/
template <class ResType> class CommandListener {
public:
...
template <class A>
ConnectionInfo *changeWorker1(A *array, size_t slotNo, set<size_t> &usedWorkers,
                              int &tryReconnect, ConnectionInfo *&ciOrig,
                              bool keepIfOk = true) {
  ...
  while (!ci) {
    ...
    cout << "changed worker for slot " << slotNo << " from " << old << " to "
         << index << endl;
...
public:
  ...
  ~PProgressInfo() { delete w; }

  int getLastValue() { return lastValue; }
  void setLastValue(int i) { lastValue = i; }
  ...
  double getTime() { return w->diffSecondsReal(); }

  void setLast() { lastT = getTime(); }
  ...
  void write(int source, int actValue, int totalValue) {
    ...
    if (actValue >= 0) {
      if (totalValue <= 0) {
        ...
        cout << progbar(value, 20) << "  " << value << "% remaining time "
             << formatTime(rt);
      }
    } else {
      info->reset();
...
int Distributed2Algebra::sendFile(const int con, const string &local,
                                  const string &remote,
...
int Distributed2Algebra::requestFile(const int con, const string &remote,
                                     const string &local,
                                     const bool allowOverwrite) {
  if (con < 0) {
    return -3;
...
void Distributed2Algebra::finishProgress() { pprogView->finish(); }
...
bool Distributed2Algebra::simpleCommand(int con, const string &cmd, int &error,
  ...
  c->simpleCommand(cmd, error, errMsg, resList, rewrite, runtime, showCommands,
                   commandLog, false, getTimeout());
  return true;
}
...
ConnectionInfo *Distributed2Algebra::getWorkerConnection(
    const DArrayElement &info, const string &dbname, CommandLogger *log) {
...
template <class A>
ConnectionInfo *Distributed2Algebra::getWorkerConnection(
    A *array, int slot, const string &dbname, CommandLogger *log,
...
int Distributed2Algebra::closeAllWorkers() {
  boost::lock_guard<boost::mutex> guard(workerMtx);
  int count = 0;
  for (auto it = workerconnections.begin(); it != workerconnections.end();
       it++) {
...
string Distributed2Algebra::getTempName(int server) {
  ...
  stringstream ss;
  ss << "TMP_" << WinUnix::getpid() << "_" << ci->serverPid() << "_"
     << nextNameNumber();
  return ss.str();
}
...
string Distributed2Algebra::getTempName(const DArrayElement &elem) {
  ...
  stringstream ss;
  ss << "TMP_" << WinUnix::getpid() << "_" << it->second.second->serverPid()
     << "_" << nextNameNumber();
  return ss.str();
}
...
string Distributed2Algebra::getTempName() {
  ...
  map<DArrayElement, pair<string, ConnectionInfo *>>::iterator it;
  for (it = workerconnections.begin(); it != workerconnections.end() && !ci;
       it++) {
    ci = it->second.second;
  }
  int spid;
  ...
  stringstream ss;
  ss << "TMP_" << WinUnix::getpid() << "_" << sh << "_" << spid << "_"
     << nextNameNumber();
  return ss.str();
}
...
void Distributed2Algebra::cleanUp() {
  ...
  for (size_t i = 0; i < unique.size(); i++) {
    ...
    boost::thread *r = new boost::thread(boost::bind(
        &ConnectionInfo::cleanUp, ci, showCommands, boost::ref(commandLog), 0));
    runners.push_back(r);
  }
...
bool Distributed2Algebra::createWorkerConnection(
    const DArrayElement &worker, pair<string, ConnectionInfo *> &res) {
  string host = worker.getHost();
  int port = worker.getPort();
  string config = worker.getConfig();
  ConnectionInfo *ci =
      ConnectionInfo::createConnection(host, port, config, timeout, heartbeat);

  if (ci == nullptr) {
    Bash::setFGColor(Red);
    cout << "Connection to Server " << host << "@" << port << " failed" << endl;
    Bash::normalColors();
  }
  res.first = "";
  res.second = ci;
...
bool Distributed2Algebra::setHeartbeat(int _heartbeat) {
  ...
  heartbeat = _heartbeat;
  {
    boost::lock_guard<boost::mutex> guard(mtx);
    for (size_t i = 0; i < connections.size(); i++) {
      cout << "set heartbeat for user connection to " << heartbeat << endl;
  ...
  }
  {
    ...
    map<DArrayElement, pair<string, ConnectionInfo *>>::iterator it;
    for (it = workerconnections.begin(); it != workerconnections.end(); it++) {
      cout << "set heartbeat for worker connection to " << heartbeat << endl;
...
void Distributed2Algebra::enableDFS(ConnectionInfo *ci) {
  ...
  string res;
  double rt;
  ci->simpleCommand(ss.str(), err, res, false, rt, false, commandLog, true,
                    timeout);
}
...
void writeLog(ConnectionInfo *ci, const string &msg) {
  ...
  if (ci) {
    algInstance->errorWriter.writeLog(ci->getHost(), ci->getPort(),
                                      ci->serverPid(), msg);
  }
}
...
ListExpr replaceSymbols(ListExpr list,
  ...
  case NoAtom: {
    ListExpr first =
        nl->OneElemList(replaceSymbols(nl->First(list), replacements));
    ListExpr last = first;
    list = nl->Rest(list);
  ...
  case NoAtom: {
    if (nl->HasLength(list, 2)) {
      if (listutils::isSymbol(nl->First(list), writeVer)) {
        return nl->FourElemList(nl->SymbolAtom("write"),
                                replaceWrite(nl->Second(list), writeVer, name),
                                nl->StringAtom(name), nl->BoolAtom(false));
      }
    }

    ListExpr first =
        nl->OneElemList(replaceWrite(nl->First(list), writeVer, name));
    ListExpr last = first;
    list = nl->Rest(list);
    while (!nl->IsEmpty(list)) {
...
*/
template <class ResType> class ConnectionTask {
public:
  ConnectionTask(int _connection, CommandListener<ResType> *_listener)
      : connection(_connection), listener(_listener), no(0), done(false) {
  ...
  void removeListener() { listener = 0; }
...
private:
  void run() {
    ...
    while (!done) {
      ...
      ResType resList;
      double runtime;
      algInstance->simpleCommand(connection, cmd, error, errMsg, resList, true,
                                 runtime);
...
int connectVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  if (!si->Initialize(user, passwd, host, stringutils::int2str(port), file, "",
                      errMsg, true)) {
    msgcenter->Send(
        nl, nl->TwoElemList(nl->SymbolAtom("simple"), nl->TextAtom(errMsg)));
    delete si;
    si = 0;
    delete mynl;
    res->Set(true, false);
...
*/
OperatorSpec
    connectSpec("text x int x text -> bool", "connect(host, port, configfile)",
...
ValueMapping connectVM[] = {
    connectVMT<CcString, CcString>, connectVMT<CcString, FText>,
    connectVMT<FText, CcString>, connectVMT<FText, FText>};
...
*/
Operator connectOp("connect",            // name
...
ListExpr checkConnectionsTM(ListExpr args) {
  ...
  ListExpr attrList = nl->SixElemList(
      nl->TwoElemList(nl->SymbolAtom("Id"), listutils::basicSymbol<CcInt>()),
      nl->TwoElemList(nl->SymbolAtom("Host"), listutils::basicSymbol<FText>()),
      nl->TwoElemList(nl->SymbolAtom("Port"), listutils::basicSymbol<CcInt>()),
      nl->TwoElemList(nl->SymbolAtom("ConfigFile"),
                      listutils::basicSymbol<FText>()),
      nl->TwoElemList(nl->SymbolAtom("OK"), listutils::basicSymbol<CcBool>()),
      nl->TwoElemList(nl->SymbolAtom("PID"), listutils::basicSymbol<CcInt>()));
...
class checkConLocal {
public:
  checkConLocal(ListExpr resType) : pos(0) { tt = new TupleType(resType); }
  ~checkConLocal() { tt->DeleteIfAllowed(); }
  Tuple *next() {
...
int checkConnectionsVM(Word __attribute__((unused)) * args, Word &result,
...
OperatorSpec checkConnectionsSpec(
    ...
    "checkConnections()", "Check connections in the Distributed2Algebra",
...
*/
Operator
    checkConnectionsOp("checkConnections",            // name
...
ListExpr rcmdTM(ListExpr args) {
  ...
  ListExpr attrList =
      nl->FourElemList(nl->TwoElemList(nl->SymbolAtom("Connection"),
...
*/
template <class C>
int rcmdVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  switch (message) {
  case OPEN: {
    ...
    string errMsg;
    double runtime;
    bool ok = algInstance->simpleCommand(conv, cmd->GetValue(), errorCode,
                                         errMsg, result, true, runtime);
    if (!ok) {
...
*/
ValueMapping rcmdVM[] = {rcmdVMT<CcString>, rcmdVMT<FText>};
...
int disconnectVM(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
  int resi = 0;
  if (qp->GetNoSons(s) == 0) {
...
OperatorSpec disconnectSpec(
    " int -> int , ->int", "disconnect(), disconnect(_)",
...
*/
Operator disconnectOp("disconnect",            // name
...
ListExpr rqueryTM(ListExpr args) {
  ...
  if (error != 0) {
    return listutils::typeError(
        "Problem in determining result type : " + errMsg + "\n" + query);
  }
...
void sendMessage(MessageCenter *msg, const string &message) {
  ListExpr list =
      nl->TwoElemList(nl->SymbolAtom("simple"), nl->TextAtom(message));
  msg->Send(nl, list);
  msg->Flush();
}
...
template <class T>
int rqueryVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  ListExpr resList;
  double runtime;
  bool ok = algInstance->simpleCommand(serv, query->GetValue(), error, errMsg,
                                       resList, true, runtime);
  if (!ok || error) {
  ...
  ok = SecondoSystem::GetCatalog()->LookUpTypeExpr(typeList, typeName, algId,
                                                   typeId);
  ...
  Word res = am->InObj(algId, typeId)(nTypeList, valueList, errorPos, errorInfo,
                                      correct);
...
ValueMapping rqueryVM[] = {rqueryVMT<CcString>, rqueryVMT<FText>};
...
*/
OperatorSpec
    rquerySpec(" int x {string,text} -> ??", " rquery(server, query)",
...
ListExpr prcmdTM(ListExpr args) {
  ...
  ListExpr extAttr =
      nl->FourElemList(nl->TwoElemList(nl->SymbolAtom("ErrorCode"),
                                       listutils::basicSymbol<CcInt>()),
  ...
  ListExpr extList =
      nl->TwoElemList(nl->IntAtom(indexi - 1), nl->IntAtom(indext - 1));
  ListExpr resType = nl->TwoElemList(
      listutils::basicSymbol<Stream<Tuple>>(),
      nl->TwoElemList(listutils::basicSymbol<Tuple>(), resAttrList));
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), extList, resType);
}

template <class T> class prcmdInfo : public CommandListener<string> {
...
public:
  ...
  virtual ~prcmdInfo() {
    ...
    map<int, Tuple *>::iterator it2;
    {
      ...
      for (it2 = validInputTuples.begin(); it2 != validInputTuples.end();
           it2++) {
  ...
  void jobDone(int id, string &command, size_t no, int error, string &errMsg,
               string &resList, double runtime) {
...
template <class T>
int prcmdVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  case OPEN: {
    ...
    local.addr = new prcmdInfo<T>(args[0], ((CcInt *)args[3].addr)->GetValue(),
...
ValueMapping prcmdVM[] = {prcmdVMT<CcString>, prcmdVMT<FText>};

int prcmdSelect(ListExpr args) {
  ListExpr attrList = nl->Second(nl->Second(nl->First(args)));
  ListExpr textType;
  listutils::findAttribute(attrList, nl->SymbolValue(nl->Third(args)),
                           textType);
...
int sendFileVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  res->Set(true,
           algInstance->sendFile(server, Local->GetValue(), Remote->GetValue(),
...
ValueMapping sendFileVM[] = {
    sendFileVMT<CcString, CcString>, sendFileVMT<CcString, FText>,
    sendFileVMT<FText, CcString>, sendFileVMT<FText, FText>};
...
Operator sendFileOp("sendFile",            // name
...
int requestFileVMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  CcInt *Server = (CcInt *)args[0].addr;
  ...
  int server = Server->GetValue();
  res->Set(true, algInstance->requestFile(server, Remote->GetValue(),
                                          Local->GetValue(),
                                          AllowOverwrite->GetValue()));
  return 0;
}
...
ValueMapping requestFileVM[] = {
    requestFileVMT<CcString, CcString>, requestFileVMT<CcString, FText>,
    requestFileVMT<FText, CcString>, requestFileVMT<FText, FText>};
...
Operator requestFileOp("requestFile",            // name
...
ListExpr ptransferFileTM(ListExpr args) {
  ...
  if (nl->HasLength(args, 5)) { // full parameter set
    ...
    appendList = nl->FourElemList(
        nl->IntAtom(indexServer - 1), nl->IntAtom(indexLocal - 1),
        nl->IntAtom(indexRemote - 1), nl->IntAtom(indexOver - 1));
    ...
    appendList = nl->FiveElemList(
        ...
        nl->IntAtom(indexServer - 1), nl->IntAtom(indexLocal - 1),
        nl->IntAtom(indexRemote - 1), nl->IntAtom(indexOver));
  }

  // input ok, generate output
  ListExpr extAttr =
      nl->TwoElemList(nl->TwoElemList(nl->SymbolAtom("ErrorCode"),
                                      listutils::basicSymbol<CcInt>()),
                      nl->TwoElemList(nl->SymbolAtom("ErrMsg"),
                                      listutils::basicSymbol<FText>()));
  ...
  ListExpr resultList = nl->TwoElemList(
      listutils::basicSymbol<Stream<Tuple>>(),
      nl->TwoElemList(listutils::basicSymbol<Tuple>(), newAttrList));
...
class sendFunctor {
public:
  int operator()(const int server, const string &source, const string &target,
                 const bool allowOverwrite) {
...
template <class T> class fileTransferator {
public:
  /*
  ~Constructor~
  ...
  */
  fileTransferator(int _server, transferFileListener *_listener)
    ...
    runnerThread = boost::thread(boost::bind(&fileTransferator::run, this));
  }
  ...
  */
  ...
  void addTransfer(Tuple *inTuple, const string &local, const string &remote,
                   const bool allowOverwrite) {
  ...
  void run() {
    ...
    while (!stopped) {
      ...
      if (!stopped) {
        ...
        int ret = functor(server, t.local, t.remote, t.allowOverwrite);
        if (listener) {
...
template <class L, class R, class T> class ptransferFileInputProcessor {
public:
  /*
  ~Constructors~

  */
  ptransferFileInputProcessor(Word &_stream, int _serverIndex, int _localIndex,
    ...
    while (inTuple && !stopped) {
      ...
      if (!Server->IsDefined() || !Local->IsDefined() || !Remote->IsDefined() ||
        ...
        if (server < 0 || server >= (int)algInstance->noConnections()) {
          ...
          processValidTuple(inTuple, server, localname, remotename, overwrite);
        }
      }
      inTuple = stream.request();
    }
...
private:
  ...
  map<int, fileTransferator<T> *> activeThreads; // threads for each connection
  ...
  */
  void processValidTuple(Tuple *inTuple, const int server, const string &local,
                         const string &remote, const bool allowOverwrite) {

    if (activeThreads.find(server) == activeThreads.end()) {
      fileTransferator<T> *fi = new fileTransferator<T>(server, listener);
      activeThreads[server] = fi;
    }
    activeThreads[server]->addTransfer(inTuple, local, remote, allowOverwrite);
  }
};
...
public:
  ...
  */
  ...
  ptransferLocal(Word &stream, int _serverindex, int _localindex,
                 int _remoteindex, int _overwrite, ListExpr _tt) {
    inputTuples = -1; // unknown number of input tuples
    outputTuples = 0; // up to now, there is no output tuple
    tt = new TupleType(_tt);
    inputProcessor = new ptransferFileInputProcessor<L, R, T>(
        stream, _serverindex, _localindex, _remoteindex, _overwrite, this);
...
template <class L, class R, class T>
int ptransferFileVMT(Word *args, Word &result, int message, Word &local,
                     Supplier s) {
  ...
  case OPEN: {
    ...
    local.addr = new ptransferLocal<L, R, T>(args[0], serv, loc, rem, over, tt);
    return 0;
  }
  case REQUEST:
    result.addr = li ? li->next() : 0;
...
ValueMapping psendFileVM[] = {ptransferFileVMT<CcString, CcString, sendFunctor>,
...
Operator psendFileOp("psendFile",            // name
...
Operator prequestFileOp("prequestFile",            // name
...
template <bool send>
int getFolderVM(Word *args, Word &result, int message, Word &local,
                Supplier s) {
...
OperatorSpec getRequestFolderSpec(
    " int x bool -> text ", " getRequestFolder(_)  ",
...
OperatorSpec getSendFolderSpec(
    " int x bool -> text ", " getSendFolder(_)  ",
    " Returns the name of the folder on the server for a given connection"
...
Operator
    getRequestFolderOp("getRequestFolder",            // name
...
Operator getSendFolderOp("getSendFolder",            // name
...
ListExpr pconnectTM(ListExpr args) {
  ...
  if (nl->AtomType(a1) != SymbolType || nl->AtomType(a2) != SymbolType ||
  ...
  if (!CcInt::checkType(type)) {
    return listutils::typeError("Attribute " + n2 + " not of type int");
  }
  ...
  ListExpr resAttrList = listutils::concat(
      attrList, nl->OneElemList(nl->TwoElemList(
                    nl->SymbolAtom("CNo"), listutils::basicSymbol<CcInt>())));
  ...
  if (nl->HasLength(args, 5)) {
    ...
    appendList = nl->ThreeElemList(nl->IntAtom(hostIndex - 1),
                                   nl->IntAtom(portIndex - 1),
                                   nl->IntAtom(configIndex - 1));
  } else {
    appendList = nl->FourElemList(
        nl->BoolAtom(false), nl->IntAtom(hostIndex - 1),
        nl->IntAtom(portIndex - 1), nl->IntAtom(configIndex - 1));
  }
...
public:
  virtual void connectionDone(Tuple *inputTuple, int no,
                              ConnectionInfo *ci) = 0;
};
...
public:
  Connector(Tuple *_inTuple, const int _inTupleNo, const string &_host,
            const int _port, const string &_config,
            ConnectionListener *_listener)
      : inTuple(_inTuple), inTupleNo(_inTupleNo), host(_host), port(_port),
        config(_config), listener(_listener) {
    mythread = boost::thread(boost::bind(&Connector::run, this));
  }

  ~Connector() { mythread.join(); }
...
private:
  ...
  void run() {
    ...
    si->setTimeout(defaultTimeout);
    string errMsg;
    if (!si->Initialize("", "", host, stringutils::int2str(port), config, "",
                        errMsg, true)) {
      listener->connectionDone(inTuple, inTupleNo, 0);
      delete si;
      si = 0;
...
template <class H, class C> class pconnectLocal : public ConnectionListener {
public:
  pconnectLocal(Word &_stream, int _hostIndex, int _portIndex, int _configIndex,
                ListExpr _tt, bool _det)
    ...
    tt = new TupleType(_tt);
    stream.open();
    runnerThread = boost::thread(boost::bind(&pconnectLocal::run, this));
  }
...
private:
  ...
  priority_queue<ConnectInfoElem, vector<ConnectInfoElem>,
                 greater<ConnectInfoElem>>
      cache;
  ...
  void run() {
    ...
    while (!stopped && inTuple != 0) {
      ...
      if (!Host->IsDefined() || !Port->IsDefined() || !Config->IsDefined()) {
        Connector *connector =
            new Connector(inTuple, noTuples, "", -1, "", this);
        connectors.push_back(connector);
      } else {
        Connector *connector =
            new Connector(inTuple, noTuples, Host->GetValue(), Port->GetValue(),
                          Config->GetValue(), this);
        connectors.push_back(connector);
      }
      inTuple = stream.request();
    }
  ...
  */
  void connectionDoneNonDet(Tuple *inTuple, int inTupleNo, ConnectionInfo *ci) {
    if (!inTuple) {
      {
  ...
  */
  void connectionDoneDet(Tuple *inTuple, int inTupleNo, ConnectionInfo *ci) {
...
*/
template <class H, class C>
int pconnectVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  case OPEN: {
    ...
    ListExpr tt = nl->Second(GetTupleResultType(s));
    local.addr = new pconnectLocal<H, C>(args[0], hostIndex, portIndex,
                                         configIndex, tt, det->GetValue());
    return 0;
  }
  case REQUEST:
    result.addr = li ? li->next() : 0;
...
ValueMapping pconnectVM[] = {
    pconnectVMT<CcString, CcString>, pconnectVMT<CcString, FText>,
    pconnectVMT<FText, CcString>, pconnectVMT<FText, FText>};
...
Operator pconnectOp("pconnect",            // name
...
bool getQueryType(const string &query1, int serverNo, ListExpr &result,
                  string &errMsg) {
  string query = query1;
  ...
  ListExpr reslist;
  double runtime;
  bool ok = algInstance->simpleCommand(serverNo, query, error, errMsg, reslist,
                                       true, runtime);
  if (!ok) {
...
ListExpr prqueryTM(ListExpr args) {
  ...
  ListExpr attrList =
      nl->TwoElemList(nl->TwoElemList(nl->SymbolAtom("ServerNo"),
                                      listutils::basicSymbol<CcInt>()),
                      nl->TwoElemList(nl->SymbolAtom("Result"), resType));
  ...
  virtual void jobDone(int id, string &command, size_t no, int error,
...

    {
      ...
      if (nl->HasLength(valueList, 2)) {
        res = inObject(nTypeList, nl->Second(valueList), errorPos, errorInfo,
                       correct);
      } else {
        correct = false;
      }
...
*/
template <class T>
int prqueryVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
...
ValueMapping prqueryVM[] = {prqueryVMT<CcString>, prqueryVMT<FText>};
...
*/
Operator prqueryOp("prquery",            // name
...
ListExpr prquery2TM(ListExpr args) {
  ...
  ListExpr resAttrList = listutils::concat(
      attrList,
      nl->OneElemList(nl->TwoElemList(nl->SymbolAtom("Result"), resType)));
  if (!listutils::isAttrList(resAttrList)) {
  ...
  return nl->ThreeElemList(
      ...
      nl->TwoElemList(nl->IntAtom(serverPos - 1), nl->IntAtom(partPos - 1)),
      nl->TwoElemList(
          listutils::basicSymbol<Stream<Tuple>>(),
          nl->TwoElemList(listutils::basicSymbol<Tuple>(), resAttrList)));
}
...
public:
  PRQuery2Info(Word &_stream, const string &_query, int _serverAttr,
               int _partAttr, ListExpr _tt)
      : stream(_stream), serverAttr(_serverAttr), partAttr(_partAttr),
        query(_query) {
    ...
    string tname;
    int algId, typeId;
    SecondoSystem::GetCatalog()->LookUpTypeExpr(resType, tname, algId, typeId);
  ...
  virtual void jobDone(int id, string &command, size_t no, int error,
...

    {
      ...
      if (nl->HasLength(valueList, 2)) {
        res = inObject(resType, nl->Second(valueList), errorPos, errorInfo,
                       correct);
      } else {
        correct = false;
      }
...
template <class T>
int prquery2VMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  case OPEN: {
    ...
    algInstance->initProgress();
    local.addr = new PRQuery2Info(
        args[0], q->GetValue(), ((CcInt *)args[6].addr)->GetValue(),
        ((CcInt *)args[7].addr)->GetValue(), qp->GetType(s));
    return 0;
  }
  case REQUEST:
    result.addr = li ? li->next() : 0;
...
ValueMapping prquery2VM[] = {prquery2VMT<CcString>, prquery2VMT<FText>};
...
*/
Operator prquery2Op("prquery2",            // name
...
ListExpr PDFArrayProperty() {
  return nl->TwoElemList(
      nl->FourElemList(
          ...
          nl->StringAtom("List Rep"), nl->StringAtom("Example List")),
      nl->FourElemList(
          nl->StringAtom(" -> SIMPLE"), nl->StringAtom(" (pdfarray rel(...))"),
          nl->TextAtom("(name <map> ( s1 s2  ...)(u1 ...u_m)) where "
                       "s_i =(server port config), u_i = int (used slot)"),
...
*/
template <class A> ListExpr OutDArray(ListExpr typeInfo, Word value) {
  A *da = (A *)value.addr;
  assert(da);
...
*/
template <class A> Word CreateDArray(const ListExpr typeInfo) {
...
*/
template <class A> void DeleteDArray(const ListExpr typeInfo, Word &w) {
  A *a = (A *)w.addr;
...
template <class A> void CloseDArray(const ListExpr typeInfo, Word &w) {
  A *a = (A *)w.addr;
  delete a;
  w.addr = 0;
}

template <class A> Word CloneDArray(const ListExpr typeInfo, const Word &w) {
  A *a = (A *)w.addr;
  Word res;
  res.addr = new A(*a);
  return res;
...
template <class A> void *CastDArray(void *addr) { return (new (addr) A(0)); }

template <class A> bool DistTypeBaseCheck(ListExpr type, ListExpr &errorInfo) {
...
TypeConstructor DArrayTC(DArray::BasicType(), DArrayProperty, OutDArray<DArray>,
                         InDArray<DArray>, 0, 0, CreateDArray<DArray>,
                         DeleteDArray<DArray>, DArray::open<DArray, false>,
                         DArray::save<false>, CloseDArray<DArray>,
                         CloneDArray<DArray>, CastDArray<DArray>, SizeOfDArray,
                         DistTypeBaseCheck<DArray>);

TypeConstructor DFArrayTC(DFArray::BasicType(), DFArrayProperty,
                          OutDArray<DFArray>, InDArray<DFArray>, 0, 0,
                          ...
                          CastDArray<DFArray>, SizeOfDArray,
                          DistTypeBaseCheck<DFArray>);

TypeConstructor PDArrayTC(PDArray::BasicType(), PDArrayProperty,
                          OutDArray<PDArray>, InDArray<PDArray>, 0, 0,
                          ...
                          CastDArray<PDArray>, SizeOfDArray,
                          DistTypeBaseCheck<PDArray>);

TypeConstructor PDFArrayTC(PDFArray::BasicType(), PDFArrayProperty,
                           OutDArray<PDFArray>, InDArray<PDFArray>, 0, 0,
                           ...
                           CastDArray<PDFArray>, SizeOfDArray,
                           DistTypeBaseCheck<PDFArray>);

TypeConstructor DFMatrixTC(DFMatrix::BasicType(), DFMatrixProperty,
                           OutDArray<DFMatrix>, InDArray<DFMatrix>, 0, 0,
                           CreateDFMatrix, DeleteDArray<DFMatrix>,
                           DFMatrix::open, DFMatrix::save,
                           CloseDArray<DFMatrix>, CloneDArray<DFMatrix>,
                           CastDArray<DFMatrix>, SizeOfDArray,
                           DistTypeBaseCheck<DFMatrix>);

TypeConstructor FRelTC(frel::BasicType(), frel::Property, frel::Out, frel::In,
                       0, 0, frel::Create, frel::Delete, frel::Open, frel::Save,
                       frel::Close, frel::Clone, frel::Cast, frel::SizeOf,
                       frel::TypeCheck);

TypeConstructor FSRelTC(fsrel::BasicType(), fsrel::Property, fsrel::Out,
                        fsrel::In, 0, 0, fsrel::Create, fsrel::Delete,
                        fsrel::Open, fsrel::Save, fsrel::Close, fsrel::Clone,
                        fsrel::Cast, fsrel::SizeOf, fsrel::TypeCheck);

TypeConstructor FObjTC(fobj::BasicType(), fobj::Property, fobj::Out, fobj::In,
                       0, 0, fobj::Create, fobj::Delete, fobj::Open, fobj::Save,
                       fobj::Close, fobj::Clone, fobj::Cast, fobj::SizeOf,
                       fobj::TypeCheck);

TypeConstructor SDArrayTC(SDArray::BasicType(), SDArray::Property, SDArray::Out,
                          fobj::In, 0, 0, SDArray::Create, SDArray::Delete,
                          SDArray::Open, SDArray::Save, SDArray::Close,
                          SDArray::Clone, SDArray::Cast, SDArray::SizeOf,
                          SDArray::TypeCheck);
...
*/
template <class A>
int putVMA(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  if (!ci->createOrUpdateObject(n, nl->Second(qp->GetType(s)), args[2],
...
int putVMFA(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  string dbname = SecondoSystem::GetInstance()->GetDatabaseName();
  ConnectionInfo *ci =
      algInstance->getWorkerConnection(array->getWorkerForSlot(index), dbname);
  if (!ci) {
  ...
  string targetDir = ci->getSecondoHome(showCommands, commandLog) +
                     "/dfarrays/" + dbname + "/" + array->getName() + "/";
  string cmd = "query createDirectory('" + targetDir + "', TRUE)";
  int err;
  string errMsg;
  ListExpr resList;
  double runtime;
  ci->simpleCommand(cmd, err, errMsg, resList, false, runtime, showCommands,
...
OperatorSpec
    putSpec(" {d[f]array(T), sdarray}  x int x T -> d[f]array(T) ",
            " put(_,_,_)",
...
int putSelect(ListExpr args) {
  ...
  if (DArray::checkType(a1))
    return 0;
  if (DFArray::checkType(a1))
    return 1;
  if (SDArray::checkType(a1))
    return 2;
  return -1;
}
...
Operator putOp("put", putSpec.getStr(), 3, putVM, putSelect, putTM);
...
ListExpr getTM(ListExpr args) {
  ...
  if (!DArray::checkType(nl->First(args)) &&
      !DFArray::checkType(nl->First(args)) &&
      !SDArray::checkType(nl->First(args))) {
    return listutils::typeError(err + " ( first is not a darray or a dfarray)");
  }
  if (!CcInt::checkType(nl->Second(args))) {
...
*/
template <class DA>
int getVMDA(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  bool ok = ci->retrieve(objName, resType, r, true, showCommands, commandLog, i,
...
int getVMDFA(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  ;
  if (ci) {
    string fileName = ci->getSecondoHome(showCommands, commandLog) +
                      "/dfarrays/" + dbname + "/" + array->getName() + "/" +
                      array->getName() + "_" + stringutils::int2str(pos) +
                      ".bin";
    ci->retrieveRelationInFile(fileName, resType, relResult, showCommands,
                               commandLog, false, algInstance->getTimeout());
...
OperatorSpec getSpec(" {darray(T), dfarray(T), sdarray}  x int -> T ",
                     " get(_,_)",
...
ValueMapping getVM[] = {getVMDA<DArray>, getVMDFA, getVMDA<SDArray>};

int getSelect(ListExpr args) {
  ListExpr a = nl->First(args);
  if (DArray::checkType(a))
    return 0;
  if (DFArray::checkType(a))
    return 1;
  if (SDArray::checkType(a))
    return 2;
  return -1;
}

Operator getOp("get", getSpec.getStr(), 3, getVM, getSelect, getTM);
...
template <class A>
int sizeVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
...
OperatorSpec sizeSpec(" d[f]array(T) -> int", " size(_)",
...
ValueMapping sizeVM[] = {sizeVMT<DArray>, sizeVMT<DFArray>, sizeVMT<DFMatrix>,
                         sizeVMT<SDArray>};

Operator sizeOp("size", sizeSpec.getStr(), 4, sizeVM, sizeSelect, sizeTM);
...
ListExpr getWorkersTM(ListExpr args) {
  ...
  ListExpr attrList = nl->FiveElemList(
      ...
      nl->TwoElemList(nl->SymbolAtom("Config"),
                      listutils::basicSymbol<FText>()),
      nl->TwoElemList(nl->SymbolAtom("No"), listutils::basicSymbol<CcInt>()),
      nl->TwoElemList(nl->SymbolAtom("Id"), listutils::basicSymbol<LongInt>()));
  return nl->TwoElemList(
      listutils::basicSymbol<Stream<Tuple>>(),
      nl->TwoElemList(listutils::basicSymbol<Tuple>(), attrList));
}

template <class A> class getWorkersInfo {
public:
  getWorkersInfo(A *_array, ListExpr _tt) : array(_array), pos(0) {
    tt = new TupleType(_tt);
    dbname = SecondoSystem::GetInstance()->GetDatabaseName();
  }

  ~getWorkersInfo() { tt->DeleteIfAllowed(); }
...
template <class A>
int getWorkersVMT(Word *args, Word &result, int message, Word &local,
                  Supplier s) {
  ...
  case OPEN: {
    ...
    if (arg->IsDefined()) {
      local.addr =
          new getWorkersInfo<A>(arg, nl->Second(GetTupleResultType(s)));
    }
    return 0;
  }
  case REQUEST:
...
OperatorSpec
    getWorkersSpec(" d[f]array(T) -> stream(tuple(...)) ", " getWorkers(_)",
                   " Returns information about workers in a d[f]array.",
                   " query getWorkers([const darray(int) value"
                   "           (da1 4 ((\"host1\" 1234 \"Config1.ini\")"
                   "           (\"host2\" 1234 \"Config2.ini\")))] ) count");

ValueMapping getWorkersVM[] = {getWorkersVMT<DArray>, getWorkersVMT<DFArray>,
                               getWorkersVMT<SDArray>};

int getWorkersSelect(ListExpr args) {
  ListExpr f = nl->First(args);
  if (DArray::checkType(f))
    return 0;
  if (DFArray::checkType(f))
    return 1;
  if (SDArray::checkType(f))
    return 2;
  return -1;
}

Operator getWorkersOp("getWorkers", getWorkersSpec.getStr(), 3, getWorkersVM,
                      getWorkersSelect, getWorkersTM);
...
ListExpr fconsume5TM(ListExpr args) {
  ...
  if (nl->HasLength(args, 2)) {
    return nl->ThreeElemList(
        nl->SymbolAtom(Symbols::APPEND()),
        nl->TwoElemList(nl->BoolAtom(true), nl->BoolAtom(false)),
        nl->First(args));
  }
...
class fconsume5Info {
public:
  fconsume5Info(Word &_stream, const string &_filename, const ListExpr typeList,
                const bool storeToDFS)
      : in(_stream) {
    binrelwriter =
        new BinRelWriter(_filename, typeList, FILE_BUFFER_SIZE, storeToDFS);
    ok = binrelwriter->ok();
    in.open();
    firstError = true;
  }

  fconsume5Info(Word &_stream, const ListExpr typeList) : in(_stream) {
    ok = false;
    firstError = true;
    binrelwriter = 0;
  }
...
template <class T>
int fconsume5VMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
  ...
  case OPEN: {
    ...
    type =
        nl->TwoElemList(listutils::basicSymbol<Relation>(), nl->Second(type));
...
ValueMapping fconsume5VM[] = {fconsume5VMT<CcString>, fconsume5VMT<FText>};
...
*/
Operator fconsume5Op("fconsume5", fconsume5Spec.getStr(), 2, fconsume5VM,
                     fconsume5Select, fconsume5TM);
...
ListExpr ffeed5TM(ListExpr args) {
  ...
  if (!in.good()) {
    in.close();
    return listutils::typeError("problem in reading from file " + filename);
  }
  string m(marker, 4);
...
template <class T>
int ffeed5VMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  case OPEN: {
    ...
    local.addr = new ffeed5Info(n, nl->Second(GetTupleResultType(s)));
    return 0;
  }
  case REQUEST: {
    result.addr = li ? li->next() : 0;
...
public:
  ffeed5fsrelInfo(fsrel *_fn, ListExpr _tt)
      : fn(_fn), tt(new TupleType(_tt)), pos(0), li(0) {
    assert(fn);
  }
...
int ffeed5VMfsrel(Word *args, Word &result, int message, Word &local,
                  Supplier s) {
...
ValueMapping ffeed5VM[] = {ffeed5VMT<CcString>, ffeed5VMT<FText>,
                           ffeed5VMT<frel>, ffeed5VMfsrel};
...
OperatorSpec ffeed5Spec(
    ...
    " _ ffeed5", " Restores  a tuple stream from a binary file. ",
    " query 'ten.bin' ffeed5 count ");

OperatorSpec feedSpec(
    " {string, text} -> stream(TUPLE)  | f[s]rel(tuple(X)) -> stream(tuple(X))",
    " _ feed", " Restores  a tuple stream from a binary file. ",
...
*/
Operator ffeed5Op("ffeed5", ffeed5Spec.getStr(), 4, ffeed5VM, ffeed5Select,
                  ffeed5TM);

Operator feedOp("feed", feedSpec.getStr(), 4, ffeed5VM, ffeed5Select, ffeed5TM);
...
template <class A>
int fcount5VMT(Word *args, Word &result, int message, Word &local, Supplier s) {
...
ValueMapping fcount5VM[] = {fcount5VMT<CcString>, fcount5VMT<FText>,
                            fcount5VMT<frel>};
...
OperatorSpec fcount5Spec("{string,text,frel} -> int", "_ fcount5 ",
                         "Returns the number of tuples stored in a binary "
                         "file containing a relation. ",
                         "query 'testrel.bin' fcount5");

Operator fcount5Op("fcount5", fcount5Spec.getStr(), 3, fcount5VM, fcount5Select,
                   fcount5TM);
...
ListExpr createDArrayTM(ListExpr args) {
  ...
  if (!CcInt::checkType(pt)) {
    return listutils::typeError("port attribute " + pn + " not of type int");
  }
  ...
  ListExpr resType =
      nl->TwoElemList(listutils::basicSymbol<DArray>(), nl->Fourth(args));
  ...
  if (nl->HasLength(args, 4)) {
    return nl->ThreeElemList(
        nl->SymbolAtom(Symbols::APPEND()),
        nl->FourElemList(nl->BoolAtom(false), nl->IntAtom(hp - 1),
                         nl->IntAtom(pp - 1), nl->IntAtom(cp - 1)),
        resType);
  }
  if (!CcBool::checkType(nl->Fifth(args))) {
...
*/
template <class T> vector<vector<pair<DArrayElement, size_t>>> group(T &array) {

  typedef map<pair<string, string>, vector<pair<DArrayElement, size_t>>> groupt;
...
template <class H, class C>
int createDArrayVMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  try {
    ...
    for (size_t i = 0; i < groups.size(); i++) {
      ...
      string query = "query getcatalog() filter[regexmatches(.ObjectName,"
                     " [const regex value '" +
                     re + "' ])] project[ObjectName, TypeExpr] consume";
      ListExpr reslist;
      int err;
      string errMsg;
      double runtime;
...
ValueMapping createDArrayVM[] = {
    createDArrayVMT<CcString, CcString>, createDArrayVMT<CcString, FText>,
    createDArrayVMT<FText, CcString>, createDArrayVMT<FText, FText>};
...
*/
Operator createDArrayOp("createDArray", createDArraySpec.getStr(), 4,
                        createDArrayVM, createDArraySelect, createDArrayTM);
...
template <class A> class SinglePutter {

public:
  SinglePutter(ListExpr _type, A *_array, int _arrayIndex, Word &_value)
      : type(_type), array(_array), arrayIndex(_arrayIndex), value(_value) {
    runner = boost::thread(&SinglePutter<A>::run, this);
  }

  ~SinglePutter() { runner.join(); }
...
private:
  ...
  void run() {
    string dbname = SecondoSystem::GetInstance()->GetDatabaseName();
    DArrayElement elem = array->getWorkerForSlot(arrayIndex);
    ConnectionInfo *ci =
        algInstance->getWorkerConnection(array, arrayIndex, dbname, 0, true);
    ...
    while (!done && ci) {
      done = ci->createOrUpdateObject(objname, type, value, showCommands,
                                      commandLog, false,
                                      algInstance->getTimeout());
      if (!done) {
...
template <class A> class PPutter {
public:
  PPutter(ListExpr _type, A *_arg, vector<pair<int, Word>> &_values) {
    this->type = _type;
    this->source = _arg;
    set<int> used;
  ...
  void start() {
    ...
    if (!started) {
      ...
      for (size_t i = 0; i < values.size(); i++) {
        SinglePutter<A> *sp = new SinglePutter<A>(type, source, values[i].first,
                                                  values[i].second);
...
*/
template <class A>
int pputVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  while (i < sons) {
    ...
    if (Index->IsDefined()) {
      ...
      if ((index >= 0) && ((size_t)index < arg->getSize()) &&
          (used.find(index) == used.end())) {
...
int pputSelect(ListExpr args) { return 0; }
...
OperatorSpec pputSpec(" [s]darray(T) x (int x T)+ -> darray ",
...
Operator pputOp("pput", pputSpec.getStr(), 1, pputVM, pputSelect, pputTM);
...
template <class R> ListExpr ddistribute2TMT(ListExpr args) {
  ...
  string errMsg;
  if (!isWorkerRelDesc(nl->Fifth(args), workerAttrPositions, workerAttrTypes,
                       errMsg)) {
  ...
  ListExpr appendList = listutils::concat(
      workerAttrPositions, nl->OneElemList(nl->IntAtom(pos - 1)));

  ListExpr res =
      nl->TwoElemList(listutils::basicSymbol<R>(),
                      nl->TwoElemList(listutils::basicSymbol<Relation>(),
                                      nl->Second(nl->First(args))));

  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), appendList, res);
}
...
public:
  RelFileRestorer(const string &_objName, DArray *_array, int _arrayIndex,
                  const string &_filename)
      : objNames(), array(_array), arrayIndex(_arrayIndex), filenames(),
        started(false) {
    objNames.push_back(_objName);
    filenames.push_back(_filename);
    string dbname = SecondoSystem::GetInstance()->GetDatabaseName();
    ci = algInstance->getWorkerConnection(array, arrayIndex, dbname, 0, true);
  }

  RelFileRestorer(const vector<string> &_objNames, DArray *_array,
                  int _arrayIndex, const vector<string> &_filenames)
      : objNames(_objNames), array(_array), arrayIndex(_arrayIndex),
        filenames(_filenames), started(false) {
    ...
    if (ci) { // initial getting of connectioninfor was successful
      ...
      while (!done && ci) {
        ...
        for (size_t i = 0; i < objNames.size() && !error; i++) {
          ...
          res = ci->createOrUpdateRelationFromBinFile(
              objName, filename, showCommands, commandLog,
              true,  // allowOverwrite
              false, // forceExec
              algInstance->getTimeout());
          if (!res) {
        ...
        if (!error) {
          ...
          // another connection
          ci = changeWorker1(array, arrayIndex, usedWorkers, reconnects, ci);
        }
      }
    } else {
...
public:
  FRelCopy(const string &_objName, DFArray *_array, int _arrayIndex,
           const string &_filename)
  ...
  FRelCopy(const vector<string> &_objNames, DFArray *_array, int _arrayIndex,
           const vector<string> &_filenames)
      : names(_filenames), slot(_arrayIndex), array(_array), started(false),
        ci(0) {
    dbname = SecondoSystem::GetInstance()->GetDatabaseName();
    ci = algInstance->getWorkerConnection(array, slot, dbname, 0, true);
  }
  ...
  void run() {
    ...
    while (!done && ci) {
      ...
      for (size_t i = 0; i < names.size() && done; i++) {
        ...
        string targetDir =
            home + "/dfarrays/" + dbname + "/" + array->getName();
        ...
        if (!nl->HasLength(result, 2) ||
          ...
          cerr << "moveFile returns unexpected result: " << nl->ToString(result)
               << endl;
        }
...
int ddistribute2VMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  res->copyFrom(DArrayBase::createFromRel<HostType, ConfigType, AType>(
      rel, size, name, hostPos, portPos, configPos));
  ...
  for (size_t i = 0; i < res->getSize(); i++) {
    FileSystem::DeleteFileOrFolder(name + "_" + stringutils::int2str(i) +
                                   ".bin");
  }
...
*/
Operator ddistribute2Op("ddistribute2", ddistribute2Spec.getStr(), 4,
                        ddistribute2VM, distribute2Select,
...
*/
template <class R> ListExpr distribute3TM(ListExpr args) {
  ...
  ListExpr resType =
      nl->TwoElemList(listutils::basicSymbol<R>(),
                      nl->TwoElemList(listutils::basicSymbol<Relation>(),
                                      nl->Second(nl->First(args))));
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), positions,
                           resType);
}
...
int distribute3VMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  ...
  if (!size->IsDefined() || !method->IsDefined() || !n->IsDefined()) {
    res->makeUndefined();
    return 0;
  }
  ...
  res->copyFrom(DArrayBase::createFromRel<HType, CType, AType>(
      workers, 1, name, hostPos, portPos, configPos));
...
Operator ddistribute3Op("ddistribute3", ddistribute3Spec.getStr(), 4,
                        ddistribute3VM, distribute3Select,
...
*/
template <class R> ListExpr distribute4TMT(ListExpr args) {
  ...
  ListExpr res =
      nl->TwoElemList(listutils::basicSymbol<R>(),
                      nl->TwoElemList(listutils::basicSymbol<Relation>(),
                                      nl->Second(nl->First(args))));
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), positions, res);
}
...
*/
template <class AType, class DType, class HType, class CType>
int distribute4VMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  ...
  (*res) = DArrayBase::createFromRel<HType, CType, AType>(
      rel, size, name, hostPos, portPos, configPos);
...
Operator ddistribute4Op("ddistribute4", ddistribute4Spec.getStr(), 4,
                        ddistribute4VM, distribute4Select,
...
*/
template <class R> ListExpr distribute5TMT(ListExpr args) {
...
*/
template <class AType, class DType, class HType, class CType>
int distribute5VMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  ...
  for (int i = 0; (unsigned)i < size; ++i) {
    ...
    std::string workers_hashing = host + "_" + std::to_string(i) + "_" +
                                  std::to_string(port) + "_" + config;
...
Operator ddistribute5Op("ddistribute5", ddistribute5Spec.getStr(), 4,
                        ddistribute5VM, distribute5Select,
...
Operator dfdistribute5Op("dfdistribute5", dfdistribute5Spec.getStr(), 4,
                         dfdistribute5VM, distribute5Select,
...
template <class T> class fdistribute5Info {
public:
...
private:
  ...
  void writeNextTuple(Tuple *tuple) {
    ...
    BinRelWriter *out;
    if (it == writers.end()) {
      out = new BinRelWriter(basename + "_" + stringutils::int2str(f) + ".bin",
                             relType, bufsize);
      writers[f] = out;
    } else {
      out = it->second;
...
template <class T>
int fdistribute5VMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  case OPEN: {
    ...
    local.addr =
        new fdistribute5Info<T>(args[0], size, fname, attrPos, relType);
    return 0;
  }
  case REQUEST:
    result.addr = li ? li->next() : 0;
...
ValueMapping fdistribute5VM[] = {fdistribute5VMT<CcString>,
...
*/
Operator fdistribute5Op("fdistribute5", fdistribute5Spec.getStr(), 2,
                        fdistribute5VM, fdistribute5Select, fdistribute5TM);
...
template <class A>
int closeWorkersVMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
...
ValueMapping closeWorkersVM[] = {closeWorkersVMT<DArray>,
...
OperatorSpec
    closeWorkersSpec(" -> int, {d[f]array, sdarray}  -> int ",
                     " closeWorkers(_)",
...
Operator closeWorkersOp("closeWorkers", closeWorkersSpec.getStr(), 3,
                        closeWorkersVM, closeWorkersSelect, closeWorkersTM);
...
ListExpr showWorkersTM(ListExpr args) {
  ...
  ListExpr attrList = nl->Cons(
      nl->TwoElemList(nl->SymbolAtom("Host"), listutils::basicSymbol<FText>()),
      nl->SixElemList(nl->TwoElemList(nl->SymbolAtom("Port"),
                                      listutils::basicSymbol<CcInt>()),
                      nl->TwoElemList(nl->SymbolAtom("ConfigFile"),
                                      listutils::basicSymbol<FText>()),
...
template <class A> class showWorkersInfo {
...
public:
  ...
  ~showWorkersInfo() { tt->DeleteIfAllowed(); }
...
private:
  ...
  Tuple *nextFromAll() {
    ...
    Tuple *res =
        createTuple(iter->first, iter->second.first, iter->second.second);
    iter++;
    return res;
  }
  ...
  Tuple *createTuple(const DArrayElement &elem, string &dbname,
    ...
    bool ok =
        ci ? ci->check(showCommands, commandLog, algInstance->getTimeout())
           : false;
...
template <class A>
int showWorkersVMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
...
ValueMapping showWorkersVM[] = {showWorkersVMT<DArray>, showWorkersVMT<DFArray>,
                                showWorkersVMT<SDArray>};

int showWorkersSelect(ListExpr args) {

  if (nl->IsEmpty(args) || DArray::checkType(nl->First(args)))
    return 0;
...
Operator showWorkersOp("showWorkers", showWorkersSpec.getStr(), 3,
                       showWorkersVM, showWorkersSelect, showWorkersTM);
...
ListExpr dloopTM(ListExpr args) {
  ...
  ListExpr result =
      nl->TwoElemList(listutils::basicSymbol<DArray>(), nl->Third(funType));
  ...
  if (DFArray::checkType(arrayType)) {
    dat = nl->TwoElemList(listutils::basicSymbol<frel>(), nl->Second(dat));
  }

  ListExpr rfunargs = nl->TwoElemList(nl->First(funargs), dat);
  ListExpr rfun =
      nl->ThreeElemList(nl->First(funquery), rfunargs, nl->Third(funquery));

  ListExpr res = nl->ThreeElemList(
      nl->SymbolAtom(Symbols::APPEND()),
      nl->OneElemList(nl->TextAtom(nl->ToString(rfun))), result);
...
template <class A>
int dloopVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
...
ValueMapping dloopVM[] = {dloopVMT<DArray>, dloopVMT<DFArray>};
...
OperatorSpec dloopSpec(
    " darray(X) x string x  (X->Y) -> darray(Y)", " _ dloop[_,_]",
...
Operator dloopOp("dloop", dloopSpec.getStr(), 2, dloopVM, dloopSelect, dloopTM);

ListExpr DARRAYELEMTM(ListExpr args) {
...
OperatorSpec
    DARRAYELEMSpec("darray(X) -> X ", "DARRAYELEM(_)",
                   "Type Mapping Operator. Extract the type of a darray.",
                   "query da2 dloop[\"da3\", . count");

Operator DARRAYELEMOp("DARRAYELEM", DARRAYELEMSpec.getStr(), 0,
                      Operator::SimpleSelect, DARRAYELEMTM);

ListExpr DARRAYELEM2TM(ListExpr args) {
...
OperatorSpec
    DARRAYELEM2Spec("T x darray(Y) x ... -> Y ", "DARRAYELEM2(_)",
                    "Type Mapping Operator. Extract the type of a darray.",
                    "query da2 da3 dloop[\"da3\", .. count");

Operator DARRAYELEM2Op("DARRAYELEM2", DARRAYELEM2Spec.getStr(), 0,
                       Operator::SimpleSelect, DARRAYELEM2TM);
...
ListExpr dsummarizeTM(ListExpr args) {
  ...
  return nl->TwoElemList(listutils::basicSymbol<Stream<Tuple>>(), streamtype);
}
...
public:
  dsummarizeAttrInfoRunner(DArrayElement &_elem, const string &_objName,
                           ListExpr _resType, const string &_db, int _index,
                           successListener *_listener)
      : elem(_elem), objName(_objName), resType(_resType), db(_db),
        index(_index), listener(_listener), attribute(0) {}

  void start() { runner = boost::thread(&dsummarizeAttrInfoRunner::run, this); }
...
private:
  ...
  void run() {
    ...
    Word result;
    bool ok = ci->retrieve(objName, resType, result, true, showCommands,
                           commandLog, index, false, algInstance->getTimeout());
    if (!ok) {
      listener->jobDone(index, false);
    } else {
...
template <class A> class dsummarizeAttrInfo : public successListener {
  ...
  void run() {
    ...
    while (!stopped && runners.size() < array->getSize()) {
      ...
      if (array->isSlotUsed(index)) {
        ...
        string objname = aname + "_" + stringutils::int2str(runners.size());
        dsummarizeAttrInfoRunner *r = new dsummarizeAttrInfoRunner(
            elem, objname, resType, dbname, runners.size(), this);
        runners.push_back(make_pair(r, false));
        r->start();
      } else {
        runners.push_back(make_pair<dsummarizeAttrInfoRunner *, bool>(0, true));
      }
    }
  }
};
...
template <class A> class RelationFileGetter {
public:
  RelationFileGetter(A *_array, int _index, dsummarizeRelListener *_listener)
  ...
  void operator()() {
    ...
    case SDARRAY:
    case DARRAY:
      retrieveFileFromObject(ci);
      break;
    case DFARRAY:
      retrieveFileFromFile(ci);
      break;
    default:
      assert(false);
    }
...
private:
  ...
  void retrieveFileFromObject(ConnectionInfo *ci) {
    ...
    if (!ci->retrieveRelationFile(objName, fname, showCommands, commandLog,
                                  false, algInstance->getTimeout())) {
      listener->connectionFailed(index);
      return;
    }
  ...
  void retrieveFileFromFile(ConnectionInfo *ci) {
    ...
    string rname =
        array->getName() + "_" + stringutils::int2str(index) + ".bin";

    string path = ci->getSecondoHome(showCommands, commandLog) + "/dfarrays/" +
                  dbname + "/" + array->getName() + "/" + rname;

    string lname = rname;

    if (!ci->retrieveAnyFile(path, lname, showCommands, commandLog, false,
                             algInstance->getTimeout())) {
      listener->connectionFailed(index);
      return;
    }
...
template <class A> class dsummarizeRelInfo : public dsummarizeRelListener {
  ...
  Tuple *next() {
    while (true) {
      ...
      if (filenames[currentIndex].length() > 0) {
        ListExpr tType = SecondoSystem::GetCatalog()->NumericType(resType);
        currentFeeder = new ffeed5Info(filenames[currentIndex], tType);
...
int dsummarizeVMT(Word *args, Word &result, int message, Word &local,
                  Supplier s) {
  ...
  case OPEN:
    if (li)
      delete li;
    local.addr = new T((A *)args[0].addr, nl->Second(qp->GetType(s)));
    return 0;
  case REQUEST:
    result.addr = li ? li->next() : 0;
    return result.addr ? YIELD : CANCEL;
...
OperatorSpec dsummarizeSpec(
    ...
    "_ dsummarize", "Produces a stream of the darray elements.",
...
Operator dsummarizeOp("dsummarize", dsummarizeSpec.getStr(), 9, dsummarizeVM,
                      dsummarizeSelect, dsummarizeTM);
...
ListExpr getValueTM(ListExpr args) {
  ...
  return nl->TwoElemList(listutils::basicSymbol<arrayalgebra::Array>(),
                         nl->Second(a1));
}
...
*/
template <class A> class getValueGetter {
public:
  ...
  void operator()() {
    ...
    ci->retrieve(name, resType, res, true, showCommands, commandLog, index,
                 false, algInstance->getTimeout());
    listener->jobDone(index, res);
    ci->deleteIfAllowed();
  }
...
public:
  ...
  void operator()() {
    ...
    string fname = home + "/dfarrays/" + dbname + "/" + array->getName() + "/" +
                   name + ".bin";
    ci->retrieveRelationInFile(fname, resType, res, showCommands, commandLog,
                               false, algInstance->getTimeout());
    listener->jobDone(index, res);
    ci->deleteIfAllowed();
  }
...
public:
  getValueInfo(AType *_arg, arrayalgebra::Array *_result, ListExpr _resType)
      : arg(_arg), result(_result), resType(_resType), algId(0), typeId(0),
        n(-1), values(0) {
...
template <class AType, class GType>
int getValueVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
...
OperatorSpec getValueSpec("{darray(T),dfarray(T), sdarray(T)} -> array(T)",
                          "_ getValue",
                          "Converts a distributed array into a normal one.",
                          "query da2 getValue");

Operator getValueOp("getValue", getValueSpec.getStr(), 3, getValueVM,
                    getValueSelect, getValueTM);
...
public:
  getValuePInfo(AType *_arg, arrayalgebra::Array *_result, ListExpr _resType,
                Word &_defaultValue)
      : arg(_arg), result(_result), resType(_resType), algId(0), typeId(0),
        n(-1), values(0), defaultValue(_defaultValue) {
...
template <class AType, class GType>
int getValuePVMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
...
int getValuePSelect(ListExpr args) {
  ListExpr a1 = nl->First(args);
  if (PDArray::checkType(a1))
    return 0;
  if (PDFArray::checkType(a1))
    return 1;
  return -1;
}
...
OperatorSpec
    getValuePSpec("{pd[f]array(A) x A  -> array(A)", "_ getValue [default]",
...
Operator getValuePOp("getValueP", getValuePSpec.getStr(), 2, getValuePVM,
                     getValuePSelect, getValuePTM);
...
ListExpr dloop2TM(ListExpr args) {
  ...
  if (!nl->Equal(fa1, a1) || !nl->Equal(fa2, a2)) {
  ...
  ListExpr resType = nl->TwoElemList(listutils::basicSymbol<DArray>(), funRes);
  ...
  fa1 =
      nl->TwoElemList(nl->First(fa1o), nl->Second(nl->First(nl->First(args))));
  ListExpr fa2o = nl->Third(funQuery);
  fa2 =
      nl->TwoElemList(nl->First(fa2o), nl->Second(nl->First(nl->Second(args))));
  funQuery =
      nl->FourElemList(nl->First(funQuery), fa1, fa2, nl->Fourth(funQuery));

  string funstr = nl->ToString(funQuery);

  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()),
                           nl->OneElemList(nl->TextAtom(funstr)), resType);
}

class dloop2Runner {
public:
  dloop2Runner(DArray *_a1, DArray *_a2, int _index, const string &_fun,
               const string &_rname)
  ...
  void operator()() {
    string dbname = SecondoSystem::GetInstance()->GetDatabaseName();
    ConnectionInfo *ci =
        algInstance->getWorkerConnection(a1->getWorkerForSlot(index), dbname);
    if (!ci) {
      sendMessage("could not open connection to " +
                  a1->getWorkerForSlot(index).getHost() + "@" +
                  stringutils::int2str(a1->getWorkerForSlot(index).getPort()));
      return;
    }
    int err;
    string errMsg;
...
int dloop2VM(Word *args, Word &result, int message, Word &local, Supplier s) {
...
Operator dloop2Op("dloop2", dloop2Spec.getStr(), dloop2VM,
                  Operator::SimpleSelect, dloop2TM);
...
*/
template <class T> class fdistribute6Info {
public:
  fdistribute6Info(Word _stream, T *_fname, CcInt *_max, ListExpr _resultType)
      : stream(_stream) {
    counter = 0;
    fileCounter = 0;
    out = 0;
...
template <class T>
int fdistribute6VMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  case OPEN:
    ...
    local.addr = new fdistribute6Info<T>(args[0], (T *)args[1].addr,
                                         (CcInt *)args[2].addr, qp->GetType(s));
    return 0;
  case REQUEST:
    result.addr = li ? li->next() : 0;
    return result.addr ? YIELD : CANCEL;
...
ValueMapping fdistribute6VM[] = {fdistribute6VMT<CcString>,
...
OperatorSpec
    fdistribute6Spec(" stream(tuple> x {string,text} x int -> stream(tuple)",
...
*/
Operator fdistribute6Op("fdistribute6", fdistribute6Spec.getStr(), 2,
                        fdistribute6VM, fdistribute6Select, fdistribute6TM);
...
ListExpr fdistribute7TM(ListExpr args) {
  ...
  if (!nl->Equal(nl->Second(nl->First(args)), nl->Second(nl->Third(args)))) {
...
template <class T> class fdistribute7Info {
public:
  fdistribute7Info(Word _stream, Supplier _fun, CcInt *_max, T *_fn,
    ...
    stream.open();
    relType = _relType;
    if (_createEmpty->IsDefined() && _createEmpty->GetValue() && (max > 0)) {
      for (int i = 0; i < max; i++) {
        getStream(i);
      }
    }
...
template <class T>
int fdistribute7VMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  case OPEN: {
    ...
    local.addr = new fdistribute7Info<T>(
        args[0], qp->GetSon(s, 2), (CcInt *)args[3].addr, (T *)args[1].addr,
        (CcBool *)args[4].addr,
        nl->TwoElemList(listutils::basicSymbol<Relation>(),
                        nl->Second(qp->GetType(s))));
  }
...
ValueMapping fdistribute7VM[] = {fdistribute7VMT<CcString>,
...
Operator fdistribute7Op("fdistribute7", fdistribute7Spec.getStr(), 2,
                        fdistribute7VM, fdistribute7Select, fdistribute7TM);
...
template <bool isKill> class Object_Del {
...
public:
  ...
  Object_Del(Object_Del &src)
      : array(src.array), dfarray(src.dfarray), sdarray(src.sdarray),
...
private:
  ...
  template <class A> void deleteArray(A *array) {
    string dbname = SecondoSystem::GetInstance()->GetDatabaseName();
    ConnectionInfo *ci = algInstance->getWorkerConnection(
        array->getWorkerForSlot(index), dbname);
    if (!ci) {
      sendMessage(
          ...
          stringutils::int2str(array->getWorkerForSlot(index).getPort()));
      return;
    }
    string objName = array->getObjectNameForSlot(index);
    int err;
    ...
    ci->simpleCommand(cmd + objName, err, errMsg, resstr, false, runtime,
                      showCommands, commandLog, false,
                      algInstance->getTimeout());
    if (err == 0) {
      del = 1;
  ...
  void deleteFArray(DFArray *farray) {
    ...
    if (!ci) {
      sendMessage(
          ...
          stringutils::int2str(farray->getWorkerForSlot(index).getPort()));
      return;
    }

    string fileName = ci->getSecondoHome(showCommands, commandLog) +
                      "/dfarrays/" + dbname + "/" + farray->getName() + "/" +
    ...
    string resstr;
    double runtime;
    ci->simpleCommand("query removeFile('" + fileName + "')", err, errMsg,
                      resstr, false, runtime, showCommands, commandLog, false,
                      algInstance->getTimeout());
    if (err == 0) {
      del = 1;
    }
...
template <class A, bool isKill> void deleteRemoteObjects(A *array) {
  if (keepRemoteObjects)
    return;
  if (!array->IsDefined())
    return;
  if (array->getKeepRemoteObjects())
    return;
...
int deleteRemoteObjectsVMT(Word *args, Word &result, int message, Word &local,
                           Supplier s) {
...
class MatrixKiller {
public:
  MatrixKiller(ConnectionInfo *_ci, const string &_dbname, const string &_mname)
...
private:
  ...
  void run() {
    string dir = ci->getSecondoHome(showCommands, commandLog) + "/dfarrays/" +
                 dbname + "/" + mname;
...
template <> void deleteRemoteObjects<DFMatrix, false>(DFMatrix *matrix) {
  ...
  if (!matrix->IsDefined())
    return;
  if (matrix->getKeepRemoteObjects())
    return;
...
ValueMapping deleteRemoteObjectsVM[] = {deleteRemoteObjectsVMT<DArray, false>,
...
Operator deleteRemoteObjectsOp("deleteRemoteObjects",
                               deleteRemoteObjectsSpec.getStr(), 4,
                               deleteRemoteObjectsVM, deleteRemoteObjectsSelect,
...
ValueMapping killRemoteObjectsVM[] = {
    deleteRemoteObjectsVMT<DArray, true>, deleteRemoteObjectsVMT<DFArray, true>,
    deleteRemoteObjectsVM_Matrix, deleteRemoteObjectsVMT<SDArray, true>};

Operator killRemoteObjectsOp("killRemoteObjects",
                             killRemoteObjectsSpec.getStr(), 4,
                             killRemoteObjectsVM, deleteRemoteObjectsSelect,
                             deleteRemoteObjectsTM);
...
private:
  ...
  void runDArray() {
    ...
    if (!ci) {
      sendMessage(
          ...
          stringutils::int2str(darray->getWorkerForSlot(index).getPort()));
      return;
    }
    ...
    ci->simpleCommand(cmd, err, errMsg, resstr, false, runtime, showCommands,
                      commandLog, false, algInstance->getTimeout());
    if (err != 0) {
      writeLog(ci, cmd, errMsg);
    }
  ...
  void runFArray() {
    ...
    string objName =
        farray->getName() + "_" + stringutils::int2str(index) + ".bin";
    string newName = name + "_" + stringutils::int2str(index) + ".bin";
    string spath = ci->getSecondoHome(showCommands, commandLog) + "/dfarrays/" +
                   dbname + "/" + farray->getName() + "/";
    string tpath = ci->getSecondoHome(showCommands, commandLog) + "/dfarrays/" +
                   dbname + "/" + name + "/";
    int err;
    string errMsg;
    string resstr;
    ...
    cmd =
        "query copyFile('" + spath + objName + "', '" + tpath + newName + "')";
    ci->simpleCommand(cmd, err, errMsg, resstr, false, runtime, showCommands,
                      commandLog, false, algInstance->getTimeout());
    if (err != 0) {
...
*/
template <class A>
int cloneVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
...
*/
OperatorSpec cloneSpec(" darray x string -> darray", " _ clone[_]",
                       "Creates a copy of a darray with a new name. ",
                       "let da9 = da8 clone[\"da9\"] ");

ValueMapping cloneVM[] = {cloneVMT<DArray>, cloneVMT<DFArray>};
...
*/
Operator cloneOp("clone", cloneSpec.getStr(), 2, cloneVM, cloneSelect, cloneTM);
    ...
    if (!DArray::checkType(nl->Third(args)) &&
      ...
      if (!isWorkerRelDesc(nl->Third(args), positions, types, errmsg)) {
        ...
        ListExpr appendList = nl->FiveElemList(
            nl->First(positions), nl->Second(positions), nl->Third(positions),
            nl->BoolAtom(CcString::checkType(nl->First(types))),
            nl->BoolAtom(CcString::checkType(nl->Third(types))));
        return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), appendList,
                                 listutils::basicSymbol<FText>());
      }
    }
...
public:
  shareRunner(ConnectionInfo *_ci, const string &_objName,
              ...
              const bool _attribute, int _id, bool _allowOverwrite,
              successListener *_listener)
      : ci(_ci), objName(_objName), fileName(_fileName), relation(_relation),
        attribute(_attribute), id(_id), allowOverwrite(_allowOverwrite),
        listener(_listener) {
    ci->copy();
  }

  shareRunner(const shareRunner &s)
      : ci(s.ci->copy()), objName(s.objName), fileName(s.fileName),
        relation(s.relation), attribute(s.attribute), id(s.id),
        allowOverwrite(s.allowOverwrite), listener(s.listener) {}

  ~shareRunner() { ci->deleteIfAllowed(); }

  void operator()() {
    if (relation) {
      bool r = ci->createOrUpdateRelationFromBinFile(
          objName, fileName, showCommands, commandLog, allowOverwrite, false,
          algInstance->getTimeout());
      listener->jobDone(id, r);
    } else if (attribute) {
      bool r = ci->createOrUpdateAttributeFromBinFile(
          objName, fileName, showCommands, commandLog, allowOverwrite, false,
          algInstance->getTimeout());
      listener->jobDone(id, r);
    } else {
      ...
      double runtime;
      if (allowOverwrite) {
        ci->simpleCommand(cmd, err, res, false, runtime, showCommands,
                          commandLog, false, algInstance->getTimeout());
      }
      // ignore error because object normally does not exist
      cmd = "restore " + objName + " from '" + fileName + "'";
      ci->simpleCommand(cmd, err, res, false, runtime, showCommands, commandLog,
                        false, algInstance->getTimeout());
      if (err != 0) {
        writeLog(ci, cmd, res);
      }
...
template <class AType> class shareInfo : public successListener {

public:
  shareInfo(const string &_name, const bool _allowOverwrite, AType *_array,
            FText *_result)
      : name(_name), allowOverwrite(_allowOverwrite), array(_array),
  ...
  shareInfo(const string &_name, Word _value, ListExpr _type,
            const bool _allowOverwrite, AType *_array, FText *_result)
      : name(_name), allowOverwrite(_allowOverwrite), array(_array),
        result(_result), fileCreated(false) {
    failed = 0;
    success = 0;
  ...
  void share() {
    if (fromCatalog) {
      ...
      bool defined;
      bool hasTypeName;
      if (!ctlg->GetObjectExpr(name, tn, typeList, value, defined,
                               hasTypeName)) {
...
private:
  ...
  void shareArray() {
    ...
    for (size_t i = 0; i < array->numOfWorkers(); i++) {
      ConnectionInfo *ci =
          algInstance->getWorkerConnection(array->getWorker(i), dbname);
      if (ci) {
        share(ci);
      } else {
        cout << "cannot create connection to " << array->getWorker(i) << endl;
      }
      ci->deleteIfAllowed();
    }
  }
  ...
  void createFile(ConnectionInfo *ci) {
    ...
    if (!fileCreated) {
      ...
      filename = name + "_" + stringutils::int2str(WinUnix::getpid()) + ".bin";
      if (isRelation) {
...
int shareVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  if (sons == 2 + q) {
    ...
    if (hostStr && confStr) {
      a = DArrayBase::createFromRel<CcString, CcString, A>(
          (Relation *)args[2 + q].addr, 400, "tmp", hostPos, portPos, confPos);
...
OperatorSpec
    shareSpec("string x bool [x d[f]array] -> text",
...
OperatorSpec
    share2Spec("string x ANY x bool [x d[f]array] -> text",
...
ValueMapping shareVM[] = {shareVMT<DArray, true>, shareVMT<DFArray, true>};

ValueMapping share2VM[] = {shareVMT<DArray, false>, shareVMT<DFArray, false>};

/*
12.5 Operator instance

*/
Operator shareOp("share", shareSpec.getStr(), 2, shareVM, shareSelect, shareTM);

/*
12.5 Operator instance

*/
Operator share2Op("share2", share2Spec.getStr(), 2, share2VM, share2Select,
                  share2TM);
...
*/
template <class A>
int cleanUpVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *res = (CcBool *)result.addr;
  res->Set(true, true);
      ...
      if (used.find(p) == used.end()) {
        ...
        if (ci) {
          ...
          boost::thread *r = new boost::thread(
              boost::bind(&ConnectionInfo::cleanUp, ci, showCommands,
                          boost::ref(commandLog), 0));
          cons.push_back(ci);
          runners.push_back(r);
        }
      }
...
OperatorSpec cleanUpSpec(
    "-> bool , {d[f]array, sdarray}  -> bool", "cleanUp(_)",
...
ValueMapping cleanUpVM[] = {cleanUpVMT<DArray>, cleanUpVMT<DFArray>,
                            cleanUpVMT<SDArray>};
...
Operator cleanUpOp("cleanUp", cleanUpSpec.getStr(), 3, cleanUpVM, cleanUpSelect,
                   cleanUpTM);
...
Operator dfdistribute2Op("dfdistribute2", dfdistribute2Spec.getStr(), 4,
                         fdistribute2VM, distribute2Select,
...
Operator dfdistribute3Op("dfdistribute3", dfdistribute3Spec.getStr(), 4,
                         dfdistribute3VM, distribute3Select,
...
Operator dfdistribute4Op("dfdistribute4", dfdistribute4Spec.getStr(), 4,
                         dfdistribute4VM, distribute4Select,
...
private:
  ...
  void convert(DArray *arg, DFArray *res, ConnectionInfo *ci) {
    ...
    string cmd = "query " + oname + " feed fconsume5['" + filename + "'] count";
    int err;
    string errMsg;
    string r;
    double runtime;
    ci->simpleCommand(cmd, err, errMsg, r, false, runtime, showCommands,
                      commandLog, false, algInstance->getTimeout());
    if (err != 0) {
...
int convertdarrayVMT(Word *args, Word &result, int message, Word &local,
                     Supplier s) {
  result = qp->ResultStorage(s);
  A *arg = (A *)args[0].addr;
  R *res = (R *)result.addr;
...
ValueMapping convertdarrayVM[] = {convertdarrayVMT<DArray, DFArray>,
                                  convertdarrayVMT<DFArray, DArray>};

OperatorSpec
    convertdarraySpec("darray -> dfarray, dfarray -> darray ",
                      "convertdarray(_)",
...
Operator convertdarrayOp("convertdarray", convertdarraySpec.getStr(), 2,
                         convertdarrayVM, convertdarraySelect, convertdarrayTM);
...
ListExpr gettuplesTM(ListExpr args) {
  ...
  ListExpr resType = nl->TwoElemList(
      listutils::basicSymbol<Stream<Tuple>>(),
      nl->TwoElemList(listutils::basicSymbol<Tuple>(), conAttrList));
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()),
                           nl->OneElemList(nl->IntAtom(pos - 1)), resType);
}
...
template <class T>
int gettuplesVMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
...
ValueMapping gettuplesVM[] = {gettuplesVMT<CcString>, gettuplesVMT<FText>};
...
OperatorSpec
    gettuplesSpec("stream(tuple(X)) x {string,text} -> stream(tuple(Y))",
                  "_ _ getTuples",
                  "Retrieves tuples from a binary relation file.",
                  "query tree exactmatch[3] 'file' gettuples consume");

Operator gettuplesOp("gettuples", gettuplesSpec.getStr(), 2, gettuplesVM,
                     gettuplesSelect, gettuplesTM);
...
template <bool partial> ListExpr dmapTM(ListExpr args) {
  ...
  if (listutils::isMap<1>(arg3Type)) {
    arg3Type =
        nl->FourElemList(nl->First(arg3Type), nl->Second(arg3Type),
                         listutils::basicSymbol<CcInt>(), nl->Third(arg3Type));
    addedFunArg = true;
  }

  if ((!DFArray::checkType(arg1Type) && !DArray::checkType(arg1Type) &&
       !SDArray::checkType(arg1Type)) ||
      !CcString::checkType(arg2Type) || !listutils::isMap<2>(arg3Type)) {
    return listutils::typeError(err);
  }

  ListExpr frelt = nl->TheEmptyList();
  if (Relation::checkType(nl->Second(arg1Type))) {
    frelt = nl->TwoElemList(listutils::basicSymbol<frel>(),
  ...
  ListExpr expFunArg1 =
      DArray::checkType(arg1Type) || SDArray::checkType(arg1Type)
          ? nl->Second(arg1Type)
          : frelt;
  ...
  ListExpr rfun = nl->FourElemList(nl->First(funq), // symbol fun
                                   rfunarg1, rfunarg2, nl->Fourth(funq));
  ...
  if (isStream) {
    funRes =
        nl->TwoElemList(listutils::basicSymbol<Relation>(), nl->Second(funRes));
  }
  ...
  ListExpr resType =
      nl->TwoElemList(isStream ? listutils::basicSymbol<DFArray>()
                               : (SDArray::checkType(arg1Type)
                                      ? nl->First(arg1Type)
                                      : listutils::basicSymbol<DArray>()),
                      funRes);
  if (partial) {
    resType = nl->TwoElemList(isStream ? listutils::basicSymbol<PDFArray>()
...
public:
  ...
  */
  Mapper(A *_array,       // argument array
         ...
         CommandLogger *_log,
         shared_ptr<set<int>> _parts)
      : // pointer to a logger or null
        array(_array), aType(_aType), ccname(_name), funText(_funText),
        isRel(_isRel), isStream(_isStream), log(_log), parts(_parts) {
...
private:
  ...
  template <class type, class rtype> void startXArray(type *&resArray) {
    if (!array->IsDefined()) {
      if (!log) {
  ...
  ConnectionInfo *changeWorker(size_t slotNo, set<size_t> &usedWorkers,
    ...
    int retries = 0;
    ConnectionInfo *ci =
        changeWorker1(array, slotNo, usedWorkers, retries, oldCi);
    return ci;
  }

  void changeConnection(ConnectionInfo *&ci, const int nr, const string &cmd,
                        const int err, const string &errMsg, bool &reconnect,
                        int &numtries, set<size_t> &usedWorkers) {
    showError(ci, cmd, err, errMsg);
    writeLog(ci, cmd, errMsg);
    if (reconnect && reconnectGlobal) {
      if (!ci->reconnect(showCommands, commandLog, algInstance->getTimeout(),
                         algInstance->getHeartbeat())) {
        reconnect = false;
  ...
  */
  ...
  public:
    fRun(ConnectionInfo *_ci,   // connection to use
         ...
         int _nr,               // slot number
         Mapper *_mapper)
        : // the calling mapper instance
          ci(_ci), dbname(_dbname), nr(_nr), mapper(_mapper) {
      ci->copy();
    }

    ~fRun() { ci->deleteIfAllowed(); }
    ...
    void run() {
...
#ifdef DPROGRESS
      ci->getInterface()->addMessageHandler(
          algInstance->progressObserver->getProgressListener(
      ...
      do {
        ...
        string r;
        double runtime;
        ci->simpleCommand(cd, err, errMsg, r, false, runtime, showCommands,
                          commandLog, false, algInstance->getTimeout());
        if (err) {
          ...
          string fname2 = targetDir + mapper->name + "_" +
                          stringutils::int2str(nr) + ".bin";
          ...
          if (mapper->isRel) {
            cmd = "(query (count (fconsume5 (feed " + cmd + " )'" + fname2 +
                  "' " + aa + ")))";
          } else {
            cmd = "(query (count (fconsume5 " + cmd + " '" + fname2 + "'" + aa +
                  " )))";
          }
...
#ifdef DPROGRESS
          ci->getInterface()->removeMessageHandler(
              algInstance->progressObserver->getProgressListener(
                  mapper->array->getObjectNameForSlot(nr),
                  mapper->array->getWorkerForSlot(nr)));
          algInstance->progressObserver->commitWorkingOnSlotFinished(
              mapper->array->getObjectNameForSlot(nr),
              mapper->array->getWorkerForSlot(nr));
#endif
  ...
  class dRun {
  public:
    dRun(ConnectionInfo *_ci, const string &_dbname, int _nr, Mapper *_mapper)
        : ci(_ci), dbname(_dbname), nr(_nr), mapper(_mapper) {
      ci->copy();
    }

    ~dRun() { ci->deleteIfAllowed(); }
...
template <class A>
int dmapVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  Mapper<A> mapper(array, qp->GetType(qp->GetSon(s, 0)), name, funText, isRel,
                   isStream, result.addr, 0, nullptr);
  mapper.start();
...
template <class A>
int pdmapVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
...
void dmapcommandsVMT(Word *args, Supplier s, CommandLogger *log) {
  ...
  bool isStream = ((CcBool *)args[5].addr)->GetValue();
  Mapper<A> mapper(array, qp->GetType(qp->GetSon(s, 0)), name, funText, isRel,
                   isStream, 0, log, nullptr);
  mapper.start();
}

// explicit instantiation
template void dmapcommandsVMT<DArray>(Word *, Supplier, CommandLogger *);

template void dmapcommandsVMT<DFArray>(Word *, Supplier, CommandLogger *);

template void dmapcommandsVMT<SDArray>(Word *, Supplier, CommandLogger *);

ValueMapping dmapVM[] = {dmapVMT<DArray>, dmapVMT<DFArray>, dmapVMT<SDArray>};

int dmapSelect(ListExpr args) {
  ListExpr arg1 = nl->First(args);
  if (DArray::checkType(arg1))
    return 0;
  if (DFArray::checkType(arg1))
    return 1;
  if (SDArray::checkType(arg1))
    return 2;
  return -1;
}

OperatorSpec
    dmapSpec("d[f]array x string x fun -> d[f]array", "_ dmap[_,_]",
...
Operator dmapOp("dmap", dmapSpec.getStr(), 3, dmapVM, dmapSelect,
                dmapTM<false>);

ValueMapping pdmapVM[] = {pdmapVMT<DArray>, pdmapVMT<DFArray>,
                          pdmapVMT<SDArray>};

int pdmapSelect(ListExpr args) {
  ListExpr arg2 = nl->Second(args);
  if (DArray::checkType(arg2))
    return 0;
  if (DFArray::checkType(arg2))
    return 1;
  if (SDArray::checkType(arg2))
    return 2;
  return -1;
}

OperatorSpec
    pdmapSpec("stream<int>  x d[f]array x string x fun -> d[f]array",
              "_ _ pdmap[_,_]",
              "Performs a function on a distributed [file] array. "
              "If the string argument is empty or undefined, a name for "
              "the result is chosen automatically. If not, the string "
              ...
              "query intstream(1,10) filter[ (. mod 2) = 0 ]dfa8 dmap[\"\", . "
              "head[23]] ");

Operator pdmapOp("pdmap", pdmapSpec.getStr(), 3, pdmapVM, pdmapSelect,
                 dmapTM<true>);
...
OperatorSpec DFARRAYSTREAMSPEC("DFARRAYSTREAMSPEC",
                               "dfarray(rel(X)) x ... -> stream(X) ",
                               "Type map operator.",
                               "query da8 dmap[\"\", . feed head[23]]");

Operator DFARRAYSTREAMOP("DFARRAYSTREAM", DFARRAYSTREAMSPEC.getStr(), 0,
                         Operator::SimpleSelect, DFARRAYSTREAMTM);
...
private:
  ...
  void run() {
    ...
    string cmd =
        "query staticFileTransferator(" + stringutils::int2str(port) + ",10)";
    int err;
    string errMsg;
    string res;
    double runtime;
...
template <class AT> bool startFileTransferators(AT *array, int port) {
  assert(port > 0);
  set<string> usedIPs;
...
ListExpr dmapXTM(ListExpr args) {
  ...
  string err = "d[f]array(X_i) ^" + stringutils::int2str(x) +
               " x string x "
               "fun : (X_0 x X_1 x ...X_t" +
               stringutils::int2str(x) + " ,int-> Z) x int expected";
  ...
  for (int i = 0; i < x; i++) {
    if (!DArray::checkType(arrays[i]) && !DFArray::checkType(arrays[i])) {
  ...
  if (listutils::isStream(funres)) {
    ...
    resType = nl->TwoElemList(listutils::basicSymbol<DArray>(), funres);
  }
  ...
  for (int i = 0; i < x; i++) { // copy arguments withount the slot
    last = nl->Append(last,
                      nl->TwoElemList(nl->First(nl->First(funQ)), // argname
  ...
  if (!funargAdded) { // integer type was already there
    ...
    last = nl->Append(
        last,
        nl->TwoElemList(nl->SymbolAtom(firstArgName + "_add42"), // argname
                        efunargs[x]                              // real type
                        ));
  }
  ...
  ListExpr finalRes =
      nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()),
                        nl->TwoElemList(nl->BoolAtom(streamRes),
                                        nl->TextAtom(nl->ToString(rfun))),
                        resType);
  return finalRes;
}

template <int x> ListExpr dmapXTMT(ListExpr args) {
...
template <int x> ListExpr pdmapXTMT(ListExpr args) {
  if (!nl->HasMinLength(args, 2)) {
  ...
  return nl->ThreeElemList(nl->First(res), nl->Second(res), resType);
}
...
*/
bool transferRequired(DArrayElement &target, DArrayElement &source, arrayType T,
                      const string &dbname) {
...
bool retrieveData(DArrayBase *target, DArrayBase *source, int slot,
                  ...
                  vector<string> &funargs, ListExpr argType,
                  bool createObjectIfNecessary = true) {
  ...
  if (transferRequired(elem0, elemi, ai->getType(), dbname)) {
    ...
    if (ai->getType() != DFARRAY) {
      ...
      // at the source node
      fileNameOnI1 = "darrays/TMP_" + dbname + "_" + ai->getName() + "_" +
                     stringutils::int2str(slot) + "_" +
                     stringutils::int2str(WinUnix::getpid()) + "_" +
                     stringutils::int2str(ci->serverPid()) + "_" + ".bobj";
      string odir = ci->getSecondoHome(showCommands, commandLog) +
                    "/dfarrays/" + dbname + "/";
      fileNameOnI = odir + fileNameOnI1;
      string oname = ai->getObjectNameForSlot(slot);

      string cmd =
          "query " + oname + " saveObjectToFile['" + fileNameOnI + "']";
      ...
      fileNameOnI = ai->getFilePath(
          ci->getSecondoHome(showCommands, commandLog), dbname, slot);
    ...
    if (!error) {
      ...
      if (c0->getHost() != ci->getHost()) { // use TCP transfer or
                                            // disc utils
        cmd = "query getFileTCP('" + fileNameOnI + "', '" + ci->getHost() +
              "', " + stringutils::int2str(port) + ", TRUE, '" + fileNameOn0 +
              "')";

      } else {
        string op = formerObject ? "moveFile" : "copyFile";
        moved = formerObject;
        cmd = "query " + op + "('" + fileNameOnI + "','" + fileNameOn0 +
              "', TRUE)";
      }
    ...
    if (formerObject && createObjectIfNecessary) {
      ...
      string cmd =
          "let " + oname + " =  '" + fileNameOn0 + "' getObjectFromFile " + end;
      c0->simpleCommand(cmd, errorCode, errMsg, resList, false, rt,
                        showCommands, commandLog, false,
                        algInstance->getTimeout());
      if (errorCode) {
      ...
      string type =
          "(frel " + nl->ToString(nl->Second(nl->Second(argType))) + ")";
...
public:
  ...
  */
  dmapXInfo(vector<DArrayBase *> &_arguments, vector<bool> &_isRelation,
            ...
            const string &_funText, const bool _isStreamRes, const int _port,
            DArrayBase *_res, set<int> *_parts)
      : arguments(_arguments), isRelation(_isRelation), argTypes(_argTypes),
        name(_name), funText(_funText), isStreamRes(_isStreamRes), port(_port),
        res(_res), parts(_parts) {}
...
private:
  ...
  */
  ...
  This functions creates a Runner object for each slot. These objects will
  create the result for their slot.
  ...
  bool buildRemoteObjects() {
    ...
    if (res->getType() == DFARRAY) {
      ...
      for (size_t i = 0; i < minSlots; i++) {
        ...
        ConnectionInfo *ci = algInstance->getWorkerConnection(elem, dbname);
        string dirname =
            res->getPath(ci->getSecondoHome(showCommands, commandLog), dbname);
        string q = "query createDirectory('" + dirname + "', TRUE)";
        int err;
        string errMsg;
        double rt;
        string result;
        ci->simpleCommand(q, err, errMsg, result, false, rt, showCommands,
                          commandLog, false, algInstance->getTimeout());
        if (err) {
          cerr << __FILE__ << "@" << __LINE__ << endl;
  ...
  */
  vector<DArrayElement> reduceFileTransferServers(vector<DArrayElement> src) {
  ...
  */
  bool startFileTransferatorOnServer(DArrayElement &elem) {
    ...
    double rt;
    ListExpr result;
    ci->simpleCommand(cmd, err, errMsg, result, false, rt, showCommands,
                      commandLog, false, algInstance->getTimeout());
    if (err) {
      cerr << __FILE__ << "@" << __LINE__ << endl;
      showError(ci, cmd, err, errMsg);
  ...
  */
  ...
  private:
    ...
    In a first step, all data are collected from servers. If the data are
    already on the correct server, no transfer is started. In the same time, the
    names of the sources and the correspnding function arguments are created.

    */
    void run() {
      for (size_t i = 0; i < info->arguments.size(); i++) {
        distributed2::retrieveData(info->arguments[0], info->arguments[i], slot,
                                   info->dbname, info->port,
                                   info->isRelation[i], sourceNames, funargs,
                                   info->argTypes[i]);
      }

      // get the info for the worker belonging to the result.
      ConnectionInfo *c0 = algInstance->getWorkerConnection(
          info->arguments[0]->getWorkerForSlot(slot), info->dbname);

#ifdef DPROGRESS
      c0->getInterface()->addMessageHandler(
          algInstance->progressObserver->getProgressListener(
...
#ifdef DPROGRESS
      c0->getInterface()->removeMessageHandler(
          algInstance->progressObserver->getProgressListener(
              info->arguments[0]->getObjectNameForSlot(slot),
              info->arguments[0]->getWorkerForSlot(slot)));
      algInstance->progressObserver->commitWorkingOnSlotFinished(
    ...
    */
      ...
      */
      ...
      if (resType == DARRAY) {
        ...
        string resFileName = info->res->getFilePath(
            ci->getSecondoHome(showCommands, commandLog), info->dbname, slot);
    ...
    Removes temporarly created data, i.e. data that was created by file
    transfers from other than the result worker.
    ...
    void removeTempData(size_t arg, ConnectionInfo *ci) {
      ...
      if (err) {
        cerr << "could not remove temporal object of argument " << arg
             << " for slot " << slot << endl;
        cerr << "because the following command failed: " << cmd << "with code "
             << err << endl
             << errMsg << endl
             << endl;
        writeLog(ci, cmd, errMsg);
...
int dmapXVM(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  for (int i = 0; i < x; i++) {
    ...
    arrays.push_back(di);
    isRelation.push_back(Relation::checkType(
        nl->Second(qp->GetType(qp->GetSon(s, i + addArg)))));
  ...
  dmapXInfo info(arrays, isRelation, argTypes, name, funtext, streamRes, port,
                 (DArrayBase *)result.addr, parts);
  info.start();
  if (parts) {
    delete parts;
  }
...
*/
OperatorSpec dmapXSpec("d[f]array^x x string x fun -> d[f]array",
                       "dmapX[_,_,_]",
                       "Joins the slots of x  distributed arrays",
                       "query mapX(df1i, df2, \"df3\" . .. product]");

Operator dmap2Op("dmap2", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<2>);

Operator dmap3Op("dmap3", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<3>);

Operator dmap4Op("dmap4", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<4>);

Operator dmap5Op("dmap5", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<5>);

Operator dmap6Op("dmap6", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<6>);

Operator dmap7Op("dmap7", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<7>);

Operator dmap8Op("dmap8", dmapXSpec.getStr(), dmapXVM<false>,
                 Operator::SimpleSelect, dmapXTMT<8>);
...
*/
OperatorSpec
    pdmapXSpec("stream(int) x d[f]array^x x string x fun x int -> pd[f]array",
               "_ _ ... dmapX[_,_,_]",
               "Joins the specified slots of x  distributed arrays",
               "query intstream(0,10) filter[ (. mod 2) = 0] da 1 da2 "
               "pdmap2(\"df3\" . .. product, 1238]");

Operator pdmap2Op("pdmap2", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<2>);

Operator pdmap3Op("pdmap3", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<3>);

Operator pdmap4Op("pdmap4", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<4>);

Operator pdmap5Op("pdmap5", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<5>);

Operator pdmap6Op("pdmap6", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<6>);

Operator pdmap7Op("pdmap7", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<7>);

Operator pdmap8Op("pdmap8", pdmapXSpec.getStr(), dmapXVM<true>,
                  Operator::SimpleSelect, pdmapXTMT<8>);
...
*/
template <int pos> ListExpr dproductargT(ListExpr args) {
  assert((pos == 1) || (pos == 2));
  ...
  return nl->TwoElemList(listutils::basicSymbol<fsrel>(), nl->Second(subtype));
}

OperatorSpec dproductarg1Spec(
    " darray(X) x ... -> X | dfarray(rel(x)) -> frel(X) ", " op(_,_,_)",
    "A type map operator",
    "query dplz1 dplz2 dproduct[\"\", .. feed . feed concat , 1234]");

OperatorSpec dproductarg2Spec(
    "  ? x d[f]array(rel(X)) x ... -> fsrel(X) ", " op(_,_,_)",
    "A type map operator",
    "query dplz1 dplz2 dproduct[\"\", .. feed . feed concat , 1234]");

Operator dproductarg1Op("DPRODUCTARG1", dproductarg1Spec.getStr(), 0,
                        Operator::SimpleSelect, dproductargT<1>);

Operator dproductarg2Op("DPRODUCTARG2", dproductarg2Spec.getStr(), 0,
                        Operator::SimpleSelect, dproductargT<2>);
...
ListExpr dproductTM(ListExpr args) {
  ...
  if (!nl->Equal(s1, nl->Second(fun)) || !nl->Equal(s2, nl->Third(fun))) {
  ...
  ListExpr rfunQuery = nl->FourElemList(
      nl->First(funQuery), nl->TwoElemList(nl->First(nl->Second(funQuery)), s1),
      nl->TwoElemList(nl->First(nl->Third(funQuery)), s2),
      nl->Fourth(funQuery));

  ListExpr resType = isStream
                         ? nl->TwoElemList(listutils::basicSymbol<Relation>(),
                                           nl->Second(funres))
                         : funres;
  ...
  ListExpr appendList = nl->TwoElemList(nl->TextAtom(nl->ToString(rfunQuery)),
                                        nl->BoolAtom(isStream));
...
void performCommand(ConnectionInfo *ci, const string &cmd) {
  ...
  double rt;
  string res;
  ci->simpleCommand(cmd, errorCode, errorMsg, res, false, rt, showCommands,
                    commandLog, false, algInstance->getTimeout());
  if (errorCode) {
...
public:
  dproductInfo(DArrayBase *_arg0, DArrayBase *_arg1, DArrayBase *_result,
               ...
               int _transferPort)
      : arg0(_arg0), arg1(_arg1), result(_result), funText(_funText),
        a0Type(_a0Type), a1Type(_a1Type), port(_transferPort) {
    dbname = SecondoSystem::GetInstance()->GetDatabaseName();
  }
...
private:
  ...
  string getHome(string IP) { return Ip2Home[IP]; }

  class copyHelper {
  public:
    copyHelper() {}

    typedef pair<vector<ConnectionInfo *>, vector<pair<bool, string>>> ipinfo;
    ...
    void add(ConnectionInfo *info, DArrayBase *source, string &dbname) {
      ...
      if (it != c.end()) {
        ...
        for (size_t i = 0; i < source->getSize(); i++) {
          ...
          string sourceHome =
              sourceInfo->getSecondoHome(showCommands, commandLog);
        ...
        pair<vector<ConnectionInfo *>, vector<pair<bool, string>>> p(infos,
                                                                     pos);
        c[ip] = p;
      }
    }
    ...
    inline iter scan() { return c.begin(); }
    inline iter end() { return c.end(); }
  ...
  private:
    ...
    void print(iter &it, ostream &out) {
      ...
      for (size_t i = 0; i < homes.size(); i++) {
        out << "  " << i << " : " << homes[i].first << ", " << homes[i].second
            << endl;
      }
    }
  };
  ...
  class copyRunner {
  public:
    copyRunner(copyHelper::ipinfo &inf, dproductInfo *_pi) : pi(_pi) {
      ...
      for (size_t i = 0; i < slots.size(); i++) {
        if (slots[i].first) { // slot missing
          ...
          if (host != elem.getHost()) { // round robin, real copying
            responsible[currentWorker].second.push_back(make_pair(true, i));
  ...
  private:
    ...
    private:
      ...
      void run() {
        pair<ConnectionInfo *, vector<pair<bool, size_t>>> &p =
            parent->responsible[index];
      ...
      void transferFile(ConnectionInfo *target, int slot) {
        ...
        ConnectionInfo *source =
            algInstance->getWorkerConnection(elem, parent->pi->dbname);
        string sourceName =
            a->getFilePath(source->getSecondoHome(showCommands, commandLog),
        ...
        string cmd = "query getFileTCP('" + sourceName + "', '" +
                     source->getHost() + "', " +
                     stringutils::int2str(parent->pi->port) + ", FALSE, '" +
                     targetName + "')";
  ...
  class fileCreator {
  public:
    fileCreator(DArrayBase *_a, ConnectionInfo *_ci, vector<int> *_s,
                string &_db)
  ...
  private:
    ...
    void createFile(int slot) {
      ...
      double rt;
      string res;
      ci->simpleCommand(cmd, errCode, errMsg, res, false, rt, showCommands,
                        commandLog, false, algInstance->getTimeout());
      if (errCode) {
  ...
  void createFilesFromDArray(DArrayBase *array) {
    ...
    for (it = resp.begin(); it != resp.end(); it++) {
      runners.push_back(
          new fileCreator(arg1, it->first, &(it->second), dbname));
  ...
  copyHelper *copySlots() {
    ...
    for (unsigned int i = 0; i < arg0->numOfWorkers(); i++) {
      ...
      ConnectionInfo *target = algInstance->getWorkerConnection(elem, dbname);
    ...
    for (unsigned int i = 0; i < arg1->numOfWorkers(); i++) {
      ...
      ConnectionInfo *source = algInstance->getWorkerConnection(elem, dbname);
  ...
  private:
    ...
    void run() {
      ...
      string funcall = "( " + pi->funText + " " + funarg1 + " " + funarg2 + ")";
      ...
      if (pi->result->getType() == DARRAY) {
        ...
        cmd = "(query (count (fconsume5 " + funcall + "'" +
              pi->result->getFilePath(
                  c0->getSecondoHome(showCommands, commandLog), pi->dbname,
                  slot) +
              "' )))";
      }
      int ec;
    ...
    string getSlotFile(DArrayElement target, size_t slot, ConnectionInfo *c0,
                       copyHelper *sh) {
...
int dproductVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
...
Operator dproductOp("dproduct", dproductSpec.getStr(), 8, dproductVM,
                    dproductSelect, dproductTM);
...
template <int pos> ListExpr arraytypeStreamTMT(ListExpr args) {
  if (!nl->HasMinLength(args, pos)) {
    return listutils::typeError("too less arguments");
  }
  for (int i = 1; i < pos; i++) {
...
OperatorSpec arraytypeStream1Spec(
    "d[f]array(rel X) x ... -> stream(X)", "arraytypeStream1(_)",
    "Type Mapping operator",
    "query dfa1 dfa2 dproductspec[\"\", . . product , 1238");

OperatorSpec arraytypeStream2Spec(
    " . x d[f]array(rel X) x ... -> stream(X)", "arraytypeStream2(_)",
    "Type Mapping operator",
    "query dfa1 dfa2 dproductspec[\"\", . . product , 1238");

Operator arraytypeStream1Op("arraytypeStream1", arraytypeStream1Spec.getStr(),
                            0, Operator::SimpleSelect, arraytypeStreamTMT<1>);

Operator arraytypeStream2Op("arraytypeStream2", arraytypeStream1Spec.getStr(),
                            0, Operator::SimpleSelect, arraytypeStreamTMT<2>);

/*
Type Map Operator subtype

*/
template <int pos1, int pos2> ListExpr SUBTYPETM(ListExpr args) {
...
OperatorSpec SUBTYPE1SPEC(" t(st) x ... -> st", "SUBTYPE1SPEC(_)",
                          "Type mapping operator.", "not for direct usage");

OperatorSpec SUBSUBTYPE1SPEC(" t(st1(st2)) x ... -> st2", "SUBSUBTYPE1SPEC(_)",
                             "Type mapping operator.", "not for direct usage");

OperatorSpec SUBTYPE2SPEC(" a x t(st) x ... -> st", "SUBTYPE2SPEC(_,_)",
                          "Type mapping operator.", "not for direct usage");

Operator SUBTYPE1OP("SUBTYPE1", SUBTYPE1SPEC.getStr(), 0,
                    Operator::SimpleSelect, SUBTYPETM<1, 1>);

Operator SUBSUBTYPE1OP("SUBSUBTYPE1", SUBTYPE1SPEC.getStr(), 0,
                       Operator::SimpleSelect, SUBTYPETM<1, 2>);

Operator SUBTYPE2OP("SUBTYPE2", SUBTYPE2SPEC.getStr(), 0,
                    Operator::SimpleSelect, SUBTYPETM<2, 1>);

/*
TypeMapOperators ARRAYFUNARG1, ARRAYFUNARG2 up to ARRAYFUNARG8

*/
template <int pos, bool makeFS> ListExpr ARRAYFUNARG(ListExpr args) {
  ...
  if (DArray::checkType(arg) || SDArray::checkType(arg)) {
    return nl->Second(arg);
  }

  if (DFArray::checkType(arg) || DFMatrix::checkType(arg)) {
    ListExpr res;
    if (makeFS) {
      res = nl->TwoElemList(listutils::basicSymbol<fsrel>(),
                            nl->Second(nl->Second(arg)));
    } else {
      res = nl->TwoElemList(listutils::basicSymbol<frel>(),
                            nl->Second(nl->Second(arg)));
    }
    return res;
  }
...
OperatorSpec
    ARRAYFUNARG1SPEC("darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)",
                     "ARRAYFUNARG1(_)", "Type mapping operator.",
                     "query df1 dmap [\"df3\" . count]");

Operator ARRAYFUNARG1OP("ARRAYFUNARG1", ARRAYFUNARG1SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<1, false>);

OperatorSpec ARRAYFUNARG2SPEC(
    " any x darray(X) x ... -> X, any x dfarray(rel(X)) x ... -> frel(X)",
    " ARRAYFUNARG2(_)", "Type mapping operator.",
    "query df1 df2 dmap2 [\"df3\" . feed  .. feed  product, 1238]");
Operator ARRAYFUNARG2OP("ARRAYFUNARG2", ARRAYFUNARG2SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<2, false>);

OperatorSpec
    ARRAYFUNARG3SPEC("darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)",
                     "ARRAYFUNARG3(_,_,_)", "Type mapping operator.",
                     "query df1 df2 df3 dmap3 [\"df9\" . count]");

Operator ARRAYFUNARG3OP("ARRAYFUNARG3", ARRAYFUNARG3SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<3, false>);
OperatorSpec
    ARRAYFUNARG4SPEC("darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)",
                     "ARRAYFUNARG4(_,_,_,_)", "Type mapping operator.",
                     "query df1 df2 df3 df4 dmap4 [\"df9\" . count]");

Operator ARRAYFUNARG4OP("ARRAYFUNARG4", ARRAYFUNARG4SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<4, false>);
OperatorSpec
    ARRAYFUNARG5SPEC("darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)",
                     "ARRAYFUNARG5(_,_,_,_,_)", "Type mapping operator.",
                     "query df1 df2 df3 df4 df5 dmap5 [\"df9\" . count]");

Operator ARRAYFUNARG5OP("ARRAYFUNARG5", ARRAYFUNARG5SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<5, false>);

OperatorSpec
    ARRAYFUNARG6SPEC("darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)",
                     "ARRAYFUNARG6(_,_,_,_,_,_)", "Type mapping operator.",
                     "query df1 df2 df3 df4 df5 df5 dmap6 [\"df9\" . count]");

Operator ARRAYFUNARG6OP("ARRAYFUNARG6", ARRAYFUNARG6SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<6, false>);

OperatorSpec ARRAYFUNARG7SPEC(
    "darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)",
    "ARRAYFUNARG7(_,_,_,_,_,_,_)", "Type mapping operator.",
    "query df1 df2 df3 df4 df5 df6 df7 dmap7 [\"df9\" . count]");

Operator ARRAYFUNARG7OP("ARRAYFUNARG7", ARRAYFUNARG7SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<7, false>);

OperatorSpec ARRAYFUNARG8SPEC(
    "darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)", "ARRAYFUNARG8(_)",
    "Type mapping operator.",
    "query df1 df2 df3 df4 df5 df6 df7 df8 dmap8 [\"df3\" . count]");

Operator ARRAYFUNARG8OP("ARRAYFUNARG8", ARRAYFUNARG8SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<8, false>);

OperatorSpec ARRAYFUNARG9SPEC(
    "darray(X) x ... -> X, dfarray(rel(X)) x ... -> frel(X)", "ARRAYFUNARG9(_)",
    "Type mapping operator.",
    "query is df1 df2 df3 df4 df5 df6 df7 df8 pdmap8 [\"df3\" . count]");

Operator ARRAYFUNARG9OP("ARRAYFUNARG9", ARRAYFUNARG9SPEC.getStr(), 0,
                        Operator::SimpleSelect, ARRAYFUNARG<9, false>);

OperatorSpec
    AREDUCEARG1SPEC("darray(X) x ... -> X, dfarray(rel(X)) x ... -> stream(X)",
                    "AREDUCEARG1(_)", "Type mapping operator.",
                    "query df1  areduce[\"d3\" . count , 1238]");

Operator AREDUCEARG1OP("AREDUCEARG1", AREDUCEARG1SPEC.getStr(), 0,
                       Operator::SimpleSelect, ARRAYFUNARG<1, true>);

OperatorSpec AREDUCEARG2SPEC(
    " any x darray(X) x ... -> X, any x dfarray(rel(X)) x ... -> stream(X)",
    " AREDUCEARG2(_)", "Type mapping operator.",
    "query df1 df2 areduce[\"df3\" . .. product, 1238]");

Operator AREDUCEARG2OP("AREDUCEARG2", AREDUCEARG2SPEC.getStr(), 0,
                       Operator::SimpleSelect, ARRAYFUNARG<2, true>);
...
OperatorSpec DFARRAYTUPLESPEC("dfarray((rel(X)) x ... -> X", "DFARRAYTUPLE1(_)",
                              "Type mapping operator.",
                              "query s7 partition[hashvlaue(.,23], \"m7\"]");

Operator DFARRAYTUPLEOP("DFARRAYTUPLE", DFARRAYTUPLESPEC.getStr(), 0,
                        Operator::SimpleSelect, DFARRAYTUPLETM);
...
int fileTransferServerVM(Word *args, Word &result, int message, Word &local,
                         Supplier s) {
...
Operator fileTransferServerOP("fileTransferServer",
                              fileTransferServerSpec.getStr(),
                              fileTransferServerVM, Operator::SimpleSelect,
...
ListExpr receiveFileClientTM(ListExpr args) {
  ...
  if (!isTextOrString(server) || !CcInt::checkType(port) ||
      !isTextOrString(remote) || !isTextOrString(local)) {
    return listutils::typeError(err);
  }
  return listutils::basicSymbol<CcBool>();
}
...
int receiveFileClientVMT(Word *args, Word &result, int message, Word &local,
                         Supplier s) {
  ...
  if (!Server->IsDefined() || !Port->IsDefined() || !Remote->IsDefined() ||
      !Local->IsDefined()) {
    res->SetDefined(false);
    return 0;
  }
...
Operator recieveFileClientOP("receiveFileClient",
                             receiveFileClientSpec.getStr(), 8,
                             receiveFileClientVM, receiveFileClientSelect,
                             receiveFileClientTM);
...
ListExpr transferFileTM(ListExpr args) {
  ...
  if (!CcInt::checkType(nl->First(args)) ||
      ...
      !CcInt::checkType(nl->Third(args)) || !isTextOrString(nl->Fourth(args)) ||
...
public:
  FileTransferServerStarter(ConnectionInfo *_ci, int _port)
      : ci(_ci), port(_port) //,error(0)
  {}
...
private:
  ...
  void run() {
    string serverQuery =
        "query fileTransferServer(" + stringutils::int2str(port) + ")";
    int err;
    string errMsg;
    string result;
    double runtime;
  ...
  void sendCancel() {
    Socket *socket = Socket::Connect(ci->getHost(), stringutils::int2str(port),
...
int transferFileVMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  if (!algInstance->serverExists(server1) ||
      !algInstance->serverExists(server2) || (port <= 0)) {
    res->Set(true, false);
    return 0;
  }
  ...
  string clientQuery = "query receiveFileClient( '" + ci1->getHost() + "', " +
                       stringutils::int2str(port) + ", '" + srcFile + "', '" +
                       targetFile + "')";
...
ValueMapping transferFileVM[] = {
    transferFileVMT<CcString, CcString>, transferFileVMT<CcString, FText>,
    transferFileVMT<FText, CcString>, transferFileVMT<FText, FText>};
...
Operator transferFileOP("transferFile", transferFileSpec.getStr(), 4,
                        transferFileVM, transferFileSelect, transferFileTM);
...
int traceCommandsVM(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  result = qp->ResultStorage(s);
...
OperatorSpec
    traceCommandsSpec("bool -> bool", "traceCommands(_)",
                      "Enables or disables tracing of remote commands.",
                      "query traceCommands(false)");

Operator traceCommandsOp("traceCommands", traceCommandsSpec.getStr(),
                         traceCommandsVM, Operator::SimpleSelect,
                         traceCommandsTM);
...
int showProgressVM(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *arg = (CcBool *)args[0].addr;
  CcBool *res = (CcBool *)result.addr;
...
OperatorSpec
    showProgressSpec("bool -> bool", "showProgress(_)",
                     "Enables or disables progress view for the prcmd command.",
                     "query showProgress(false)");

Operator showProgressOp("showProgress", showProgressSpec.getStr(),
                        showProgressVM, Operator::SimpleSelect, showProgressTM);
...
public:
  static staticFileTransferator *getInstance(int port, int &noTransfers) {
    ...
    if (staticFileTransferators.find(port) != staticFileTransferators.end()) {
      staticFileTransferator *res = it->second;
      noTransfers = res->maxTransfers;
      return res;
    }

    staticFileTransferator *res = new staticFileTransferator(port, noTransfers);
    if (res->running) {
...
int staticFileTransferatorVM(Word *args, Word &result, int message, Word &local,
                             Supplier s) {
...
OperatorSpec staticFileTransferatorSpec(
    "int x int -> bool", "staticFileTransferator(port, maxConnections)",
...
Operator staticFileTransferatorOp("staticFileTransferator",
...
OperatorSpec killStaticFileTransferatorSpec(
    "int -> bool", "killStaticFileTransferator(port)",
...
Operator killStaticFileTransferatorOp("killStaticFileTransferator",
...
ListExpr putOrGetFileTCPTM(ListExpr args) {
  ...
  if (!isTextOrString(nl->First(args)) || !isTextOrString(nl->Second(args)) ||
      !CcInt::checkType(nl->Third(args)) ||
      !CcBool::checkType(nl->Fourth(args)) ||
      !isTextOrString(nl->Fifth(args))) {
    return listutils::typeError(err);
...
class fileCopy {
public:
  fileCopy(const string &_local, const string &_host, const int _port,
           const bool _overwrite, const string &_remote)
      : local(_local), host(_host), port(_port), overwrite(_overwrite),
        remote(_remote) {}
...
int putOrGetFileTCPVMT(Word *args, Word &result, int message, Word &local,
                       Supplier s) {
...
Operator putFileTCPOp("putFileTCP", putFileTCPSpec.getStr(), 8, putFileTCPVM,
                      putOrGetFileTCPSelect, putOrGetFileTCPTM);
...
Operator getFileTCPOp("getFileTCP", getFileTCPSpec.getStr(), 8, getFileTCPVM,
                      putOrGetFileTCPSelect, putOrGetFileTCPTM);
...
ListExpr fsfeed5TM(ListExpr args) {
  ...
  if (!Stream<FText>::checkType(s) && !Stream<CcString>::checkType(s)) {
    return listutils::typeError(err);
  }
...
template <class T> class fsfeed5Info {
public:
...
template <class T>
int fsfeed5VMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  case OPEN:
    ...
    local.addr = new fsfeed5Info<T>(args[0], nl->Second(GetTupleResultType(s)));
    return 0;
  case REQUEST:
    result.addr = li ? li->next() : 0;
    return result.addr ? YIELD : CANCEL;
...
ValueMapping fsfeed5VM[] = {fsfeed5VMT<CcString>, fsfeed5VMT<FText>};
...
Operator fsfeed5Op("fsfeed5", fsfeed5Spec.getStr(), 2, fsfeed5VM, fsfeed5Select,
                   fsfeed5TM);
...
ListExpr partitionTM(ListExpr args) {
  ...
  if (!nl->HasLength(a0, 2) || !nl->HasLength(f0, 2) || !nl->HasLength(n0, 2) ||
      !nl->HasLength(s0, 2)) {
    return listutils::typeError("internal error");
  }
  ...
  ListExpr append =
      nl->HasLength(args, 4)
          ...
          : nl->TwoElemList(nl->IntAtom(0), nl->TextAtom(nl->ToString(rfun)));

  ListExpr res =
      nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), append,
                        nl->TwoElemList(listutils::basicSymbol<DFMatrix>(), r));
  return res;
}

template <class A> class partitionInfo {

public:
  partitionInfo(A *_array, int _resSize, size_t _wnum, ConnectionInfo *_ci,
                const string &_sfun, const string &_dfun, string &_tname,
                ListExpr _relType, const string &_dbname)
      : array(_array), resSize(_resSize), workerNumber(_wnum), ci(_ci),
        sfun(_sfun), dfun(_dfun), tname(_tname), sname(array->getName()),
...
private:
  ...
  void run() {
    ...
    if (cmd == "") {
      cerr << "worker " << workerNumber << " does not contain any slot" << endl;
      return;
    }
    string res;
    ...
    if (err != 0) {
      ...
      ci->getInterface()->removeMessageHandler(
          algInstance->progressObserver->getProgressListener(
              array->getWorker(workerNumber)));
      algInstance->progressObserver->commitWorkingOnElementFinished(
          array->getWorker(workerNumber));
#endif
    }
  }
  ...
  string constructQuery(const string &targetDir) {
    switch (array->getType()) {
    case DARRAY:
      return constructQueryD(targetDir);
    case DFARRAY:
      return constructQueryDF(targetDir);
    case DFMATRIX:
      assert(false);
    case SDARRAY:
      assert(false);
  ...
  string constructQueryD(const string &targetDir) {
    ...
    string stream2 =
        "(feedS " + stream1 + "(" + nl->ToString(relType) + " ()))";
    ...
    query << "(query "
          << " (count "
          << "   ( fdistribute7 " << stream3 << " '" << targetDir << "/"
          << tname << "' "
          << " " << dfun << " " << resSize << " TRUE "
          << " )))";
    return query.str();
  ...
  string constructQueryDF(const string &dir) {
    ...
    string sourceDir = ci->getSecondoHome(showCommands, commandLog) +
                       "/dfarrays/" + dbname + "/" + sname + "/";
    ...
    ListExpr fnrelType = nl->TwoElemList(
        ...
        nl->TwoElemList(
            ...
            nl->OneElemList(nl->TwoElemList(nl->SymbolAtom("T"),
                                            listutils::basicSymbol<FText>()))));
    ...
    for (size_t i = 0; i < array->getSize(); i++) {
      if (array->getWorkerIndexForSlot(i) == workerNumber) {
        ListExpr F = nl->OneElemList(nl->TextAtom(
            sourceDir + sname + "_" + stringutils::int2str(i) + ".bin"));
        if (first) {
    ...
    ListExpr stream = nl->ThreeElemList(
        nl->SymbolAtom("projecttransformstream"), feed, nl->SymbolAtom("T"));

    string streamer = sfun.empty() ? "fsfeed5" : "createFSrel";

    ListExpr fsfeed =
        nl->ThreeElemList(nl->SymbolAtom(streamer), stream, relTemp);
    ...
    ListExpr fdistribute =
        nl->SixElemList(nl->SymbolAtom("fdistribute7"), streamFun,
                        nl->TextAtom(dir + "/" + tname), dfunl,
                        nl->IntAtom(resSize), nl->BoolAtom(true));

    ListExpr query =
        nl->TwoElemList(nl->SymbolAtom("query"),
                        nl->TwoElemList(nl->SymbolAtom("count"), fdistribute));
...
template <class A>
int partitionVMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
  ...
  if (newSize->IsDefined() && newSize->GetValue() > 0) {
  ...
  for (size_t i = 0; i < array->numOfWorkers(); i++) {
    ...
    if (ci) {
      partitionInfo<A> *info = new partitionInfo<A>(
          array, size, i, ci, funText, dfunText, tname, relType, dbname);
      ci->deleteIfAllowed();
      infos.push_back(info);
    }
  }
...
ValueMapping partitionVM[] = {partitionVMT<DFArray>, partitionVMT<DArray>};

Operator partitionOp("partition", partitionSpec.getStr(), 2, partitionVM,
...
ListExpr partitionFTM(ListExpr args) {
  ...
  ListExpr expFunArg = DArray::checkType(a1)
                           ? subtype
                           : nl->TwoElemList(listutils::basicSymbol<fsrel>(),
                                             nl->Second(subtype));
  ...
  if (!nl->Equal(nl->Second(f1Res), d1UsedArg)) {
    return listutils::typeError(
        "type mismatch between result of the funciton "
  ...
  ListExpr dfcomp =
      nl->HasLength(funDef, 3) ? nl->Third(funDef) : nl->Fourth(funDef);

  // rewrite function
  ListExpr rfunDef = nl->ThreeElemList(
      nl->First(funDef), nl->TwoElemList(nl->First(fdarg), expFunArg), dfcomp);
  ...
  ListExpr ddarg =
      nl->HasLength(dfunDef, 3) ? nl->Second(dfunDef) : nl->Third(dfunDef);
  ListExpr ddcomp =
      nl->HasLength(dfunDef, 3) ? nl->Third(dfunDef) : nl->Fourth(dfunDef);

  ListExpr rdfunDef = nl->ThreeElemList(
      nl->First(dfunDef), nl->TwoElemList(nl->First(ddarg), nl->Second(f1Res)),
      ddcomp);

  ListExpr appendList = nl->TwoElemList(nl->TextAtom(nl->ToString(rfunDef)),
                                        nl->TextAtom(nl->ToString(rdfunDef)));

  ListExpr resType = nl->TwoElemList(
      listutils::basicSymbol<DFMatrix>(),
      nl->TwoElemList(listutils::basicSymbol<Relation>(), nl->Second(f1Res)));
...
OperatorSpec FFRSpec("d[f]array(rela(tuple(X))) x A -> frel(tuple(X)) or  "
                     "B x C x (frel(tuple(D) -> stream(tuple(E)) ) -> tuple(E)",
                     " FFR(_,_,_)", "Type Map Operator",
                     "query FFR(test) getTypeNL");

Operator FFROp("FFR", FFRSpec.getStr(), 0, Operator::SimpleSelect, FFRTM);
...
Operator partitionFOp("partitionF", partitionFSpec.getStr(), 2, partitionVM,
                      partitionSelect, partitionFTM);
...
ListExpr areduceTM(ListExpr args) {
  ...
  if (!DFMatrix::checkType(m1) || !CcString::checkType(n1) ||
      !listutils::isMap<1>(f1) || !CcInt::checkType(p1)) {
    return listutils::typeError(err);
  }
  ...
  ListExpr fsrelt = nl->TwoElemList(listutils::basicSymbol<fsrel>(), tupleType);
  ...
  ListExpr res;
  if (!isF) {
    res = nl->TwoElemList(listutils::basicSymbol<DArray>(), funres);
  } else {
    if (!isStream) {
      res = nl->TwoElemList(listutils::basicSymbol<DFArray>(), funres);
    } else {
  ...
  ListExpr fres = nl->ThreeElemList(
      nl->SymbolAtom(Symbols::APPEND()),
      nl->TwoElemList(nl->TextAtom(nl->ToString(rfun)), nl->BoolAtom(isStream)),
      res);
  ...
  if (error != 0) {
    ...
    if (!nl->HasLength(resList, 2)) {
      ...
      if (nl->AtomType(rv) != IntType) {
        cerr << " unexpected result " << endl << nl->ToString(resList) << endl;
      } else {
        res = nl->IntValue(rv);
      }
    }
...
template <class R> class AReduceTask {

public:
  AReduceTask(DFMatrix *_matrix1, DFMatrix *_matrix2, int _worker, R *_result,
              int _port, string &_funtext, ListExpr _relType1,
              ListExpr _relType2, bool _isStream, AReduceListener *_listener)
      : matrix1(_matrix1), matrix2(_matrix2), worker(_worker), result(_result),
        port(_port), funtext(_funtext), relType1(_relType1),
    ...
    ci = algInstance->getWorkerConnection(matrix1->getWorker(worker), dbname);
    runner = 0;
    running = false;
  }
...
private:
  ...
  void run() {
    stringstream ss;
    ss << "process slot " << currentSlot << " on worker " << worker << endl;
    ...
    ci->simpleCommand(cmd, err, errMsg, res, false, runtime, showCommands,
                      commandLog, false, algInstance->getTimeout());
    if (err) {
      cerr << "creating directory " << dir << " on worker " << worker
           << "failed" << endl;
      writeLog(ci, cmd, errMsg);
    ...
    for (size_t i = 0; i < matrix1->numOfWorkers(); i++) {
      ConnectionInfo *wi =
          algInstance->getWorkerConnection(matrix1->getWorker(i), dbname);
      string remote1 = wi->getSecondoHome(showCommands, commandLog) +
                       "/dfarrays/" + dbname + "/" + matrix1->getName() + "/" +
                       stringutils::int2str(i) + "/" + matrix1->getName() +
                       "_" + stringutils::int2str(currentSlot) + ".bin";
      string local1 = dir + "/" + matrix1->getName() + "_" +
                      stringutils::int2str(i) + ".bin";
      rv += "( '" + remote1 + "' '" + wi->getHost() + "' '" + local1 + "' 1 )";
      // also insert the files for matrix2 if necessary
      if (matrix2 && (matrix2->getName() != matrix1->getName())) {
        string remote2 = wi->getSecondoHome(showCommands, commandLog) +
                         "/dfarrays/" + dbname + "/" + matrix2->getName() +
                         "/" + stringutils::int2str(i) + "/" +
                         matrix2->getName() + "_" +
    ...
    if (err) {
      cerr << "command  " << cmd << " on worker " << worker << "failed" << endl;
      cerr << errMsg << endl;
      writeLog(ci, cmd, errMsg);
      listener->ready(currentSlot, worker, runner);
      return;
    ...
    if (!nl->IsEmpty(nl->Second(resList))) {
      errMsg = "Got errors during file transfer: " + nl->ToString(resList);
      cerr << errMsg << endl;
      writeLog(ci, cmd, errMsg);
      listener->ready(currentSlot, worker, runner);
      return;
    ...
    string tuplestream1 = " ( createFSrel (projecttransformstream" +
                          tuplestream0_1 + " L )(" + nl->ToString(relType1) +
                          "()) )";

    string tuplestream2 = ""; // no tuple stream
    if (matrix2) {
      tuplestream2 = "( createFSrel (projecttransformstream" + tuplestream0_2 +
    ...
    if (result->getType() == DARRAY) { // a usual let command
      ...
      string objname =
          result->getName() + "_" + stringutils::int2str(currentSlot);
      ci->simpleCommand("delete " + objname, err, errMsg, res, false, runtime,
                        showCommands, commandLog, false,
                        algInstance->getTimeout());
      // ignore error about non existent object
      ...
      string tdir = ci->getSecondoHome(showCommands, commandLog) +
                    "/dfarrays/" + dbname + "/" + result->getName() + "/";
      ...
      cmd = "( query ( count ( fconsume5 " + feed1 + "" + kt + " " + feed2 +
            "'" + filename + "')))";
    }
    ci->simpleCommandFromList(cmd, err, errMsg, res, false, runtime,
                              showCommands, commandLog, false,
    ...
    ci->simpleCommand(cmd, err, errMsg, res, false, runtime, showCommands,
                      commandLog, false, algInstance->getTimeout());
    if (err) {
      cerr << "removing directory " << dir << " on worker " << worker
           << "failed" << endl;
      cerr << errMsg << endl;
      writeLog(ci, cmd, errMsg);
    }
...
*/
template <bool tupleCount> class getTupleCounts {
public:
  getTupleCounts(DFMatrix *_matrix, int _wnumber, const std::string &_dbname)
      : matrix(_matrix), wnumber(_wnumber), dbname(_dbname), finished(false) {
...
private:
  ...
  void run2() {
    ...
    string basename = ci->getSecondoHome(showCommands, commandLog) +
                      "/dfarrays/" + dbname + "/" + name + "/" +
                      stringutils::int2str(wnumber) + "/" + name + "_";
    int error;
    string errMsg;
    ListExpr resList = nl->TheEmptyList();
    bool rewrite = false;
    ...
    for (size_t slot = 0; slot < matrix->getSize(); slot++) {
      ...
      ci->simpleCommand(query, error, errMsg, resList, rewrite, runtime,
  ...
  */
  ...
  void run() {
    ...
    bool rewrite = false;
    double runtime;
    ci->simpleCommand(query.str(), error, errMsg, resList, rewrite, runtime,
                      showCommands, commandLog);
...
template <bool tupleSizes> class matrixSlotSizes {
...
public:
  ...
  vector<int> getSlotSizes() { return slotSizes; }
...
template <class R> class AReducer : public AReduceListener {
public:
  ...
  vector<int> combineSlotSizes(vector<int> &slotsizes1, vector<int> &slotsizes2,
                               void *function) {
  ...
  void reduce(void *function) {
    ...
    for (size_t worker = 0; worker < maxStart; worker++) {
      tasks.push_back(new AReduceTask<R>(matrix1, matrix2, worker, result, port,
                                         funtext, relType1, relType2, isStream,
                                         this));
    }
...
template <class R>
int areduceVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  AReducer<R> reducer(matrix1, matrix2, res, port, funtext, isStream, relType1,
                      relType2);
...
ValueMapping areduceVM[] = {areduceVMT<DFArray>, areduceVMT<DArray>};

int areduceSelect(ListExpr args) {

  ListExpr funRes;

  funRes = nl->HasLength(args, 4) ? nl->Third(nl->Third(args))
                                  : nl->Fourth(nl->Fourth(args));

  if (Stream<Tuple>::checkType(funRes) || Relation::checkType(funRes)) {
    return 0;
  }
  return 1;
}

Operator areduceOp("areduce", areduceSpec.getStr(), 2, areduceVM, areduceSelect,
                   areduceTM);
...
ListExpr areduce2TM(ListExpr args) {
  ...
  // check types
  if (!DFMatrix::checkType(a1t) || !DFMatrix::checkType(a2t) ||
      !CcString::checkType(a3t) || !listutils::isMap<2>(a4t) ||
      !CcInt::checkType(a5t)) {
    return listutils::typeError(err);
  }
  ...
  ListExpr s1 = nl->TwoElemList(listutils::basicSymbol<fsrel>(),
                                nl->Second(nl->Second(a1t)));

  ListExpr s2 = nl->TwoElemList(listutils::basicSymbol<fsrel>(),
                                nl->Second(nl->Second(a2t)));

  if (!nl->Equal(s1, fa1) || !nl->Equal(s2, fa2)) {
  ...
  if (!listutils::isStream(funRes)) {
    if (!Relation::checkType(funRes)) {
      ...
      resType = nl->TwoElemList(listutils::basicSymbol<DFArray>(), funRes);
    }
  ...
  ListExpr rfundef = nl->FourElemList(
      ...
      nl->TwoElemList(nl->First(nl->Second(fundef)), fa1),
      nl->TwoElemList(nl->First(nl->Third(fundef)), fa2), nl->Fourth(fundef));
...
Operator areduce2Op("areduce2", areduce2Spec.getStr(), 2, areduceVM,
                    areduceSelect, areduce2TM);
...
Operator areduce2FOp("areduce2F", areduce2FSpec.getStr(), 2, areduceVM,
                     areduceSelect, areduce2TM);
...
ListExpr collectBTM(ListExpr args) {
  ...
  if (nl->HasLength(args, 4)) {
    ...
    if (!CcInt::checkType(nl->Second(fun)) ||
        !CcReal::checkType(nl->Third(fun))) {
      return listutils::typeError(err +
...
public:
  slotGetter(int _myNumber, string &_sname, string &_tname, int _size,
             const vector<ConnectionInfo *> &_workers, int _port,
             DFArray *resultArray, string _constrel)
      : myNumber(_myNumber), sname(_sname), tname(_tname),
        // size(_size),
        workers(_workers), port(_port), resArray(resultArray),
        constrel(_constrel) {
...
private:
  ...
  void run() {
    ...
    ci->getInterface()->addMessageHandler(
        algInstance->progressObserver->getProgressListener(
            workers[myNumber]->getHost(), workers[myNumber]->getPort(),
            workers[myNumber]->getNum(), "part_1"));
#endif
    // temoporal directory for partitined slots
    string dir = "tmp/" + tname + "/" + stringutils::int2str(myNumber) + "/";
    // final directory for dfarray
    string dbname = SecondoSystem::GetInstance()->GetDatabaseName();
    string tdir = ci->getSecondoHome(showCommands, commandLog) + "/dfarrays/" +
                  dbname + "/" + tname + "/";
    int err;
    string errMsg;
    double runtime;
    ...
    ci->getInterface()->addMessageHandler(
        algInstance->progressObserver->getProgressListener(
            workers[myNumber]->getHost(), workers[myNumber]->getPort(),
            workers[myNumber]->getNum(), "part_2"));
#endif
  ...
  void getFilesFromWorker(size_t worker, const string &myDir,
                          ConnectionInfo *ci, DFArray *resArray) {
    ...
    string sbasename = w->getSecondoHome(showCommands, commandLog) +
                       "/dfarrays/" + dbname + "/" + sname + "/" +
                       stringutils::int2str(worker) + "/" + sname + "_";
    ...
    for (size_t slot = 0; slot < resArray->getSize(); slot++) {
      ...
      if (wfs == myNumber) {
        ...
        frel += "( '" + sbasename + n + ".bin'  '" + myDir + "s_" + n + "_" +
                ws + "')";
      }
    }
  ...
  void createSlot(int num, const string &sdir, const string &tdir,
    ...
    string cmd = " query getDirectory('" + sdir + "') " +
                 ...
                 stringutils::int2str(num) + "_\"]" + " fsfeed5[" + constrel +
                 "] fconsume5['" + tdir + tname + "_" +
                 stringutils::int2str(num) + ".bin'] count";
    int err;
    string errMsg;
    double runtime;
    string res;
...
template <bool tupleSizes> class BalancedSlotDistributor {
public:
  static bool distribute(Word *args, int noArgs, DFArray *res) {
    DFMatrix *matrix = (DFMatrix *)args[0].addr;
    ...
    vector<uint32_t> mapping =
        loadbalance::getMapping(ss, matrix->numOfWorkers(), false);
    res->set(mapping, res->getName(), res->getWorkers());
    return true;
  }
...
int collect2VMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  for (size_t i = 0; i < cis.size(); i++) {
    ...
    slotGetter *getter =
        new slotGetter(i, sname, n, matrix->getSize(), cis, p, res, constRel);
    getters.push_back(getter);
  }
...
int collect2VM(Word *args, Word &result, int message, Word &local, Supplier s) {
  return collect2VMT<StdSlotDistributor>(args, result, message, local, s);
}

OperatorSpec
    collect2Spec("dfmatrix x string x int -> dfarray", " _ collect2[ _ , _] ",
...
Operator collect2Op("collect2", collect2Spec.getStr(),
                    collect2VMT<StdSlotDistributor>, Operator::SimpleSelect,
                    collect2TM);

OperatorSpec
    collectCSpec("dfmatrix x string x int x vector(int) -> dfarray",
                 " _ collectC[ _ , _, _] ",
                 "Collects the slots of a matrix into a "
                 " dfarray. The string is the name of the "
                 "resulting array, the int value specified a "
...
Operator collectCOp("collectC", collectCSpec.getStr(),
                    collect2VMT<VectorSlotDistributor>, Operator::SimpleSelect,
                    collectCTM);
...
Operator collectBOp("collectB", collectBSpec.getStr(),
                    collect2VMT<BalancedSlotDistributor<true>>,
                    Operator::SimpleSelect, collectBTM);

OperatorSpec
    collectDSpec("dfmatrix x string x int -> dfarray", " _ collectD[ _ , _] ",
                 "Collects the slots of a matrix into a "
                 " dfarray. The string is the name of the "
                 "resulting array, the int value specifies a "
                 "port for file transfer. The port value can be any "
...
Operator collectDOp("collectD", collectDSpec.getStr(),
...
OperatorSpec
    loadBalanceSpec("stream(real) x int x bool -> stream(int) ",
                    " _ loadBalance[_,_] ",
...
Operator loadBalanceOp("loadBalance", loadBalanceSpec.getStr(), loadBalanceVM,
                       Operator::SimpleSelect, loadBalanceTM);
...
template <class T>
int saveAttrVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  res->Set(true, FileAttribute::saveAttribute(qp->GetType(qp->GetSon(s, 0)),
                                              attr, filename->GetValue()));
  return 0;
}

ValueMapping saveAttrVM[] = {saveAttrVMT<CcString>, saveAttrVMT<FText>};

int saveAttrSelect(ListExpr args) {
  return CcString::checkType(nl->Second(args)) ? 0 : 1;
}

OperatorSpec saveAttrSpec("DATA x {string,text} -> bool", " _ saveAttr[_] ",
                          "saves an attribute in binary format "
                          "to a file",
                          "query 6 saveAttr['six.bin']");

Operator saveAttrOp("saveAttr", saveAttrSpec.getStr(), 2, saveAttrVM,
                    saveAttrSelect, saveAttrTM
...
template <class T>
int loadAttrVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
...
ValueMapping loadAttrVM[] = {loadAttrVMT<CcString>, loadAttrVMT<FText>};

int loadAttrSelect(ListExpr args) {
  return (CcString::checkType(nl->First(args))) ? 0 : 1;
}

OperatorSpec loadAttrSpec("{string,text} -> DATA ", "loadAttr(_)",
                          "loads an attribute from a binary file",
                          "query loadAttribute('six.bin')");

Operator loadAttrOp("loadAttr", loadAttrSpec.getStr(), 2, loadAttrVM,
                    loadAttrSelect, loadAttrTM);
...
ListExpr createFrelTM(ListExpr args) {
  ...
  if (!CcString::checkType(arg1t) && !FText::checkType(arg1t)) {
    return listutils::typeError(err + " (first arg not of type string or text");
  }
  if (nl->HasLength(args, 1)) {
    ...
    ListExpr relType = fi.getRelType();
    return nl->TwoElemList(listutils::basicSymbol<frel>(), nl->Second(relType));
  }
...
template <class T>
int createFrelVMT(Word *args, Word &result, int message, Word &local,
                  Supplier s) {
  ...
  bool ok =
      BinRelWriter::writeRelationToFile((Relation *)args[1].addr, relType, fn);
...
ValueMapping createFrelVM[] = {createFrelVMT<CcString>, createFrelVMT<FText>};
...
Operator createFrelOP("createFrel", createFrelSpec.getStr(), 2, createFrelVM,
                      createFrelSelect, createFrelTM);
...
template <class T>
int createFSrelVMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
...
ValueMapping createFSrelVM[] = {createFSrelVMT<CcString>,
...
OperatorSpec createFSrelSpec(
    " stream({string,text}) x rel(x) -> fsrel(x)", " _ createFSRel[_]",
...
Operator createFSrelOP("createFSrel", createFSrelSpec.getStr(), 2,
                       createFSrelVM, createFrelSelect, createFSrelTM);
...
ListExpr saveObjectToFileTM(ListExpr args) {
  ...
  if (nl->HasLength(a1, 2) && listutils::isStream(nl->First(a1))) {
    ...
    a1 = nl->TwoElemList(listutils::basicSymbol<Relation>(), nl->Second(a1));
  }
  ListExpr a2 = nl->Second(args);
  ...
  return nl->TwoElemList(listutils::basicSymbol<fobj>(), a1);
}
...
*/
template <class T>
int saveObjectToFileVMT(Word *args, Word &result, int message, Word &local,
                        Supplier s) {
...
ValueMapping saveObjectToFileVM[] = {saveObjectToFileVMT<CcString>,
...
OperatorSpec
    saveObjectToFileSpec("D x {string, text} -> fobj(D)",
                         " _ saveObjectToFile[_]",
                         "Saves an object into a binary file.",
                         "query strassen saveObjectToFile['strassen.bin']");

Operator saveObjectToFileOp("saveObjectToFile", saveObjectToFileSpec.getStr(),
                            2, saveObjectToFileVM, saveObjectToFileSelect,
                            saveObjectToFileTM);
...
ListExpr getObjectFromFileTM(ListExpr args) {
  ...
  if (!CcString::checkType(a1t) && !FText::checkType(a1t) &&
      !fobj::checkType(a1t)) {
  ...
  if (fobj::checkType(a1t)) {
    ...
    return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()),
                             nl->OneElemList(nl->BoolAtom(isstream)), subtype);
  }
  ...
  if (Relation::checkType(r)) {
    r = nl->TwoElemList(listutils::basicSymbol<Stream<Tuple>>(), nl->Second(r));
    isstream = true;
  }
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()),
                           nl->OneElemList(nl->BoolAtom(isstream)), r);
}
...
template <class T>
int getObjectFromFileVMT(Word *args, Word &result, int message, Word &local,
                         Supplier s) {
...
ValueMapping getObjectFromFileVM[] = {getObjectFromFileVMT<CcString>,
...
int getObjectFromFileSelect(ListExpr args) {
  ...
  ListExpr a = nl->First(args);
  return CcString::checkType(a) ? 0 : (FText::checkType(a) ? 1 : 2);
}

OperatorSpec
    getObjectFromFileSpec(" {string, text, fobj(X) -> [stream] X",
                          " _ getObjectFromFile ",
...
Operator getObjectFromFileOp("getObjectFromFile",
                             getObjectFromFileSpec.getStr(), 3,
                             getObjectFromFileVM, getObjectFromFileSelect,
                             getObjectFromFileTM);
...
ListExpr ddistribute8TM(ListExpr args) {
  ...
  string errMsg;
  if (!isWorkerRelDesc(nl->Sixth(nl->Rest(args)), workerPositions, workerTypes,
                       errMsg)) {
    return listutils::typeError(
        err + " (7th arg is not a worker relation: " + errMsg);
  }
  ...
  ListExpr appendList = listutils::concat(
      workerPositions,
      nl->TwoElemList(
          nl->BoolAtom(CcString::checkType(nl->First(workerTypes))),
          nl->BoolAtom(CcString::checkType(nl->Third(workerTypes)))));

  ListExpr subType = nl->TwoElemList(listutils::basicSymbol<Relation>(),
                                     nl->Second(nl->First(args)));

  ListExpr daType = nl->TwoElemList(listutils::basicSymbol<DArray>(), subType);
  ListExpr resType =
      nl->TwoElemList(listutils::basicSymbol<arrayalgebra::Array>(), daType);
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), appendList,
                           resType);
}

template <class AType, class HType, class CType, class DType>
int ddistribute8VMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  ...
  AType temp = DArrayBase::createFromRel<HType, CType, AType>(
      (Relation *)args[6].addr, size2, "blah", hostPos, portPos, configPos);
  ...
  for (int i = 0; i < nstreams; i++) {
    ...
    string fname = name + "_" + stringutils::int2str(array_num) + "_" +
                   stringutils::int2str(darray_num);
  ...
  for (int islots = 0; islots < size2; islots++) {
    ...
    for (int oslots = 0; oslots < size1; oslots++) {
      snames.push_back(name + "_" + stringutils::int2str(oslots) + "_" +
                       stringutils::int2str(islots));
    }
  ...
  for (size_t i = 0; i < names.size(); i++) {
    DType *transfer =
        new DType(names[i], (AType *)darrays[0].addr, i, names[i]);
    transfer->start();
...
OperatorSpec ddistribute8Spec(
    ...
    "The first function determines the slot within  the main array, whereas "
    "the "
    "second function determines the slot within the distributed array.",
    "query plz feed ddistribute8[\"plzd8\", .PLZ, hashvalue(.Ort,58], "
    "4, 12 ,workers] ");

Operator ddistribute8Op("ddistribute8", ddistribute8Spec.getStr(), 4,
                        ddistribute8VM, ddistribute8Select, ddistribute8TM);
...
Operator dfdistribute8Op("dfdistribute8", dfdistribute8Spec.getStr(), 4,
                         dfdistribute8VM, ddistribute8Select, ddistribute8TM);
...
public:
  partition8LocalInfo(Word _stream, Word _fun1, Word _fun2, const string &_home,
                      const string &_name, int _size1, int _size2, int _wnum,
                      ListExpr streamType)
      : stream(_stream), fun1(_fun1.addr), fun2(_fun2.addr), home(_home),
        name(_name), size1(_size1), size2(_size2), wnum(_wnum),
...
private:
  ...
  string getName(int i) { return name + "_" + stringutils::int2str(i); }
  ...
  void storeTuple(Tuple *tuple) {
    ...
    if (!outputWriters[index]) {
      string filename = getDirName(res1) + getName(res1) + "_" +
                        stringutils::int2str(res2) + ".bin";
...
int partition8LocalVM(Word *args, Word &result, int message, Word &local,
                      Supplier s) {
  ...
  case OPEN: {
    ...
    if (!home->IsDefined() || !name->IsDefined() || !size1->IsDefined() ||
        !size2->IsDefined()) {
      return 0;
    }
    ...
    local.addr = new partition8LocalInfo(
        args[0], args[1], args[2], home->GetValue(), name->GetValue(),
...
Operator partition8LocalOp("partition8Local", partition8LocalSpec.getStr(),
                           partition8LocalVM, Operator::SimpleSelect,
                           partition8LocalTM);
...
ListExpr partitionP8TM(ListExpr args) {
  ...
  ListExpr stream =
      nl->TwoElemList(listutils::basicSymbol<Stream<Tuple>>(), nl->Second(rel));

  if (!nl->Equal(f1a, nl->Second(at)) && !nl->Equal(f1a, stream)) {
  ...
  f1def = nl->ThreeElemList(
      nl->First(f1def), nl->TwoElemList(nl->First(nl->Second(f1def)), stream),
      nl->Third(f1def));

  f2def = nl->ThreeElemList(
      nl->First(f2def), nl->TwoElemList(nl->First(nl->Second(f2def)), tuplet),
      nl->Third(f2def));

  f3def = nl->ThreeElemList(
      nl->First(f3def), nl->TwoElemList(nl->First(nl->Second(f3def)), tuplet),
      nl->Third(f3def));

  // now we can build the result
  ListExpr restype = nl->TwoElemList(
      listutils::basicSymbol<arrayalgebra::Array>(),
      nl->TwoElemList(listutils::basicSymbol<DFMatrix>(), nl->Second(at)));

  ListExpr appendList = nl->ThreeElemList(nl->TextAtom(nl->ToString(f1def)),
...
template <class AType> class partitionF8Runner {
public:
  partitionF8Runner(AType *_array, ListExpr _atype, int _workerNumber,
                    int _asize, int _msize, const string &_fun1,
                    const string &_fun2, const string &_fun3,
                    const string _name, const vector<int> &_slots)
      : array(_array), atype(_atype), workerNumber(_workerNumber),
        asize(_asize), msize(_msize), fun1(_fun1), fun2(_fun2), fun3(_fun3),
...
private:
  ...
  void run() {
    ...
    if (ci) {
      ...
      cerr << "could not connet to " << elem.getHost() << "@" << elem.getPort()
           << endl;
    }
  }
  ...
  string createQuery() {
    ...
    string res = string("( count (partition8Local ") + "(" + fun1 +
                 getStream() + " ) " + " " + fun2 + " " + fun3 + " '" + home +
                 "' \"" + name + "\" " + stringutils::int2str(asize) + " " +
...
int partitionF8VMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  ...
  int typeId;
  string tname;
  SecondoSystem::GetCatalog()->LookUpTypeExpr(nl->Second(qp->GetType(s)), tname,
                                              algId, typeId);
  ...
  for (size_t i = 0; i < array->numOfWorkers(); i++) {
    if (!responsible[i].empty()) {
      partitionF8Runner<AType> *run =
          new partitionF8Runner<AType>(array, atype, i, asize, msize, fun1,
                                       fun2, fun3, name, responsible[i]);
...
ValueMapping partitionF8VM[] = {partitionF8VMT<DArray>,
...
Operator partitionF8Op("partitionF8", partitionF8Spec.getStr(), 2,
                       partitionF8VM, partitionF8Select, partitionP8TM);
...
OperatorSpec P8TMSpec(
    ...
    " P8TM(_,_,_) ", "Type Map operator.",
    "query array partitionF8[\"mynewname\",  . feed addcouunter[B, 16],"
    " .PLZ , .B , 8, 16]");

Operator P8TMOp("P8TM", P8TMSpec.getStr(), 0, Operator::SimpleSelect, P8TM);
...
int da2enableLogVM(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
...
OperatorSpec da2enableLogSpec(
    "bool -> bool", "da2enableLog(_)",
    "Enables or disables loggin within the Distributed2Algebra",
    "query da2enableLog(TRUE)");

Operator da2enableLogOp("da2enableLog", da2enableLogSpec.getStr(),
                        da2enableLogVM, Operator::SimpleSelect, da2enableLogTM);
...
int da2clearLogVM(Word *args, Word &result, int message, Word &local,
                  Supplier s) {
  commandLog.clear();
  result = qp->ResultStorage(s);
...
OperatorSpec
    da2clearLogSpec(" -> bool", "da2clearLog()",
                    "Removes log entries from the Distribute2Algebra log table",
                    "query da2clearLog()");

Operator da2clearLogOp("da2clearLog", da2clearLogSpec.getStr(), da2clearLogVM,
                       Operator::SimpleSelect, da2clearLogTM);
...
int da2LogVM(Word *args, Word &result, int message, Word &local, Supplier s) {
...
OperatorSpec da2LogSpec(" -> stream(tuple) ", "da2Log()",
                        "Returns a stream of tuples with informations "
                        "about remote secondo commands.",
                        "query da2Log() consume");

Operator da2LogOp("da2Log", da2LogSpec.getStr(), da2LogVM,
                  Operator::SimpleSelect, da2LogTM);
...
ListExpr deleteRemoteDatabasesTM(ListExpr args) {
  ...
  if (!isWorkerRelDesc(rel, positions, types, err)) {
    return listutils::typeError(relarg +
                                " arg is not a worker relation: " + err);
  }

  ListExpr appendList = nl->TwoElemList(
      nl->TwoElemList(nl->SymbolAtom("Ok"), listutils::basicSymbol<CcBool>()),
      nl->TwoElemList(nl->SymbolAtom("Msg"), listutils::basicSymbol<FText>()));
  ListExpr attrList = nl->Second(nl->Second(rel));
  ListExpr resType =
...
template <class H, class C> class deleteRemoteDatabasesInfo {
public:
  deleteRemoteDatabasesInfo(const string &_name, Relation *_rel, int _hpos,
                            int _ppos, int _cpos, ListExpr _resType)
      : name(_name), hpos(_hpos), ppos(_ppos), cpos(_cpos) {
    it = _rel->MakeScan();
  ...
  Tuple *next() {
    ...
    if (!host->IsDefined() || !port->IsDefined() || !config->IsDefined()) {
      ...
      ok = deleteDatabase(host->GetValue(), port->GetValue(), f, msg);
    }
...
private:
  ...
  bool deleteDatabase(const string &host, const int port, string &config,
                      string &msg) {
    NestedList *mynl = new NestedList("temp_nested_list");
    SecondoInterfaceCS *si = new SecondoInterfaceCS(true, mynl, true);
    string user = "";
    ...
    if (err.code == 0) {
      ...
      if (err.code == 0) {
        if (mynl->HasLength(resList, 2) &&
          ...
          cerr << "query secondoHome() returns unexpected result" << endl;
        }
      } else {
...
template <class H, class C>
int deleteRemoteDatabasesVMT(Word *args, Word &result, int message, Word &local,
                             Supplier s) {
  ...
  case OPEN: {
    ...
    local.addr = new deleteRemoteDatabasesInfo<H, C>(n, r, hpos, ppos, cpos, t);
    return 0;
  }
  case REQUEST:
    result.addr = li ? li->next() : 0;
...
OperatorSpec deleteRemoteDatabasesSpec(
    "string x rel -> stream(tuple)", "deleteRemoteDatabases(_,_)",
...
Operator deleteRemoteDatabasesOp("deleteRemoteDatabases",
                                 deleteRemoteDatabasesSpec.getStr(), 4,
...
template <class F>
int writeRelVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *res = (CcBool *)result.addr;
  Relation *rel = (Relation *)args[0].addr;
...
ValueMapping writeRelVM[] = {writeRelVMT<CcString>, writeRelVMT<FText>};

int writeRelSelect(ListExpr args) {
  return CcString::checkType(nl->Second(args)) ? 0 : 1;
}

OperatorSpec writeRelSpec("rel x {string,text} -> bool", "_ op[_]",
                          "Write the relation in the first argument into a "
                          "binary file specified by the second arg",
                          "query ten writeRel['ten.bin']");

Operator writeRelOp("writeRel", writeRelSpec.getStr(), 2, writeRelVM,
                    writeRelSelect, writeRelTM);
...
int write2VM(Word *args, Word &result, int message, Word &local, Supplier s) {
  return OperatorConsume::Consume(args, result, message, local, s);
}

OperatorSpec
    write2Spec("stream(tuple(X)) -> rel(tuple(X))", "_ write2",
...
Operator write2Op("write2", write2Spec.getStr(), write2VM,
                  Operator::SimpleSelect, write2TM);

OperatorSpec write3Spec(
    "stream(tuple(X)) -> rel(tuple(X))", "_ write3",
    "An alias for the consume operator. This operator will "
...
Operator write3Op("write3", write3Spec.getStr(), write2VM,
                  Operator::SimpleSelect, write2TM);
...
int db2tryReconnectVM(Word *args, Word &result, int message, Word &local,
                      Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *res = (CcBool *)result.addr;
  CcBool *arg = (CcBool *)args[0].addr;
...
Operator db2tryReconnectOp("db2tryReconnect", db2tryReconnectSpec.getStr(),
                           db2tryReconnectVM, Operator::SimpleSelect,
                           db2tryReconnectTM);
...
int setHeartbeatVM(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *res = (CcBool *)result.addr;
...
Operator setHeartbeatOp("setHeartbeat", setHeartbeatSpec.getStr(),
                        setHeartbeatVM, Operator::SimpleSelect, setHeartbeatTM);
...
int setTimeoutVM(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *res = (CcBool *)result.addr;
  CcInt *arg = (CcInt *)args[0].addr;
...
Operator setTimeoutOp("setTimeout", setTimeoutSpec.getStr(), setTimeoutVM,
                      Operator::SimpleSelect, setTimeoutTM);
...
template <class FN>
int db2LogToFileVMT(Word *args, Word &result, int message, Word &local,
                    Supplier s) {
  result = qp->ResultStorage(s);
  CcBool *res = (CcBool *)result.addr;
  if (qp->GetNoSons(s) == 0) {
...
ValueMapping db2LogToFileVM[] = {db2LogToFileVMT<CcString>,
                                 db2LogToFileVMT<FText>};

OperatorSpec db2LogToFileSpec(
    " -> bool | {string,text } -> bool", " db2LogToFile(_)",
...
Operator db2LogToFileOp("db2LogToFile", db2LogToFileSpec.getStr(), 2,
                        db2LogToFileVM, db2LogToFileSelect, db2LogToFileTM);
...
template <class T>
int enableDFSFTVMT(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
...
ValueMapping enableDFSFTVM[] = {enableDFSFTVMT<CcString>,
...
OperatorSpec
    enableDFSFTSpec("string x int -> bool", "enableDFSFT(host, port",
...
Operator enableDFSFTOp("enableDFSFT", enableDFSFTSpec.getStr(), 2,
                       enableDFSFTVM, enableDFSFTSelect, enableDFSFTTM);
...
int disableDFSFTVM(Word *args, Word &result, int message, Word &local,
                   Supplier s) {
...
Operator disableDFSFTOp("disableDFSFT", disableDSFFTSpec.getStr(),
                        disableDFSFTVM, Operator::SimpleSelect, disableDFSFTTM);
...
int removeTempInDFSVM(Word *args, Word &result, int message, Word &local,
                      Supplier s) {
...
Operator removeTempInDFSOp("removeTempInDFS", removeTempInDFSSpec.getStr(),
                           removeTempInDFSVM, Operator::SimpleSelect,
                           removeTempInDFSTM);
...
template <bool all>
int removeDFSFilesVM(Word *args, Word &result, int message, Word &local,
                     Supplier s) {
...
OperatorSpec removeAllDFSFilesSpec("-> bool", "removeAllDFSFiles() ",
                                   "Removes all files in a connected dfs. "
                                   "Returns the success.",
                                   " query removeAllDFSFiles() ");

Operator removeDFSFilesInDBOp("removeDFSFilesInDB",
                              removeDFSFilesInDBSpec.getStr(),
                              removeDFSFilesVM<false>, Operator::SimpleSelect,
                              removeDFSFiles);

Operator removeAllDFSFilesOp("removeAllDFSFiles",
                             removeAllDFSFilesSpec.getStr(),
                             removeDFSFilesVM<true>, Operator::SimpleSelect,
                             removeDFSFiles);
...
ListExpr createintdarrayTM(ListExpr args) {
  ...
  ListExpr workerAttrTypes;
  string errMsg;
  if (!isWorkerRelDesc(nl->Second(args), workerAttrPositions, workerAttrTypes,
                       errMsg)) {
...
int createintdarrayVMT(Word *args, Word &result, int message, Word &local,
                       Supplier s) {
  result = qp->ResultStorage(s);
  DArray *res = (DArray *)result.addr;
  ...
  if (res->IsDefined()) {
    ...
    for (size_t i = 0; i < res->getSize() && ok; i++) {
      DArrayElement elem = res->getWorkerForSlot(i);
      ConnectionInfo *ci =
          algInstance->getWorkerConnection(res, i, dbname, 0, true);
      if (!ci) {
        res->makeUndefined();
        ok = false;
      } else {
...
ValueMapping createintdarrayVM[] = {
    createintdarrayVMT<CcString, CcString>, createintdarrayVMT<CcString, FText>,
    createintdarrayVMT<FText, CcString>, createintdarrayVMT<FText, FText>};
...
OperatorSpec createintdarraySpec(
    "string x rel [x int] -> darray(int)", "createintdarray(name, workers)",
...
Operator createintdarrayOp("createintdarray", createintdarraySpec.getStr(), 4,
                           createintdarrayVM, createintdarraySelect,
                           createintdarrayTM);
...
template <class A> class dcommandInfo {

public:
  dcommandInfo(A *array, string cmd, TupleType *tt, bool useResult) {
    this->array = array;
    this->cmd = cmd;
    this->tt = tt;
    tt->IncReference();
...
private:
  ...
  void compute() {
    ...
    for (size_t i = 0; i < array->numOfWorkers(); i++) {
      ...
      ConnectionInfo *ci = algInstance->getWorkerConnection(elem, dbname);
      boost::thread *r =
          new boost::thread(boost::bind(&dcommandInfo::run, this, ci));
      runners.push_back(r);
      cons.push_back(ci);
    }
    for (size_t i = 0; i < runners.size(); i++) {
  ...
  void computeWOResult() {
    ...
    for (size_t i = 0; i < array->numOfWorkers(); i++) {
      ...
      ConnectionInfo *ci = algInstance->getWorkerConnection(elem, dbname);
      boost::thread *r =
          new boost::thread(boost::bind(&dcommandInfo::runWOResult, this, ci));
      runners.push_back(r);
      cons.push_back(ci);
    }
    for (size_t i = 0; i < runners.size(); i++) {
...
int dcommandVMT(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  case OPEN: {
    ...
    TupleType *tt = new TupleType(nl->Second(GetTupleResultType(s)));
    local.addr =
        new dcommandInfo<A>((A *)args[0].addr, cmd->GetValue(), tt, true);
    tt->DeleteIfAllowed();
    return 0;
  }
  case REQUEST:
...
ValueMapping dcommandVM[] = {
    dcommandVMT<DArray, CcString>,  dcommandVMT<DArray, FText>,
    dcommandVMT<DFArray, CcString>, dcommandVMT<DFArray, FText>,
    dcommandVMT<SDArray, CcString>, dcommandVMT<SDArray, FText>};
...
OperatorSpec
    dcommandSpec(" {d[f]array, sdarray} x {string, text} -> stream(tuple) ",
...
Operator dcommandOp("dcommand", dcommandSpec.getStr(), 6, dcommandVM,
                    dcommandSelect, dcommandTM);
...
template <class A, class C>
int dcommand2VMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
  result = qp->ResultStorage(s);
  A *res = (A *)result.addr;
  C *cmd = (C *)args[1].addr;
...
ValueMapping dcommand2VM[] = {
    dcommand2VMT<DArray, CcString>,  dcommand2VMT<DArray, FText>,
    dcommand2VMT<DFArray, CcString>, dcommand2VMT<DFArray, FText>,
    dcommand2VMT<SDArray, CcString>, dcommand2VMT<SDArray, FText>};

OperatorSpec
    dcommand2Spec(" {d[f]array, sdarray} x {string, text} -> d[f]array",
...
Operator dcommand2Op("dcommand2", dcommand2Spec.getStr(), 6, dcommand2VM,
                     dcommandSelect, dcommand2TM);
...
template <class A> class dletInfo {

public:
  dletInfo(A *array, string name, string cmd, TupleType *tt) {
    this->array = array;
    this->cmd = cmd;
    this->name = name;
    this->tt = tt;
...
private:
  ...
  void compute() {
    ...
    for (size_t i = 0; i < array->numOfWorkers(); i++) {
      ...
      if (ci) {
        boost::thread *r =
            new boost::thread(boost::bind(&dletInfo::run, this, ci));
        runners.push_back(r);
      } else {
    ...
    boost::thread *r =
        new boost::thread(boost::bind(&dletInfo::runLocal, this));
    runners.push_back(r);
    for (size_t i = 0; i < runners.size(); i++) {
      runners[i]->join();
      delete runners[i];
  ...
  void runLocal() {
    ...
    if (ctlg->IsObjectName(name)) {
      generateLocal(true, "Identifier already used", timer.diffSecondsReal());
      return;
    }
    ...
    if (parser.Text2List(localcmd1, localcmd) != 0) {
      generateLocal(true, "command cannot be parsed", timer.diffSecondsReal());
      return;
    }
    ...
    bool success;
    try {
      success = qp->ExecuteQuery(expression, qresult, typestring, errorstring,
...
template <class A, class C>
int dletVMT(Word *args, Word &result, int message, Word &local, Supplier s) {
  ...
  case OPEN: {
    ...
    TupleType *tt = new TupleType(nl->Second(GetTupleResultType(s)));
    local.addr = new dletInfo<A>((A *)args[0].addr, name->GetValue(),
                                 cmd->GetValue(), tt);
    tt->DeleteIfAllowed();
    return 0;
  }
...
ValueMapping dletVM[] = {dletVMT<DArray, CcString>,  dletVMT<DArray, FText>,
                         dletVMT<DFArray, CcString>, dletVMT<DFArray, FText>,
                         dletVMT<SDArray, CcString>, dletVMT<SDArray, FText>};
...
Operator dletOp("dlet", dletSpec.getStr(), 6, dletVM, dletSelect, dletTM);
...
int makeSimpleVM(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
...
OperatorSpec makeSimpleSpec(
    "darray x bool [ x string] -> sdarray", "_ makeSimple[_] ",
...
Operator makeSimpleOp("makeSimple", makeSimpleSpec.getStr(), makeSimpleVM,
                      Operator::SimpleSelect, makeSimpleTM);
...
public:
  makeDArraySlotRunner(SDArray *_sarray, DArray *_tarray, int _slot, bool _copy)
      : sarray(_sarray), tarray(_tarray), slot(_slot), copy(_copy), runner(0),
        success(false) {}
...
int makeDArrayVM(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
...
OperatorSpec makeDArraySpec(
    "sdarray x bool [x string] -> darray", "_ makeDArray[_,_] ",
...
Operator makeDArrayOp("makeDArray", makeDArraySpec.getStr(), makeDArrayVM,
                      Operator::SimpleSelect, makeDArrayTM);
...
template <class A> class ShortMaker {
public:
  ...
  void start() {
    ...
    vector<boost::thread *> runners;
    for (size_t i = 0;
         i < src->getSize() && usedWorkers.size() < res->getSize(); i++) {
      ...
      if (usedWorkers.find(elem) == usedWorkers.end()) {
        ...
        boost::thread *runner = new boost::thread(
            boost::bind(&ShortMaker<A>::run, this, i, resSlot, w, elem));
        resSlot++;
        runners.push_back(runner);
      }
    }
...
private:
  ...
  void run(int srcSlot, int resSlot, DArrayElement &elem, DArray *src,
           DArray *res) {
  ...
  void run(int srcSlot, int resSlot, DArrayElement &elem, DFArray *src,
           DFArray *res) {
    ConnectionInfo *ci = algInstance->getWorkerConnection(elem, dbname);
    if (!ci) {
      return;
...
template <class A>
int makeShortVMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
...
ValueMapping makeShortVM[] = {makeShortVMT<DArray>, makeShortVMT<DFArray>};
...
OperatorSpec makeShortSpec(
    "d[f]array x string x bool -> d[f]array ", "_ makeShort[_,_] ",
...
Operator makeShortOp("makeShort", makeShortSpec.getStr(), 2, makeShortVM,
                     makeShortSelect, makeShortTM);
...
int keepRemoteObjectsVM(Word *args, Word &result, int message, Word &local,
                        Supplier s) {
...
OperatorSpec keepRemoteObjectsSpec(
    "-> bool or bool -> bool", "keepRemoteObjects(_)",
...
Operator keepRemoteObjectsOp("keepRemoteObjects",
                             keepRemoteObjectsSpec.getStr(),
                             keepRemoteObjectsVM, Operator::SimpleSelect,
                             keepRemoteObjectsTM);
...
ListExpr createSDArrayTM(ListExpr args) {
  ...
  string resStr;
  ci->simpleCommand(query, errorCode, errmsg, resStr, false, rt, false,
                    commandLog);
  if (errorCode) {
...
template <class H, class C>
int createSDArrayVMT(Word *args, Word &result, int message, Word &local,
                     Supplier s) {
...
ValueMapping createSDArrayVM[] = {
    createSDArrayVMT<CcString, CcString>, createSDArrayVMT<CcString, FText>,
    createSDArrayVMT<FText, CcString>, createSDArrayVMT<FText, FText>};
...
OperatorSpec
    createSDArraySpec("string x rel -> sdarray(X)", "createSDArray(_,_)",
                      "Creates an SDarray from existing objects on workers",
                      "query makeSDArray(\"obj\",workers)");

Operator createSDArrayOp("createSDArray", createSDArraySpec.getStr(), 4,
                         createSDArrayVM, createSDArraySelect, createSDArrayTM);
...
ListExpr getFTM(ListExpr args) {
  ...
  return nl->ThreeElemList(
      nl->SymbolAtom(Symbols::APPEND()),
      nl->TwoElemList(nl->BoolAtom(isStream),
                      nl->TextAtom(nl->ToString(nl->Second(nl->Third(args))))),
      res);
}
...
ListExpr slotSizesTM(ListExpr args) {
  ...
  ListExpr attrList = nl->TwoElemList(
      nl->TwoElemList(nl->SymbolAtom("Slot"), listutils::basicSymbol<CcInt>()),
      nl->TwoElemList(nl->SymbolAtom("Size"), listutils::basicSymbol<CcInt>()));
...
public:
  ...
  ~slotSizesInfo() { tt->DeleteIfAllowed(); }
...
int slotSizesVM(Word *args, Word &result, int message, Word &local,
                Supplier s) {
  ...
  case OPEN:
    if (li)
      delete li;
    local.addr = new slotSizesInfo((DFMatrix *)args[0].addr,
                                   nl->Second(GetTupleResultType(s)));
    return 0;
  case REQUEST:
    result.addr = li ? li->next() : 0;
...
OperatorSpec slotSizesSpec(
    "dfmatrix -> stream(tuple(Slot int)(Size int))", " _ slotSizes ",
    "returns the number of tuples in each slot aggregates over all workers",
    " query dfm slotSizes consume");

Operator slotSizesOp("slotSizes", slotSizesSpec.getStr(), slotSizesVM,
                     Operator::SimpleSelect, slotSizesTM);
...
template <class T> class fileSizesInfo {
public:
...
template <class T>
int fileSizesVMT(Word *args, Word &result, int message, Word &local,
                 Supplier s) {
  ...
  case OPEN: {
    ...
    if (a1->IsDefined()) {
      local.addr = new fileSizesInfo<T>(args[0], a1->GetValue());
    }
  }
    return 0;
  case REQUEST:
...
ValueMapping fileSizesVM[] = {fileSizesVMT<CcString>, fileSizesVMT<FText>};
...
/* 
   ...
   Returns the number of such workers that are connected with a host.
...
Operator getWorkersForHostOp("getWorkersForHost", 
                 getWorkersForHostSpec.getStr(), 
...
extern "C" Algebra *InitializeDistributed2Algebra(NestedList *nlRef,
