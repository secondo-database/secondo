...
// No localtime_r with MSVC, but arguments are swapped for localtime_s
...
#define VTSEQ(ID) ("\x1b[" #ID "m")
	...
	void file_reopen(void* user_data)
	{
		...
		if(!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 || 
			...
			if(!file_abs->fp) {
				VLOG_F(g_internal_verbosity, "Reopening file '" LOGURU_FMT(s) "' due \
				to previous error", file_abs->path);
			}
			else if(ret < 0) {
				const auto why = errno_as_text();
				VLOG_F(g_internal_verbosity, "Reopening file '" LOGURU_FMT(s) "' due \
				to '" LOGURU_FMT(s) "'", file_abs->path, why.c_str());
			}
			else {
				VLOG_F(g_internal_verbosity, "Reopening file '" LOGURU_FMT(s) "' due \
				to file changed", file_abs->path);
			}
		...
		for(int arg_it = 1; arg_it < argc; ++arg_it) {
			...
			if(strncmp(cmd, verbosity_flag, arg_len) == 0 && 
				...
				if(value_str[0] == '\0') {
					...
					arg_it += 1;
					CHECK_LT_F(arg_it, argc, "Missing verbosiy level \
					after " LOGURU_FMT(s) "", verbosity_flag);
...
#define getcwd _getcwd
	...
	bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
	{
		...
		snprintf(file_abs->mode_str, sizeof(file_abs->mode_str) - 1, 
			"%s", mode_str);
		stat(file_abs->path, &file_abs->st);
		file_abs->fp = file;
		...
		VLOG_F(g_internal_verbosity, "Logging to '" LOGURU_FMT(s) "', mode: \
			'" LOGURU_FMT(s) "', verbosity: " LOGURU_FMT(d) "", 
		...
		flush_handler_t on_flush)
	{
		...
		s_callbacks.push_back(Callback{ id, callback, user_data, verbosity, 
			on_close, on_flush, 0 });
		on_callback_change();
	}
	...
	void set_thread_name(const char* name)
	{
		...
		strncpy_s(get_thread_name_win32(), LOGURU_THREADNAME_WIDTH + 1, 
			name, _TRUNCATE);
	...
	std::string prettify_stacktrace(const std::string & input)
	{
		...
		try {
			...
			output = std::regex_replace(output, template_spaces_re, 
				std::string("<$1>"));
		}
		...
		Verbosity verbosity, const char* file, unsigned line)
	{
		...
		auto uptime_ms = duration_cast<milliseconds>(steady_clock::now() - 
			s_start_time).count();
		...
		const char* prefix, const char* buff)
	{
		...
		auto message = Message{ verbosity, file, line, preamble_buff, "", 
			prefix, buff };
	...
	{
		auto formatted = fmt::vformat(format, args);
		auto message = Message{ verbosity, file, line, "", "", "", 
			formatted.c_str() };
	...
	LogScopeRAII::LogScopeRAII(Verbosity verbosity, const char* file, 
		...
		unsigned line, const char* format, fmt::format_args args)
	{
		...
		// log_to_everywhere already does this, but this makes the analyzer happy.
		abort(); 
	}
#else
...
#undef DEFINE_EC
		...
		Text ec_to_text(EcHandle ec_handle)
	{
		...
		char* with_newline = reinterpret_cast<char*>(malloc(strlen(
				parent_ec.c_str()) + 2));
		with_newline[0] = '\n';
		...
		if(s_unsafe_signal_handler) {
			...
			*/
			...
			try {
				...
				write_to_stderr("Exception caught and ignored by Loguru signal \
					handler.\n");
			}
			flush();
