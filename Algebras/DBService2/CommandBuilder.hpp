...
This struct combines an \textit{AttributeType2} with the name of the attribute.
...
given \textit{AttributeType2}.

*/
    static std::string getTypeName(AttributeType2 type);
...
In case the \textit{values} is empty a create statement without values will be issued.
