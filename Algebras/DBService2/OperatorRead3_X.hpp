...
*/
template <int X> struct Read3_XInfo : OperatorInfo {
  Read3_XInfo() {
    ...
    if (X == 0) {
      ...
      for (int i = 0; i < X; i++) {
        example +=
            " $" + stringutils::int2str(i + 1) + " exactmatch[1000] sort";
        if (i > 0)
          example += " mergesec ";
      }
      example += "] count";
...
template <int X> class OperatorRead3_X {
public:
  /*
...
template <int X> ListExpr OperatorRead3_X<X>::mapType(ListExpr args) {
  ...
  {
    ErrorReporter::ReportError("expected " + stringutils::int2str(2 + X) +
                               "  arguments");
    return nl->TypeError();
  }
  ...
  if (nl->Equal(nl->First(rel), nl->Second(rel))) {
    ...
    relType = OperatorCommon::getRelType(nl->OneElemList(nl->First(rel)),
                                         locallyAvailable);
  } else {
  ...
  types.push_back(
      std::make_pair(relType, make_pair(locallyAvailable, relName)));
  ...
  for (int i = 0; i < X; i++) {
    ...
    if (nl->AtomType(nl->Second(index)) != SymbolType) {
      return listutils::typeError("argument number " +
    ...
    types.push_back(
        std::make_pair(indexType, make_pair(locallyAvailable, indexName)));
  ...
  for (int i = 0; i < X + 1; i++) {
    ...
    if (!nl->Equal(farg, types[i].first)) {
      ...
      err << "type mismatch in function argument " << (i + 1)
          << " , expected is " << nl->ToString(farg) << " provided is "
          << nl->ToString(types[i].first);
  ...
  for (int i = 0; i < X + 1; i++) {
    ...
    last = nl->Append(last, nl->TwoElemList(nl->First(farg), types[i].first));
  }
  ...
  ListExpr appendList =
      nl->ThreeElemList(nl->BoolAtom(allAvailable),
                        nl->TextAtom(nl->ToString(newfunq)), objectNames);
  return nl->ThreeElemList(nl->SymbolAtom(Symbols::APPEND()), appendList, res);
}

template <int X>
int OperatorRead3_X<X>::mapValue(Word *args, Word &result, int message,
  ...
  if (locallyAvailable) {
    ...
    case OPEN: {
      ...
      fs::path fileName =
          DBServiceClient::getInstance()->retrieveReplicaAndGetFileName(
              databaseName, relationName, otherObjects, funText);
      if (fileName.empty()) {
