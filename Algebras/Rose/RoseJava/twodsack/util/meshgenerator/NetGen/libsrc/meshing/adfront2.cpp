                      ...
                      MultiPointGeomInfo * amgi)
{
  ...
  if (amgi)
    ...
    if (mgi->GetPGI(i).trignum <= 0)
      std::cout << "Add FrontPoint2, illegal geominfo = " 
              << mgi->GetPGI(i).trignum << std::endl;
    }
  else
    mgi = NULL;
}
 ...
 if (nfl > 0)
    ...
    if (lines.Get(i).Valid())
      ost << GetGlobalIndex (lines.Get(i).L().I1()) << "-"
          << GetGlobalIndex (lines.Get(i).L().I2()) << std::endl;
  ...
  */
  ...
  if (allflines)
                    ...
                    GetGlobalIndex (pi2))))
    {
      ...
      (*testout) << "ERROR Adfront2::AddLine: line exists" << std::endl;
    }
               ...
               double xh)
{
  ...
  */
  ...
  if (loclines.Size() == 1)
    {
      std::cout << "loclines.Size = 1" << std::endl;
      (*testout) << "loclines.size = 1" << std::endl
         << " h = " << xh << std::endl
...
void AdFront2 :: Print (std::ostream & ost) const
{
  ...
  for (i = 1; i <= points.Size(); i++)
    if (points.Get(i).Valid())
      ost << i << "  " << points.Get(i).P() << std::endl;

  ost << nfl << " Lines: " << std::endl;
  for (i = 1; i <= lines.Size(); i++)
    if (lines.Get(i).Valid())
      ost << lines.Get(i).L().I1() << " - " << lines.Get(i).L().I2() 
          << std::endl;
