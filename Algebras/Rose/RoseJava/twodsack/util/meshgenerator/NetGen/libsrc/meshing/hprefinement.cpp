...
#include "hpref_prism.hpp"
  ...
  // Elements (volume, surface, edges)  are classified by 
  // singular vertices, edges, faces
  ...
  void PrepareElements (Mesh & mesh, ARRAY<HPRefElement> & elements)
  {
    ...
    */
    ...
    for (int i = 1; i <= mesh.GetNP(); i++)
      {
        // mark points for refinement that are in corners 
        // between two anisotropic edges 
        if (surfonpoint.Get(i).I1())
          {
        cornerpoint.Set(i);
        edgepoint.Set(i);
    ...
    switch (el.GetType())
      ...
      case PRISM:
          ...
          for (int j = 1; j <= 3; j++)
        {
          ...
          if (type != HP_NONE)
            {
              cout << "classified element, type = " << type 
                   << ", el = "  << el << endl;
              pnums[0] = el.PNum (pi1);
              pnums[1] = el.PNum (pi2);
              pnums[2] = el.PNum (pi3);
    ...
    for (int i = 0; i < misses.Size(); i++)
      if (misses[i])
    cout << "missing case " << i << " occured " << misses[i] 
         << " times" << endl;
      ...
      case TRIG:
          ...
          for (j = 1; j <= 3; j++)
        {
          ...
          if (mesh.GetDimension() == 2)
            {
              ep1 = edgepoint_dom.Used (INDEX_2
                 (mesh.SurfaceElement(i).GetIndex(), el.PNumMod(j)));
              ep2 = edgepoint_dom.Used (INDEX_2
                  (mesh.SurfaceElement(i).GetIndex(), el.PNumMod(j+1)));
              ep3 = edgepoint_dom.Used (INDEX_2
                  (mesh.SurfaceElement(i).GetIndex(), el.PNumMod(j+2)));
            }
          ...
          if (mesh.GetDimension() == 2)
            {
              ...
              ep3 = edgepoint_dom.Used (INDEX_2
                       (mesh.SurfaceElement(i).GetIndex(), el.PNumMod(j+2)));
              ep4 = edgepoint_dom.Used (INDEX_2
                        (mesh.SurfaceElement(i).GetIndex(), el.PNumMod(j+3)));
            }
    ...
    (*testout) << "param = " << elements.Last().param[0][0] 
               << ", " << elements.Last().param[1][0] << endl;
      }
  }
  ...
  void CalcStatistics (ARRAY<HPRefElement> & elements)
  {
    ...
    cout << "costs for p = " << p << ": mem = " << memcost 
         << ", cpu = " << cpucost << endl;
      }
    ...
    switch (el.type)
      ...
      case HP_TET_1E_1VA:
        {
          ...
          (*testout) << "p1 = " << p1 << ", P1^6 = " 
                     << pow (static_cast<double>(p1), 6.0)
            << " (p1-3)^6 = " << pow ( static_cast<double>(max2(p1-3, 0)), 6.0) 
             << " p1^3 = " << pow ( static_cast<double>(p1), 3.0) 
             << " (p1-3)^3 = " << pow ( static_cast<double>(p1-3), 3.0) 
             << " [p1^3-(p1-3)^3]^2 = " 
             << sqr (pow (static_cast<double>(p1),3.0) 
                      - pow ( static_cast<double>(p1-3), 3.0))
             << endl;

          p1 /= 2 +1;
          memcosttet += pow (static_cast<double>(p1), 6.0);
          memcostsctet += pow (static_cast<double>(p1), 6.0) 
                          - pow ( static_cast<double>(max2(p1-3, 1)), 6.0);
      ...
      case HP_PRISM_SINGEDGE:
        {
          ...
          memcostscprism += pow (static_cast<double>(p1), 6.0) 
                             - pow ( static_cast<double>(max2(p1-3, 1)), 6.0);
      ...
      case HP_HEX:
        {    
          ...
          p1 /= 2 +1;
          p2 /= 2 +1;
          memcosthex += pow (static_cast<double>(p1), 4.0)
                        * pow (static_cast<double>(p2), 2.0);
          memcostschex += pow (static_cast<double>(p1), 6.0) 
                          - pow ( static_cast<double>(max2(p1-2, 0)), 6.0);
          cpucosthex += pow (static_cast<double>(p1), 6.0) 
                        * pow (static_cast<double>(p2), 3.0);
          break;
        }
      default:
        ;
  ...
  void HPRefinement (Mesh & mesh, Refinement * ref, int levels)
  {
    ...
    *mesh.coarsemesh = mesh;
    ...
    // last parameter: true for fine subdivision (7/8 : 1/8), 
    // false for 3/4 : 1/4
