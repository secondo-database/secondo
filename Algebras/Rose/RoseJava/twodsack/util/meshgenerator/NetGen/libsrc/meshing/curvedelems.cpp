...
// ----------------------------------------------------------------------------
    ...
    if (locmaxfaceorder > maxfaceorder)
    {
        ...
        fshape.SetSize( nfaces * (maxfaceorder-1) * (maxfaceorder-1));
      // number of face shape functions
   ...
   // (*testout) << "eshape = " << eshape << ", edshape = " << edshape << endl;
    ...
    for (int n1 = 2; n1 < faceorder; n1++)
    {
        ...
        fdshape[index] = Vec<2>
         ...
         b1.GetF(n0) * b2.GetDf(n1) * (dl2x * arg 
         - l2 * dargx)/(arg*arg) * decay
         + b1.GetF(n0) * b2.GetF(n1) * ddecay * dargx,
          b1.GetDf(n0) * dl0y * b2.GetF(n1) * decay +
          b1.GetF(n0) * b2.GetDf(n1) * (dl2y * arg 
         - l2 * dargy)/(arg*arg) * decay
    ...
    for (int n1 = 2; n1 < faceorder; n1++)
    {
        ...
        //  b1.GetDf(n0) * dl0x * b2.GetF(n1) * decay .... derived
            ...
            b1.GetDf(n0) * dl0x * b2.GetDf(n1) * (dl2x * arg 
       - l2 * dargx)/(arg*arg) * decay +
            b1.GetDf(n0) * dl0x * b2.GetF(n1) * ddecay * dargx +

            
            //  b1.GetF(n0) * b2.GetDf(n1) 
          //* (dl2x * arg - l2 * dargx)/(arg*arg) * decay ... derived

            b1.GetDf(n0) * dl0x * b2.GetDf(n1) * (dl2x * arg 
       - l2 * dargx)/(arg*arg) * decay +
            b1.GetF(n0) * b2.GetDDf(n1) 
        * pow((dl2x * arg - l2 * dargx)/(arg*arg),2) * decay +
            b1.GetF(n0) * b2.GetDf(n1) * (-2*dargx/arg) 
        * (dl2x * arg - l2 * dargx)/(arg*arg) * decay +
            b1.GetF(n0) * b2.GetDf(n1) 
        * (dl2x * arg - l2 * dargx)/(arg*arg) * ddecay * dargx +
        ...
        * (dl2x * arg - l2 * dargx)/(arg*arg) * ddecay * dargx +
        ...
        //  b1.GetDf(n0) * dl0y * b2.GetF(n1) * decay ... derived
            ...
            b1.GetDf(n0) * dl0y * b2.GetDf(n1) 
        * (dl2y * arg - l2 * dargy)/(arg*arg) * decay +
            b1.GetDf(n0) * dl0y * b2.GetF(n1) * ddecay * dargy +
            

        //  b1.GetF(n0) * b2.GetDf(n1) 
    // * (dl2y * arg - l2 * dargy)/(arg*arg) * decay ... derived

            b1.GetDf(n0) * dl0y * b2.GetDf(n1) 
        * (dl2y * arg - l2 * dargy)/(arg*arg) * decay +
            b1.GetF(n0) * b2.GetDDf(n1) * pow((dl2y * arg 
        - l2 * dargy)/(arg*arg),2) * decay +
            b1.GetF(n0) * b2.GetDf(n1) * (-2*dargy/arg) 
       * (dl2y * arg - l2 * dargy)/(arg*arg) * decay +
            b1.GetF(n0) * b2.GetDf(n1) 
        * (dl2y * arg - l2 * dargy)/(arg*arg) * ddecay * dargy +
...
*/
    ...
    for (int n0 = 2; n0 <= faceorder; n0++)
        ...
        fdshape[index] = Vec<2> (b1.GetDf(n0) * dxx * b2.GetF(n1)
              + b1.GetF(n0) * b2.GetDf(n1) * dyx,
    ...
    for (int f = 0; f < nfaces; f++)
    {
      if (faceorder[f] <= 2) continue;
        ...
        for (int n0 = 2; n0 < faceorder[f]-n1+2; n0++)
        {
            ...
            fdshape[index] = Vec<3>
            (b1.GetDf(n0) * (dl0x * arg2 - l0 * darg2x)/(arg2*arg2) 
       * b2.GetF(n1) * decay * decay2 +
             b1.GetF(n0) * b2.GetDf(n1) * (dl2x * arg 
       - l2 * dargx)/(arg*arg) * decay * decay2 +
             b1.GetF(n0) * b2.GetF(n1) * ddecay * dargx * decay2 +
             b1.GetF(n0) * b2.GetF(n1) * decay * ddecay2 * darg2x,
            
             b1.GetDf(n0) * (dl0y * arg2 - l0 * darg2y)/(arg2*arg2) 
       * b2.GetF(n1) * decay * decay2 +
             b1.GetF(n0) * b2.GetDf(n1) * (dl2y * arg 
       - l2 * dargy)/(arg*arg) * decay * decay2 +
             b1.GetF(n0) * b2.GetF(n1) * ddecay * dargy * decay2 +
             b1.GetF(n0) * b2.GetF(n1) * decay * ddecay2 * darg2y,

             b1.GetDf(n0) * (dl0z * arg2 - l0 * darg2z)/(arg2*arg2) 
       * b2.GetF(n1) * decay * decay2 +
             b1.GetF(n0) * b2.GetDf(n1) * (dl2z * arg 
       - l2 * dargz)/(arg*arg) * decay * decay2 +
...
// ----------------------------------------------------------------------------
    ...
    vdshape[0] = Vec<3>( -(z1-y)/z1, -(z1-x)/z1, 
                       ((x+y+2*z-2)*z1+(z1-y)*(z1-x))/z2 );
...
// ----------------------------------------------------------------------------
  ...
  int CurvedElements :: IsSurfaceElementCurved (int elnr) const
  {
    ...
    return mesh.coarsemesh->GetCurvedElements().IsSurfaceElementCurved
        (hpref_el.coarse_elnr);
      }
  ...
  int CurvedElements :: IsElementCurved (int elnr) const
  {
    ...
    return mesh.coarsemesh->GetCurvedElements().IsElementCurved
              (hpref_el.coarse_elnr);
      }
...
//    segm.CalcVertexShapes (x != NULL, dxdxi != NULL);
    ...
    if (dxdxi)
    {
        (*dxdxi) = Vec<3>(0,0,0);
        for (int v = 0; v < 2; v++)
        (*dxdxi) = (*dxdxi) + segm.GetVertexDShape(v)  
                  * mesh.Point(segm.GetVertexNr(v));
    }
...
//        segm.CalcEdgeShapes (x != NULL, dxdxi != NULL);
      ...
      if (mesh.coarsemesh)
    {
      ...
      mesh.coarsemesh->GetCurvedElements().CalcSurfaceTransformation(
    ...
    */
    ...
    if (x)
    {
        (*x) = Point<3>(0,0,0);
        for (int v = 0; v < fe2d->GetNVertices(); v++)
        (*x) = (*x) + fe2d->GetVertexShape(v) 
                * mesh.Point(fe2d->GetVertexNr(v));
    }
    ...
    if (dxdxi)
    {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 2; j++)
            (*dxdxi)(i,j) += fe2d->GetVertexDShape(v)(j) 
             * mesh.Point(fe2d->GetVertexNr(v)).X(i+1);
    }
...
//        fe2d->CalcEdgeShapes (x != NULL, dxdxi != NULL);
        ...
        for (int e = 0; e < fe2d->GetNEdges(); e++)
        {
            ...
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 2; j++)
                (*dxdxi)(i,j) += fe2d->GetEdgeDShape(index)(j) 
                                 * edgecoeffs[gindex](i);
        }
        }
...
//        fe2d->CalcFaceShapes (x != NULL, dxdxi != NULL);
        ...
        if (dxdxi)
        {
            ...
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 2; j++)
                (*dxdxi)(i,j) += fe2d->GetFaceDShape(index)(j) 
                                 * facecoeffs[gindex](i);
        }
        }
    } 
     ...
     if (mesh.coarsemesh)
    {
      ...
      mesh.coarsemesh->GetCurvedElements().CalcElementTransformation (
              coarse_xi, hpref_el.coarse_elnr, x, &dxdxic);
...
//    fe3d->CalcVertexShapes (x != NULL, dxdxi != NULL);
    ...
    if (x)
    {
        ...
        (*x) += fe3d->GetVertexShape(v) 
                * Vec<3> (mesh.Point(fe3d->GetVertexNr(v)));
    }
    ...
    if (dxdxi)
    {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
            (*dxdxi)(i,j) += fe3d->GetVertexDShape(v)(j) 
            * mesh.Point(fe3d->GetVertexNr(v)).X(i+1);
    }
...
//        fe3d->CalcEdgeShapes (x != NULL, dxdxi != NULL);
        ...
        for (int e = 0; e < fe3d->GetNEdges(); e++)
        {
            ...
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                (*dxdxi)(i,j) += fe3d->GetEdgeDShape(index)(j) 
                                 * edgecoeffs[gindex](i);
        }
        }
...
//        fe3d->CalcFaceShapes (x != NULL, dxdxi != NULL);
        ...
        if (dxdxi)
        {
            ...
            for (int k = 0; k < fe3d->GetNFaceShapes(f); k++, index++, gindex++)
                for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                    (*dxdxi)(i,j) += fe3d->GetFaceDShape(index)(j) 
                                     * facecoeffs[gindex](i);
            }
        }
        } 
