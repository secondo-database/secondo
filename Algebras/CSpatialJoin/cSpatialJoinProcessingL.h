      ...
      while(!eq.empty()) {
        ...
        if(tempEvent.in_out == 1) {
          ...
          if(tempEvent.stream == 1) {
            if(!resume) {
              fSweepStruct.push_back(tempEvent.bt);             
            }
            ...
            if(!resume) {
              sSweepStruct.push_back(tempEvent.bt);
            }
            // search for intersections
...
bool sweepSearch(std::vector<binaryTuple> &sweepStruct,
    ...
    for(uint64_t iter = resumeIter; iter < sweepStruct.size(); iter++) {
          
      tempTuple = sweepStruct[iter];
