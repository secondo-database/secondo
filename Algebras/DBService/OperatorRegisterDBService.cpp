...
ListExpr OperatorRegisterDBService::mapType(ListExpr nestedList)
{
    print(nestedList);
    ...
    if (!nl->HasLength(nestedList, 2))
    {
        ErrorReporter::ReportError(
                "expected signature: host x port");
        return nl->TypeError();
    }
...
int OperatorRegisterDBService::mapValue(Word* args,
                    Word& result,
                    int message,
                    Word& local,
                    Supplier s)
