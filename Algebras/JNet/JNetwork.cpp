...
{
  //cout << "Compute and add units" << endl;
  JRouteInterval actInt;
    ...
    if (actInt.GetLength() != 0.0)
    {
      ...
      if (unitstart < unitend)
      {
        JUnit actUnit(JUnit(Interval<Instant>(unitstart, unitend,
                                              unitlc, unitrc),
                            actInt));
        //cout << "add act Unit: " << actUnit << endl;
        result->Add(actUnit);
        unitstart = unitend;
        unitlc = !unitrc;
        unitrc = !unitlc;
...
void JNetwork::ShortestPathTree(const jnetwork::JPoint* source,
                                DbArray< std::pair< int, double > >* result,
                                const double targetDist)
{
  if (result != NULL)
  {
    ...
    if (source != NULL && source->IsDefined())
    {
      ...
      InitPriorityQueue(pqueue, source, startJID1, startJID2, dist1, dist2);
      ProcessPriorityQueue(pqueue, result, targetDist, startJID1, startJID2,
                           dist1, dist2);
                            ...
                            JPath* result)
{
      ...
      sources->Size() > 0 && targets->Size() > 0)
  {
    ...
    {
      //cout << "Init Set of EndPoints" << endl;
      ...
      //cout << "Init Set of StartPoints" << endl;
      ...
      if (sp != NULL)
      {
        //cout << "write path to result" << endl;
      ...
      sources->Size() > 0 && targets->Size() > 0)
  {
    if (!JNetUtil::ArrayContainIntersections(*sources, *targets))
    {
      ...
      else
      {
        if (sp != NULL)
        {
          //cout << "write length of path to result" << endl;
          result->SetDefined(true);
...
{
  //cout << "Write Shortest Path: sources " << sources
  //     << ", targets" << targets
  //     << ", scrPathJID:" << srcStartPathJID
  //     << ", tgtPosInArray: " << tgtPosInArray
  //     << ", tgtOverStartJunc: " << boolalpha << tgtOverStartJunction
  //     << endl;
      ...
      targets->Size() > tgtPosInArray)
  {
    ...
    //cout << "got startPosJnet: " << start << endl;
    if (start.GetSectionId() > -1)
    {
      //cout << "valid section id" << endl;
      sectTup = GetSectionTupleWithId(start.GetSectionId());
      if (sectTup != NULL)
      {
        //cout << "got sectTup" << endl;
        curRint = GetSectionFirstRouteInterval(sectTup);
        if (curRint != NULL)
        {
          //cout << "got routeTup" << endl;
          if (srcOverStartJunc)
          {
            //cout << "overStartJunc" << endl;
          ...
          if (tmp != NULL)
          {
            ...
            {
              //cout << "add: startinterval: " << *tmp << endl;
              result->Append(*tmp);
            }
            else
            {
              //cout << "not move in interval" << endl;
            }
            tmp->DeleteIfAllowed();
            tmp = 0;
          }
    ...
    {
      //cout << "valid end section" << endl;
      sectTup = GetSectionTupleWithId(end.GetSectionId());
      if (sectTup != NULL)
      {
        //cout << "got sectionTuple" << endl;
        curRint = GetSectionFirstRouteInterval(sectTup);
        if (curRint != NULL)
        {
          //cout << "got rint" << endl;
...
void JNetwork:ectSpatialPositions(const DbArray<RouteLocation>* targets,
                                 ...
                                 int& startJID2, double& dist2) const
{
  ...
  Tuple* sectTup = GetSectionTupleFor(source->GetLocation(), distFromStart);
  if (sectTup != NULL)
  {
    if (AlmostEqualAbsolute(distFromStart, 0.0, tolerance) ||
                                     ...
                                     tolerance))
    {
      if (AlmostEqualAbsolute(distFromStart, 0.0, tolerance))
      {
        startJID1 = GetSectionStartJunctionID(sectTup);
        dist1 = 0.0;
      }
      else
      {
        startJID1 = GetSectionEndJunctionID(sectTup);
        dist1 = 0.0;
      }
      Tuple* juncTup = GetJunctionTupleWithId(startJID1);
      if (juncTup != NULL)
      {
        JListInt* adjSect = GetJunctionOutSectionList(juncTup);
        if (adjSect != NULL)
        {
          JPQEntry tmp(source->GetLocation().GetSide(), -1, startJID1,
                       startJID1, -1, startJID1, startJID1, dist1, dist1);
          AddAdjacentSections<Point>(pqueue, adjSect, tmp, 0);
          adjSect->Destroy();
    ...
    else
    {
      ...
      if (test != 0)
      {
        startJID2 = GetSectionEndJunctionID(sectTup);
        dist2 = GetSectionLength(sectTup)- distFromStart;
        juncTup = GetJunctionTupleWithId(startJID2);
        if (juncTup != 0)
        {
          adjSect = GetSectionListAdjSectionsUp(sectTup);
          if (adjSect != NULL)
          {
            JPQEntry tmp2((Direction) Up, -1, startJID2, startJID2,
                        -1, startJID2, startJID2,
                        dist2, dist2);
            AddAdjacentSections<Point>(pqueue, adjSect, tmp, 0);
            adjSect->Destroy();
            adjSect->DeleteIfAllowed();
      ...
      if (test >= 0)
      {
        startJID1 = GetSectionStartJunctionID(sectTup);
        dist1 = distFromStart;
        juncTup = GetJunctionTupleWithId(startJID1);
        if (juncTup != NULL)
        {
          adjSect = GetSectionListAdjSectionsDown(sectTup);
          if (adjSect != NULL)
          {
            JPQEntry tmp1((Direction) Down, -1, startJID1, startJID1,
                        -1, startJID1,startJID1, dist1, dist1);
            AddAdjacentSections<Point>(pqueue, adjSect, tmp, 0);
          }
          adjSect->Destroy();
          adjSect->DeleteIfAllowed();
                                    ...
                                    const double dist1, const double dist2)
{
  ...
  {
    curPQElement = pqueue->GetAndDeleteMin();
     if (targetDist >= curPQElement->GetPriority())
    {
      InsertNetdistanceTuple(curPQElement->GetStartPathJID(), curPQElement,
                             wayEntries);
      double distLastJuncEndPoint = -1.0;
...
{
  //cout << "ExistsNetDistanceFor " << start << ", to " << end << endl;
  bool result = false;
  bool tmp = false;
