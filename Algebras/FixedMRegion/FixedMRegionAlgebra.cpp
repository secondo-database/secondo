...
class FixedMRegionPoint  : public Attribute{
  public :
    ...
    ~FixedMRegionPoint(){}
    static const string BasicType() { return "pstpoint";}
...
return 1;
  ...
  ListExpr FixedMRegionPointProperty() {
    ...
    nl->FourElemList (
      nl->StringAtom("-> DATA"),
      nl->StringAtom(FixedMRegionPoint::BasicType()),
      nl->StringAtom("(real real) = (x,y)"),
      nl->StringAtom("(13.5 -76.0)")
    ));
    }
  

Word InFixedMRegionPoint( const ListExpr typeInfo, 
...
ListExpr OutFixedMRegionPoint( ListExpr typeInfo, Word value ) {
...
Word CreateFixedMRegionPoint( const ListExpr typeInfo ) {
  Word w;
  w.addr = (new FixedMRegionPoint(0,0));
  return w;
}

void DeleteFixedMRegionPoint( const ListExpr typeInfo, Word& w ) {
  FixedMRegionPoint *k = (FixedMRegionPoint *)w.addr;
  delete k;
  w.addr = 0;
}

bool SaveFixedMRegionPoint(  SmiRecord& valueRecord, size_t& offset,
...
bool OpenFixedMRegionPoint( SmiRecord& valueRecord,
...
void CloseFixedMRegionPoint( const ListExpr typeInfo, Word& w ) {
  FixedMRegionPoint *k = (FixedMRegionPoint *)w.addr;
  delete k;
  w.addr = 0;
}

Word CloneFixedMRegionPoint( const ListExpr typeInfo, const Word& w ){
  FixedMRegionPoint* k = (FixedMRegionPoint*) w.addr;
  Word res;
...
void* CastFixedMRegionPoint( void* addr ) {
  return (new (addr) FixedMRegionPoint);
}

int SizeOfFixedMRegionPoint() {
  return 2*sizeof(double);
}

bool FixedMRegionPointTypeCheck(ListExpr type, ListExpr& errorInfo){
  return nl->IsEqual(type, FixedMRegionPoint::BasicType());
}



TypeConstructor FixedMRegionPointTC(
FixedMRegionPoint::BasicType(),
FixedMRegionPointProperty,
OutFixedMRegionPoint, InFixedMRegionPoint,
0, 0, //deprecated, don’t think about it
CreateFixedMRegionPoint, DeleteFixedMRegionPoint,
OpenFixedMRegionPoint, SaveFixedMRegionPoint,
CloseFixedMRegionPoint, CloneFixedMRegionPoint,
...
OperatorSpec testoperatoraSpec(
"int -> int",
"testoperatora(_)",
...
extern "C"
Algebra*
InitializeFixedMRegionAlgebra( NestedList* nlRef,
