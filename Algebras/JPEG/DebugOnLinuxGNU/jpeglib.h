 ...
 *
 * uli: far pointers: 'machines' ??? or operating systems?
 * Lane wrote 1991-1998 ... so this code seems to be of 1991 rather.
 * should we indeed also write our Secondo-Extension portable to DOS ???
 * I guess even the secondo-start-procedure would hang up cause of un-
 * sufficient memory.
 */

typedef JSAMPLE FAR *JSAMPROW;  /* ptr to one image 'row' of pixel samples. 'pixel[ numColourCmps]'?  */
  // example.c:   JSAMPARRAY buffer;		/* Output row buffer */
typedef JSAMPROW *JSAMPARRAY; /* ptr to some rows (a 2-D sample array) */
typedef JSAMPARRAY *JSAMPIMAGE; /* a 3-D sample array: top index is color */
                                /* uli: 'top' ???  index to leaves or (single) root-index?
                                   I suppose its the leaves-index, cf. 'row' in JSAMPROW-typedef */
