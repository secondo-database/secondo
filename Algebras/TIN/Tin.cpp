...
"MemoryState RANDOMACCES|GRADUAL(TEMPORARY)F"
...
|| conf.memoryState == GRADUALTEMPORARYFILE)
throw std::runtime_error(
"MemoryState RANDOMACCES|GRADUAL(TEMPORARY)FILE"
...
throw std::runtime_error(
"Switching to temporary file is not possible,"
...
break;
case GRADUALTEMPORARYFILE:
throw std::runtime_error("This is not reasonable.(Tin::setMemoryState)");
break;
case INMEMORY:
openParts(true);
break;
...
if (!stlfile.is_open())
throw std::runtime_error("Could not open|create stl file.(Tin::tin2stl)");
...
|| config.memoryState == GRADUALTEMPORARYFILE)
throw std::runtime_error(
"Tried to save a tin in state stay"
...
TinAttribute * at = static_cast<TinAttribute*>(Attribute::Open(valueRecord,
offset, typeInfo));
...
last = (*it)->outPart();
(*it)->unloadContentData();
ret = nl->OneElemList(nl->OneElemList(nl->IntAtom(t->config.maxSizePart)));
...
if (intin->estimateMaxSizeInMemory(noPartsl) > OPERATE_IN_MEMORY_THRESHOLD)
...
if (!tt || !tt->isDefined())
throw std::runtime_error(
"Null pointer or undefined as input."
...
return NList::typeError(
"Expected type tin as first argument and a "
...
return NList::typeError(
"Expected second argument of type sint"
...
|| nl->SymbolValue(nl->Second(nl->First(attrList))) != ::Point::BasicType()
...
nl->TwoElemList(nl->SymbolAtom(Tuple::BasicType()), tuplelist.listExpr()));
}
/*
pdcheck
