...
bool bulkload, const TinConfiguration& conf = TinConfiguration::DEFAULT);
static TinPart* getInstanceFromDiscRandomAccess(Tin* tt,
SmiRecord& valueRecord, const uint32_t idx, const TinConfiguration& conf =
TinConfiguration::DEFAULT);
...
- (TinFeatures::getSizeOnDisc() + sizeof(int))) / Triangle::getSizeOnDisc());
}
...
LOGP
TIN_SIZE size = sizeof(TinPart::noTriangles) + sizeof(TinPart::noVertices)
