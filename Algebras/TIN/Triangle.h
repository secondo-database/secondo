...
static bool heightWalk(Triangle::triangleWalker * walker, Triangle* next);
static bool stopAtEdgeWalk(Triangle::triangleWalker * walker,
Triangle* next);

triangleWalker(Triangle * istart, Point_p * iarPoints, TIN_SIZE inoPoints,
bool istayInSamePart = false, bool inoload = false,
bool (*iuserExit)(Triangle::triangleWalker * walker, Triangle* next) = 0,
const void * istopParameter1 = 0) {

if (inoPoints < 1)
throw std::runtime_error(
"No points for construction is"
" not allowed.(triangleWalker::triangleWalker)");

if (!istart)
throw std::runtime_error(
"No start triangle for constru"
...
return (t != (Triangle *) END_OF_ACCESS && t != (Triangle *) END_OF_SEGMENT
&& t != (Triangle *) END_OF_WALK);
}
