...
// Extra spaces after format specifiers are only for readability purposes in log files
            ...
            static const base::type::char_t *kVerboseLevelLogValue =
                    ELPP_LITERAL(
                            "VERBOSE"); // will become VERBOSE-x where x = verbose level
            ...
            static const base::type::char_t *kDebugLevelShortLogValue = ELPP_LITERAL(
                    "D");
            static const base::type::char_t *kWarningLevelShortLogValue = ELPP_LITERAL(
                    "W");
            ...
            static const base::type::char_t *kFatalLevelShortLogValue = ELPP_LITERAL(
                    "F");
            static const base::type::char_t *kVerboseLevelShortLogValue = ELPP_LITERAL(
                    "V");
...
// Format specifiers - These are used to define log format
            ...
            static const base::type::char_t *kThreadIdFormatSpecifier = ELPP_LITERAL(
                    "%thread");
            static const base::type::char_t *kSeverityLevelFormatSpecifier = ELPP_LITERAL(
                    "%level");
            static const base::type::char_t *kSeverityLevelShortFormatSpecifier = ELPP_LITERAL(
                    "%levshort");
            ...
            static const base::type::char_t *kLogFileFormatSpecifier = ELPP_LITERAL(
                    "%file");
            static const base::type::char_t *kLogFileBaseFormatSpecifier = ELPP_LITERAL(
                    "%fbase");
            static const base::type::char_t *kLogLineFormatSpecifier = ELPP_LITERAL(
                    "%line");
            static const base::type::char_t *kLogLocationFormatSpecifier = ELPP_LITERAL(
                    "%loc");
            static const base::type::char_t *kLogFunctionFormatSpecifier = ELPP_LITERAL(
                    "%func");
            static const base::type::char_t *kCurrentUserFormatSpecifier = ELPP_LITERAL(
                    "%user");
            static const base::type::char_t *kCurrentHostFormatSpecifier = ELPP_LITERAL(
                    "%host");
...
// Date/time
            static const char *kDays[7] = {"Sunday", "Monday", "Tuesday",
                                           "Wednesday", "Thursday", "Friday",
                                           "Saturday"};
            static const char *kDaysAbbrev[7] = {"Sun", "Mon", "Tue", "Wed",
                                                 "Thu", "Fri", "Sat"};
            static const char *kMonths[12] = {"January", "February", "March",
                                              "Apri", "May", "June", "July",
                                              "August",
                                              "September", "October",
                                              "November", "December"
...
// ConfigurationTypeHelper
    ...
    void
    ConfigurationTypeHelper::forEachConfigType(base::type::EnumType *startIndex,
  ...
  return RegistryWithPred::get(level, configurationType) != nullptr;
    ...
    void Configurations::setToDefault(void) {
        ...
        setGlobally(ConfigurationType::ToStandardOutput, std::string("true"),
                    true);
        setGlobally(ConfigurationType::SubsecondPrecision, std::string("3"),
                    true);
    ...
    bool
    Configurations::Parser::parseFromFile(const std::string &configurationFile,
    ...
    bool Configurations::Parser::isLevel(const std::string &line) {
        return base::utils::Str::startsWith(line, std::string(
                base::consts::kConfigurationLevel));
    }

    bool Configurations::Parser::isComment(const std::string &line) {
        return base::utils::Str::startsWith(line, std::string(
                base::consts::kConfigurationComment));
    }
    ...
    void Configurations::unsafeSetIfNotExist(Level level,
                                             ConfigurationType configurationType,
                                             const std::string &value) {
        Configuration *conf = RegistryWithPred<Configuration, Configuration::Predicate>::get(
    ...
    void
    Configurations::unsafeSet(Level level, ConfigurationType configurationType,
                              const std::string &value) {
        Configuration *conf = RegistryWithPred<Configuration, Configuration::Predicate>::get(
                level, configurationType);
        if (conf == nullptr) {
    ...
    void Configurations::unsafeSetGlobally(ConfigurationType configurationType,
        ...
        base::type::EnumType lIndex = LevelHelper::kMinValid;
        LevelHelper::forEachLevel(&lIndex, [&](void) -> bool {
            unsafeSet(LevelHelper::castFromInt(lIndex), configurationType,
                      value);
            return false;  // Do not break lambda function yet as we need to set all levels regardless
        });
    }
...
// Logger
    ...
    void Logger::configure(const Configurations &configurations) {
        ...
        if (m_typedConfigurations != nullptr) {
            ...
            if (c->hasConfiguration(Level::Global,
    ...
    bool Logger::isValidId(const std::string &id) {
        for (std::string::const_iterator it = id.begin();
             it != id.end(); ++it) {
    ...
    void Logger::flush(Level level, base::type::fstream_t *fs) {
        ...
        if (fs != nullptr) {
            fs->flush();
            std::unordered_map<Level, unsigned int>::iterator iter = m_unflushedCount.find(
                    level);
    ...
    void Logger::resolveLoggerFormatSpec(void) const {
        ...
        LevelHelper::forEachLevel(&lIndex, [&](void) -> bool {
            ...
            base::utils::Str::replaceFirstWithEscape(logFormat->m_format,
                                                     base::consts::kLoggerIdFormatSpecifier,
                                                     m_id);
            return false;
        });
    }
...
#  endif // ELPP_OS_WINDOWS
  ...
  return considerFile ? true : ((fileType & FILE_ATTRIBUTE_DIRECTORY) == 0 ? false : true);
            ...
            bool File::createPath(const std::string &path) {
                ...
                currPath = STRTOK(currPath, base::consts::kFilePathSeperator,
                                  0);
#elif ELPP_OS_WINDOWS
  ...
  ELPP_UNUSED(nextTok_);
                ...
                while (currPath != nullptr) {
                    ...
                    currPath = STRTOK(nullptr, base::consts::kFilePathSeperator,
                                      0);
#elif ELPP_OS_WINDOWS
    ...
    currPath = STRTOK(nullptr, base::consts::kFilePathSeperator, &nextTok_);
            ...
            std::string
            File::extractPathFromFilename(const std::string &fullPath,
            ...
            void
            File::buildBaseFilename(const std::string &fullPath, char buff[],
...
// Str
            ...
            std::string &
            Str::replaceAll(std::string &str, const std::string &replaceWhat,
    ...
    tv->tv_usec = static_cast<long>(present % usecOffSet);
            ...
            DateTime::timevalToString(struct timeval tval, const char *format,
                ...
                parseFormat(buff_, kBuffSize, format, &timeInfo,
                            static_cast<std::size_t>(tval.tv_usec /
                                                     ssPrec->m_offset),
                            ssPrec);
            ...
            DateTime::getTimeDifference(const struct timeval &endTime,
                ...
                return static_cast<unsigned long long>(conv(endTime) -
                                                       conv(startTime));
            }
...
#  endif  // ELPP_COMPILER_MSVC
            ...
            char *DateTime::parseFormat(char *buf, std::size_t bufSz,
                                        const char *format,
                        ...
                        switch (*++format) {
                            ...
                            case 'a':  // Day of week (short)
                                buf = base::utils::Str::addToBuff(
                                        base::consts::kDaysAbbrev[tInfo->tm_wday],
                                        buf, bufLim);
                                continue;
                            case 'A':  // Day of week (long)
                                buf = base::utils::Str::addToBuff(
                                        base::consts::kDays[tInfo->tm_wday],
                                        buf, bufLim);
                                continue;
                            case 'M':  // month
                                buf = base::utils::Str::convertAndAddToBuff(
                            ...
                            case 'b':  // month (short)
                                buf = base::utils::Str::addToBuff(
                                        base::consts::kMonthsAbbrev[tInfo->tm_mon],
                                        buf, bufLim);
                                continue;
                            case 'B':  // month (long)
                                buf = base::utils::Str::addToBuff(
                                        base::consts::kMonths[tInfo->tm_mon],
                                        buf, bufLim);
                                continue;
                            case 'y':  // year (two digits)
                                buf = base::utils::Str::convertAndAddToBuff(
                                        tInfo->tm_year +
                                        base::consts::kYearBase, 2, buf,
                                        bufLim);
                                continue;
                            case 'Y':  // year (four digits)
                                buf = base::utils::Str::convertAndAddToBuff(
                                        tInfo->tm_year +
                                        base::consts::kYearBase, 4, buf,
                                        bufLim);
                                continue;
                            case 'h':  // hour (12-hour)
                                buf = base::utils::Str::convertAndAddToBuff(
...
// LogFormat
        ...
        bool LogFormat::operator==(const LogFormat &other) {
            return m_level == other.m_level &&
                   m_userFormat == other.m_userFormat &&
        ...
        LogFormat::parseFromFormat(const base::type::string_t &userFormat) {
            ...
            auto conditionalAddFlag = [&](const base::type::char_t *specifier,
                ...
                while ((foundAt = formatCopy.find(specifier, foundAt + 1)) !=
                       base::type::string_t::npos) {
                    if (foundAt > 0 && formatCopy[foundAt - 1] ==
                                       base::consts::kFormatSpecifierChar) {
                        if (hasFlag(flag)) {
...
// TypedConfigurations
        ...
        const std::string &TypedConfigurations::filename(Level level) {
            return getConfigByRef<std::string>(level, &m_filenameMap,
                                               "filename");
        }
        ...
        TypedConfigurations::subsecondPrecision(Level level) {
            return getConfigByRef<base::SubsecondPrecision>(level,
        ...
        TypedConfigurations::resolveFilename(const std::string &filename) {
            ...
            if ((dateIndex = resultingFilename.find(
                ...
                if (dateIndex != std::string::npos) {
                    ...
                    std::string fmt;
                    if ((resultingFilename.size() > dateIndex) &&
                        (ptr[0] == '{')) {
                        // User has provided format for date/time
                        ++ptr;
                        int count = 1;  // Start by 1 in order to remove starting brace
                        ...
                        resultingFilename.erase(
                                dateIndex + dateTimeFormatSpecifierStr.size(),
                                count);
                        fmt = ss.str();
                    } else {
                        fmt = std::string(
        ...
        void TypedConfigurations::insertFile(Level level,
            ...
            auto create = [&](Level level) {
                ...
                if (filestreamIter == m_logStreamsReference->end()) {
                    ...
                    m_filenameMap.insert(
                            std::make_pair(level, resolvedFilename));
            ...
            create(m_filenameMap.empty() && m_fileStreamMap.empty()
                   ? Level::Global : level);
        }
...
// RegisteredHitCounters
        ...
        bool RegisteredHitCounters::validateEveryN(const char *filename,
            ...
            if (counter == nullptr) {
                registerNew(
                        counter = new base::HitCounter(filename, lineNumber));
            }
...
// RegisteredLoggers
        ...
        RegisteredLoggers::RegisteredLoggers(
        ...
        RegisteredLoggers::get(const std::string &id, bool forceCreation) {
            base::threading::ScopedLock scopedLock(lock());
            Logger *logger_ = base::utils::Registry<Logger, std::string>::get(
                    id);
        ...
        bool RegisteredLoggers::remove(const std::string &id) {
            ...
            Logger *logger = base::utils::Registry<Logger, std::string>::get(
                    id);
            if (logger != nullptr) {
...
// VRegistry
        ...
        VRegistry::VRegistry(base::type::VerboseLevel level,
                             base::type::EnumType *pFlags) : m_level(level),
                                                             m_pFlags(pFlags) {
        }
...
/// @brief Sets verbose level. Accepted range is 0-9
        ...
        void VRegistry::setModules(const char *modules) {
            ...
            auto addSuffix = [](std::stringstream &ss, const char *sfx,
                ...
                if (prev != nullptr &&
                    ...
                    std::string chr(
                            ss.str().substr(0, ss.str().size() - strlen(prev)));
                    ss.str(std::string(""));
                    ss << chr;
                }
                if (base::utils::Str::endsWith(ss.str(), std::string(sfx))) {
                    std::string chr(
                            ss.str().substr(0, ss.str().size() - strlen(sfx)));
                    ss.str(std::string(""));
                    ss << chr;
                }
                ss << sfx;
            };
            auto insert = [&](std::stringstream &ss,
                ...
                switch (*modules) {
                    ...
                    case ',':
                        ...
                        if (!ss.str().empty() && level != -1) {
                            insert(ss,
                                   static_cast<base::type::VerboseLevel>(level));
                            ss.str(std::string(""));
                            level = -1;
                        }
                        break;
        ...
        void VRegistry::setFromArgs(
            ...
            if (commandLineArgs->hasParam("-v") ||
                ...
                setLevel(static_cast<base::type::VerboseLevel>(atoi(
                        commandLineArgs->getParamValue("--v"))));
            } else if (commandLineArgs->hasParamWithValue("--V")) {
                setLevel(static_cast<base::type::VerboseLevel>(atoi(
                        commandLineArgs->getParamValue("--V"))));
...
// Storage
        ...
        Storage::Storage(const LogBuilderPtr &defaultLogBuilder) :
                ...
                m_registeredLoggers(
                        new base::RegisteredLoggers(defaultLogBuilder)),
  ...
  base::utils::safeDelete(m_asyncLogQueue);
        ...
        bool Storage::hasCustomFormatSpecifier(const char *formatSpecifier) {
            base::threading::ScopedLock scopedLock(
        ...
        void Storage::installCustomFormatSpecifier(
            ...
            if (hasCustomFormatSpecifier(
                    customFormatSpecifier.formatSpecifier())) {
                return;
            }
            base::threading::ScopedLock scopedLock(
                    customFormatSpecifiersLock());
...
// DefaultLogBuilder
        ...
        base::type::string_t
        DefaultLogBuilder::build(const LogMessage *logMessage,
                                 bool appendNewLine) const {
            base::TypedConfigurations *tc = logMessage->logger()->typedConfigurations();
            const base::LogFormat *logFormat = &tc->logFormat(
                    logMessage->level());
            ...
            el::base::threading::ScopedLock lock_(
...
// MessageBuilder
        ...
        void MessageBuilder::initialize(Logger *logger) {
            m_logger = logger;
            m_containerLogSeperator = ELPP->hasFlag(
...
// Writer
        ...
        void Writer::initializeLogger(const std::string &loggerId, bool lookup,
            ...
            if (m_logger == nullptr) {
                ...
                if (ELPP->hasFlag(LoggingFlag::HierarchicalLogging)) {
                    m_proceed = m_level == Level::Verbose ? m_logger->enabled(
                            m_level) :
    ...
    m_logger->releaseLock();
        ...
        void Writer::triggerDispatch(void) {
            if (m_proceed) {
                if (m_msg == nullptr) {
                    LogMessage msg(m_level, m_file, m_line, m_func,
                                   m_verboseLevel,
                                   m_logger);
                    base::LogDispatcher(m_proceed, &msg,
                                        m_dispatchAction).dispatch();
                } else {
                    base::LogDispatcher(m_proceed, m_msg,
                                        m_dispatchAction).dispatch();
                }
            }
            if (m_logger != nullptr) {
    ...
    void Loggers::configureFromGlobal(const char *globalConfigurationFilePath) {
        ...
        while (gcfStream.good()) {
            ...
            if (line.size() > 2 && base::utils::Str::startsWith(line,
                                                                std::string(
                                                                        base::consts::kConfigurationLoggerId))) {
