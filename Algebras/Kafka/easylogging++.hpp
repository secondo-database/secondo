...
/// @brief Base of thread safe class, this class is inheritable-only
            ...
            public:
                virtual inline void
                acquireLock(void) ELPP_FINAL { m_mutex.lock(); }

                virtual inline void
                releaseLock(void) ELPP_FINAL { m_mutex.unlock(); }

                virtual inline base::threading::Mutex &
                lock(void) ELPP_FINAL { return m_mutex; }
...
/// @brief String utilities helper class used internally. You should not use it.
            ...
            public:
                ...
                static std::string &
                replaceAll(std::string &str, char replaceWhat,
                           char replaceWith);
                ...
                static std::string &
                replaceAll(std::string &str, const std::string &replaceWhat,
...
/// @brief Gets pointer from repository. If none found, nullptr is returned.
            ...
            private:
                virtual void deepCopy(
                    ...
                    for (const_iterator it = sr.cbegin();
                         it != sr.cend(); ++it) {
...
/// in order to validate pointer.
            ...
            public:
                ...
                static T *callback(const std::string &id,
                                   std::unordered_map<std::string, TPtr> *mapT) {
                    typename std::unordered_map<std::string, TPtr>::iterator iter = mapT->find(
                            id);
                    if (iter != mapT->end()) {
...
/// @brief Resolving function for format specifier
    typedef std::function<std::string(
            const LogMessage *)> FormatSpecifierValueResolver;
...
/// This is thread safe and final class containing non-virtual destructor (means nothing should inherit this class)
        ...
        private:
            ...
            Conf_T unsafeGetConfigByVal(Level level,
                ...
                typename std::unordered_map<Level, Conf_T>::const_iterator it = confMap->find(
                        level);
                if (it == confMap->end()) {
                    try {
            ...
            Conf_T &unsafeGetConfigByRef(Level level,
                                         ...
                                         const char *confName) {
                ELPP_UNUSED(confName);
                typename std::unordered_map<Level, Conf_T>::iterator it = confMap->find(
                        level);
                if (it == confMap->end()) {
                    try {
                        return confMap->at(Level::Global);
        ...
        public:
            ...
            inline void validateHitCounts(std::size_t n) {
                if (m_hitCounts >= base::consts::kMaxLogPerCounter) {
                    m_hitCounts = (n >= 1 ? base::consts::kMaxLogPerCounter % n
                                          : 0);
                }
                ++m_hitCounts;
            }
...
/// @brief Action to be taken for dispatching
    ...
    public:
        LogBuilder() : m_termSupportsColor(
                base::utils::OS::termSupportsColor()) {}
...
/// @brief Loggers repository
        ...
        public:
            ...
            inline void unregister(Logger *&logger) {
                base::threading::ScopedLock scopedLock(lock());
                base::utils::Registry<Logger, std::string>::unregister(
                        logger->id());
            }
...
/// @brief Represents registries for verbose logging
        ...
        public:
            ...
            inline bool vModulesEnabled(void) {
                return !base::utils::hasFlag(LoggingFlag::DisableVModules,
                                             *m_pFlags);
            }
    ...
    class LogMessage {
    public:
        LogMessage(Level level, const std::string &file,
                   base::type::LineNumber line, const std::string &func,
...
/// @brief Easylogging++ management storage
        ...
        public:
            ...
            inline bool validateEveryNCounter(const char *filename,
                                              base::type::LineNumber lineNumber,
                                              std::size_t occasion) {
                return hitCounters()->validateEveryN(filename, lineNumber,
                                                     occasion);
            }
  ...
  inline T* performanceTrackingCallback(const std::string& id) {
    return base::utils::Utils::callback<T, base::type::PerformanceTrackingCallbackPtr>(id, &m_performanceTrackingCallbacks);
            ...
            inline std::string getThreadName(const std::string &threadId) {
                base::threading::ScopedLock scopedLock(m_threadNamesLock);
                std::unordered_map<std::string, std::string>::const_iterator it = m_threadNames.find(
                        threadId);
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
        private:
            ...
            writeIterator(Iterator begin_, Iterator end_, std::size_t size_) {
                m_logger->stream() << ELPP_LITERAL("[");
                for (std::size_t i = 0; begin_ != end_ && i <
                                                          base::consts::kMaxLogPerContainer; ++i, ++begin_) {
                    operator<<(*begin_);
                    m_logger->stream()
                            << ((i < size_ - 1) ? m_containerLogSeperator
                                                : ELPP_LITERAL(""));
                }
                if (begin_ != end_) {
                    m_logger->stream() << ELPP_LITERAL("...");
...
/// @brief Main entry point of each logging
        ...
        public:
            Writer(Level level, const char *file, base::type::LineNumber line,
                   ...
                   base::type::VerboseLevel verboseLevel = 0) :
                    ...
                    m_func(func), m_verboseLevel(verboseLevel),
                    m_logger(nullptr), m_proceed(false),
                    m_dispatchAction(dispatchAction) {
            }
        ...
        protected:
            ...
            void
            initializeLogger(const std::string &loggerId, bool lookup = true,
                             bool needLock = true);
        ...
        public:
            PErrorWriter(Level level, const char *file,
                         base::type::LineNumber line,
                         const char *func,
                         base::DispatchAction dispatchAction = base::DispatchAction::NormalLog,
                         base::type::VerboseLevel verboseLevel = 0) :
                    base::Writer(level, file, line, func, dispatchAction,
                                 verboseLevel) {
            }
        ...
        if (level == Level::Verbose) {
            if (ELPP->vRegistry()->allowed(vlevel, __FILE__)) {
                base::Writer(Level::Verbose, "FILE", 0, "FUNCTION",
                             base::DispatchAction::NormalLog, vlevel).construct(
                        this, false) << log;
            } else {
            ...
            base::Writer(level, "FILE", 0, "FUNCTION").construct(this, false)
                    << log;
        }
    }
...
/// @brief Static helpers to deal with loggers and their configurations
    ...
    public:
        ...
        static inline bool
        installLoggerRegistrationCallback(const std::string &id) {
            return ELPP->registeredLoggers()->installLoggerRegistrationCallback<T>(
                    id);
        }
        ...
        static inline void
        uninstallLoggerRegistrationCallback(const std::string &id) {
            ELPP->registeredLoggers()->uninstallLoggerRegistrationCallback<T>(
                    id);
        }
        ...
        static inline void
        reconfigureAllLoggers(ConfigurationType configurationType,
                              const std::string &value) {
        ...
        static void
        reconfigureAllLoggers(Level level, ConfigurationType configurationType,
                              const std::string &value);
