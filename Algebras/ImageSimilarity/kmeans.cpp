...
[1] Implementation of the kmeans algorithm
...
Kmeans::Kmeans() : x(NULL), n(0), k(0), d(0), numThreads(0), 
    converged(false),
...
void Kmeans::initialize(Dataset const *aX, unsigned short aK, 
...
void Kmeans::changeAssignment(int xIndex, int closestCluster, 
    int threadId) {
...
void *Kmeans::runner(void *args) {
    ...
    ti->numIterations = ti->km->runThread(ti->threadId, 
        ti->maxIterations);
    #endif
    return NULL;
}
...
int Kmeans::run(int maxIterations) {
    ...
    #ifdef USE_THREADS
    {
        ...
        for (int t = 0; t < numThreads; ++t) {
            ...
            pthread_create(&info[t].pthread_id, NULL, Kmeans::runner, 
                &info[t]);
        }
...
void Kmeans::verifyAssignment(int iteration, int startNdx, int endNdx) 
    const {
    ...
    for (int i = startNdx; i < endNdx; ++i) {
        ...
        if (closest != assignment[i]) {
            ...
            << std::endl;
            std::cerr << "point index           = " << i                      
            << std::endl;
            std::cerr << "closest center        = " << closest                
            << std::endl;
            std::cerr << "closest center dist2  = " << closest_dist2         
            << std::endl;
            std::cerr << "assigned center       = " << assignment[i]          
            << std::endl;
            std::cerr << "assigned center dist2 = " 
                << original_closest_dist2 
                << std::endl;
            assert(false);
        }
    }
