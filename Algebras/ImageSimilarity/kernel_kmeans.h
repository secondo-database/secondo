...
[1] Declarations for the KernelKmeans algorithm
...
class Kernel {
    public:
        virtual ~Kernel() {}
        virtual double operator()(double const *, double const *, int) 
        const = 0;
...
class LinearKernel : public Kernel {
    public:
        virtual double operator()(double const *a, double const *b, 
                                    int dimension) const { 
...
class PolynomialKernel : public Kernel {
    public:
        ...
        virtual double operator()(double const *a, double const *b, 
                                                int dimension) const { 
            return pow(std::inner_product(a, a + dimension, b, 0.0) 
            + c, power);
        }
...
class KernelKmeans : public Kmeans {
    ...
    protected:
        ...
        double centerCenterInnerProductGeneral(
        std::vector<unsigned int> const &members1, 
        std::vector<unsigned int> const &members2) const;
        double pointCenterInnerProductGeneral(int xndx, 
        ...
        unsigned short c2) const {
            return centerCenterInnerProductGeneral(memberships[c1], 
            memberships[c2]);
        }
        virtual double pointCenterInnerProduct(int xndx, 
        unsigned short cluster) const {
            return pointCenterInnerProductGeneral(xndx, 
            memberships[cluster]);
        }
        ...
        void computeMemberships(int threadId, 
