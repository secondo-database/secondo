...
[1] Implementation of general functions
...
Dataset *init_centers(Dataset const &x, unsigned short k) {
    ...
    for (int i = 0; i < k; ++i) {
        ...
        if (c->sumDataSquared) {
            c->sumDataSquared[i] = std::inner_product(cdp, cdp + x.d, 
                cdp, 0.0);
        }
    }
...
Dataset *init_centers_kmeanspp(Dataset const &x, unsigned short k) {
    ...
    while (ndx < k) {
        ...
        for (int i = 1; i < x.n; ++i) {
            distribution[i] = distribution[i - 1] + dist2[i].first / 
                sum_distribution;
        }

        // choose a random interval according to the new distribution
        double r = (double)rand() / (double)RAND_MAX;
        double *new_center_ptr = std::lower_bound(distribution, 
            distribution + x.n, r);
...
Dataset *init_centers_kmeanspp_v2(Dataset const &x, unsigned short k) {
    ...
    while (ndx < k) {
        ...
        do {
            // choose a random interval according to the new distribution
            double r = sum_distribution * (double)rand() 
                / (double)RAND_MAX;
    ...
    for (int i = 0; i < c->n; ++i) {
        double *cdp = c->data + i * x.d;
        memcpy(cdp, x.data + chosen_pts[i] * x.d, sizeof(double) 
            * x.d);
        if (c->sumDataSquared) {
            c->sumDataSquared[i] = std::inner_product(cdp, cdp 
            + x.d, cdp, 0.0);
        }
    }
...
double getMemoryUsage() {
    ...
    if (pf) {
        int numScanned = fscanf(pf, "%u %u" /* %u %u %u %u %u"*/, 
            &totalProgramSizeInPages, &residentSetSizeInPages);
        if (numScanned != 2) {
            return 0.0;
        }
    }
    ...
    double sizeInKilobytes = residentSetSizeInPages * 4.0; 
    // assumes 4096 byte page
    // getconf PAGESIZE
...
void assign(Dataset const &x, Dataset const &c, 
    unsigned short *assignment) {
    for (int i = 0; i < x.n; ++i) {
        double shortestDist2 = std::numeric_limits<double>::max();
        int closest = 0;
        for (int j = 0; j < c.n; ++j) {
            double d2 = 0.0, *a = x.data + i * x.d, *b = c.data 
                + j * x.d;
