...
[1] Implementation of the sort kmeans algorithm
 ...
 * Return value: none
 */
void SortKmeans::initialize(Dataset const *aX, unsigned short aK, 
...
void SortKmeans::sort_centers() {
    ...
    for (int j = 0; j < k; ++j) {
        ...
        for (int p = j + 1; p < k; ++p) {
            sortedCenters[p * k + j].first = 
                sortedCenters[j * k + p].first = 
                    centerCenterDist2(j, p) / 4.0;
