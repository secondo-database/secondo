...
[1] Implementation of the annulus algorithm
...
int AnnulusKmeans::runThread(int threadId, int maxIterations) {
    ...
    while ((iterations < maxIterations) && ! converged) {
        ...
        if (threadId == 0) {
            // compute the inter-center distances, 
            //keeping only the closest distances
            sort_means_by_norm();
        }
        synchronizeAllThreads();
        ...
        for (int i = startNdx; i < endNdx; ++i) {
            ...
            std::pair<double, int>* begin = 
                std::lower_bound(cOrder, cOrder + k, 
                std::make_pair(xNorm[i] - beta, k));
            std::pair<double, int>* end = 
                std::lower_bound(begin, cOrder + k, 
                std::make_pair(xNorm[i] + beta, k));
...
void AnnulusKmeans::initialize(Dataset const *aX, unsigned short aK, 
