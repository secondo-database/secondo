...
1 Auxiliary definitions for value mapping function of operators ~sort~ and
~sortby~
...
public:
  TupleAndRelPos() : ref(0), pos(0), cmpPtr(0){};

  ~TupleAndRelPos() {}

  TupleAndRelPos(Tuple *newTuple, TupleCompareBy *cmpObjPtr = 0, int newPos = 0)
      : ref(newTuple), pos(newPos), cmpPtr(cmpObjPtr){};

  inline TupleAndRelPos(const TupleAndRelPos &rhs)
      : ref(rhs.ref), pos(rhs.pos), cmpPtr(rhs.cmpPtr){};
...
template <class TupleCmp> class UniversalCompare {
public:
  ...
  CompareObject(bool lexOrder, Word *args) {
    ...
    if (lexOrder) {
      ...
      for (int i = 1; i <= nSortAttrs; i++) {
        ...
        spec.push_back(
            std::pair<int, bool>(sortAttrIndex, sortOrderIsAscending));
      };
