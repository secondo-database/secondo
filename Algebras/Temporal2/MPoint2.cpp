...
MPoint2::MPoint2( const MPoint2& rhs ) :
     ...
     bbox2(rhs.bbox2.IsDefined())
 {
     ...
     //optimized Flob-Copy for persintent data
...
MPoint2* MPoint2::Clone() const{
 ...
 void MPoint2::StartBulkLoad() //TODO
 {
   assert( IsDefined() );
   assert( ordered );
   ordered = false; // wozu brauchen wir das?
 }
...
// no longer needed? for objects everything happens in the flob?
// even if we had memory: cleanup should work automagically, too
// we'd also need this for regular flushes of the memdata
namespace gentc {
template<>
void Delete<temporal2algebra::MPoint2>(const ListExpr typeInfo,Word& w){
    ...
    cout << "gentc::Delete<MPoint2>(): " << *B << endl;
    ...
    B->memClear(); // This is the only difference to the Default Delete!
    // we may skip this, as automatic cleanup should detect it as obsolete
    delete B;
    B = NULL;
    w.addr=0;
}
...
void Close<temporal2algebra::MPoint2>(const ListExpr typeInfo, Word& w){
    temporal2algebra::MPoint2* B =
            static_cast<temporal2algebra::MPoint2*>(w.addr);
    cout << "genttc::Close<MPoint2>(): " << *B << endl;
    cout << "IsDatabaseOpen()="
            << (SmiEnvironment::IsDatabaseOpen()?"true\n":"false\n")
            << "CurrentDatabase()="
            << SmiEnvironment::CurrentDatabase() << endl;
