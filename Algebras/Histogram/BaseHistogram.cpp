...
namespace hgr
{
  ...
  const HIST_REAL* BaseHistogram::GetBin(int i) const
  {
  ...
  void BaseHistogram::SetDefined(bool _defined)
  {
    defined = _defined;
  }  
  ...
  int FoldFun(Word* args, Word& result, int message, Word& local, Supplier s)
  {
    ...
    ArgVectorPointer funargs = qp->Argument(fun);
    Word value = SetWord(args[2].addr);
    CcReal* bin = new CcReal();
  ...
  ListExpr FoldTypeMap(ListExpr args)
  {
    ...
    CHECK_COND(fun.second() == type
        && initVal == type,
        "First function parameter or init value "
        "not of type " + type.convertToString());

    ListExpr dummy;

    CHECK_COND(am->CheckKind("DATA", type.listExpr(), dummy),
        "Type of parameter is not in kind DATA.");
      ...
      Supplier s)
  {
    ...
    result = qp->ResultStorage(s);
    CcReal* distance = (CcReal*)result.addr;
