...
Histogram2d::Histogram2d(bool _defined, size_t sizeX, size_t sizeY)
    : BaseHistogram(_defined, sizeX * sizeY), rangeX(sizeX + 1),
      rangeY(sizeY + 1) {}
...
*/
void Histogram2d::AppendRangeX(const HIST_REAL &r) { rangeX.Append(r); }

void Histogram2d::AppendRangeY(const HIST_REAL &r) { rangeY.Append(r); }

/*
4.3 GetNoRangeX, GetNoRangeY

*/
int Histogram2d::GetNoRangeX() const { return rangeX.Size(); }

int Histogram2d::GetNoRangeY() const { return rangeY.Size(); }
...
void Histogram2d::Coarsen(const BaseHistogram *b) {
  ...
  for (int i = 0; i < pattern->GetNoBin(); i++) {
    ...
    while (myRangeX < GetNoRangeX() &&
           CmpReal(GetRangeX(myRangeX), pattern->GetRangeX(patternRangeX)) !=
               1) {
      myRangeY = initY;
      while (myRangeY < GetNoRangeY() &&
             CmpReal(GetRangeY(myRangeY), pattern->GetRangeY(patternRangeY)) !=
                 1) {
        binVal += GetBin((myRangeX - 1) * (GetNoRangeY() - 1) + myRangeY - 1);
        myRangeY++;
...
const CcInt Histogram2d::FindBinX(const HIST_REAL &x) const {
  if (GetNoRangeX() < 2 || x < GetRangeX(0) ||
...
const CcInt Histogram2d::FindBinY(const HIST_REAL &y) const {
  if (GetNoRangeY() < 2 || y < GetRangeY(0) ||
...
*/
int Histogram2d::NoBinsX() const { return max(0, GetNoRangeX() - 1); }

int Histogram2d::NoBinsY() const { return max(0, GetNoRangeY() - 1); }
...
*/
size_t Histogram2d::Sizeof() const { return sizeof(*this); }
...
*/
bool Histogram2d::Adjacent(const Attribute *attrib) const { return false; }

/*
6.7 Clone

*/
Attribute *Histogram2d::Clone() const { return new Histogram2d(*this); }
...
*/
int Histogram2d::NumOfFLOBs() const { return 3; }
...
*/
void *Histogram2d::Cast(void *addr) { return (new (addr) Histogram2d); }
...
*/
ostream &Histogram2d::Print(ostream &os) const { return (os << *this); }
...
int SetHistogram2dFun(Word *args, Word &result, int message, Word &local,
  ...
  while (receivedX || receivedY) {
    if (receivedX) {
      ...
      if (!firstElemX && (CmpReal(valueX, lastValueX) < 1)) {
        cout << "Error in operator set_histogram2d: "
             << "The first stream is not sorted or contains duplicates."
             << endl;
        resultHg->Clear();
        resultHg->SetDefined(false);
        qp->Close(args[0].addr);
    ...
    if (receivedY) {
      ...
      if (!firstElemY && (CmpReal(valueY, lastValueY) < 1)) {
        cout << "Error in operator set_histogram2d: "
             << "The second stream is not sorted or contains duplicates."
             << endl;
        resultHg->Clear();
        resultHg->SetDefined(false);
        qp->Close(args[0].addr);
  ...
  int noOfBins = (resultHg->GetNoRangeX() - 1) * (resultHg->GetNoRangeY() - 1);
...
int CreateHistogram2dFun(Word *args, Word &result, int message, Word &local,
  ...
  while (qp->Received(args[0].addr)) {
    ...
    attrValX = (CcReal *)currentTuple->GetAttribute(indexX->GetIntval() - 1);
    attrValY = (CcReal *)currentTuple->GetAttribute(indexY->GetIntval() - 1);
...
ListExpr CreateHistogram2dTypeMap(ListExpr args) {
  ...
  if (stream.length() != 2 || !stream.first().isSymbol(Symbol::STREAM()) ||
  ...
  ListExpr result = nl->ThreeElemList(
      ...
      nl->TwoElemList(nl->IntAtom(indexX), nl->IntAtom(indexY)),
...
ListExpr CreateHistogram2dEquiwidthTypeMap(ListExpr args) {
  ...
  errorMsg = "Operator create_histogram2d_equiwidth expects as first argument "
  ...
  errorMsg = "Attributename '" + attrname +
             "' is not known.\n"
             "Known Attribute(s): " +
             NList(attrlist).convertToString();
...
int Shrink2dFun(Word *args, Word &result, int message, Word &local,
  ...
  if (!hist->IsDefined() || hist->IsEmpty() || !hist->IsConsistent(false) ||
      !loX->IsDefined() || !hiX->IsDefined() || !loY->IsDefined() ||
      !hiY->IsDefined()) {
  ...
  if ((lowX < hist->GetRangeX(0) && highX < hist->GetRangeX(0)) ||
      (lowX > hist->GetRangeX(hist->GetNoRangeX() - 1) &&
       highX > hist->GetRangeX(hist->GetNoRangeX() - 1))) {
    cout << "Error in operator shrink_x: "
         << "The specified x-interval is outside the histogram-xrange." << endl;
    res->SetDefined(false);
    return 0;
  }

  if ((lowY < hist->GetRangeY(0) && highY < hist->GetRangeY(0)) ||
      (lowY > hist->GetRangeY(hist->GetNoRangeY() - 1) &&
       highY > hist->GetRangeY(hist->GetNoRangeY() - 1))) {
    cout << "Error in operator shrink_x: "
         << "The specified y-interval is outside the histogram-yrange." << endl;
    res->SetDefined(false);
    return 0;
  }
...
template int Shrink2dFun<false>(Word *args, Word &result, int message,
                                Word &local, Supplier s);

template int Shrink2dFun<true>(Word *args, Word &result, int message,
                               Word &local, Supplier s);
...
ListExpr Shrink2dTypeMap(ListExpr args) {
  ...
  if (!hist.isSymbol(Histogram2d::BasicType())) {
    return listutils::typeError("Expects '" + Histogram2d::BasicType() +
                                "' as 1st argument");
  }
...
ListExpr Getcount2dTypeMap(ListExpr args) {
  ...
  if (arg1.isSymbol(Histogram2d::BasicType()) &&
      arg2.isSymbol(CcInt::BasicType()) && arg3.isSymbol(CcInt::BasicType()))
    return NList(CcReal::BasicType()).listExpr();
...
class SortStream {
public:
  SortStream(Word stream, const bool lexicographic, void *tupleSmaller)
      : stream(stream),
        lexiTupleSmaller(
            lexicographic ? (LexicographicalTupleSmaller *)tupleSmaller : 0),
    ...
    [[maybe_unused]] size_t i = 0, a = 0, n = 0, m = 0,
                            r = 0; // counter variables
  ...
  size_t TupleCount() { return count; }

  priority_queue<TupleAndRelPos> GetTuples() { return tuples; }
  ...
  void ShowPartitionInfo(int c, int a, int n, int m, int r,
    ...
    if (RTFlag::isActive("ERA:Sort:PartitionInfo")) {
      cmsg.info() << "Current run finished: "
                  << "  processed tuples=" << c << ", append minimum=" << m
                  << ", append next=" << n << endl
                  << "  materialized runs=" << r
...
class SortQueue {
public:
  SortQueue(priority_queue<TupleAndRelPos> stream, const bool lexicographic,
            void *tupleSmaller)
      : stream(stream),
        lexiTupleSmaller(
            lexicographic ? (LexicographicalTupleSmaller *)tupleSmaller : 0),
...
int CreateHistogram2dEquicountFun(Word *args, Word &result, int message,
  ...
  if (maxCategoriesX->IsDefined() && maxCategoriesY->IsDefined() &&
    ...
    SortStream *sli;
    sli = new SortStream(args[0], false, tupleCmp);
    ...
    while ((currentTuple = sli->NextResultTuple()) != 0) {
      ...
      // = noBins + 1
      if ((currentBin >= rest && count % noBins == 0) ||
          (currentBin < rest && count % (noBins + 1) == 0)) {
    ...
    // SortByLocalInfo* sli;
    SortQueue *sq = new SortQueue(sli->GetTuples(), false, tupleCmp);
    ...
    while ((currentTuple = sq->NextResultTuple()) != 0) {
      ...
      if ((currentBin >= rest && count % noBins == 0) ||
          (currentBin < rest &&
           count % (noBins + 1) ==
               0)) { // current value not yet greater than precedent value
        if (CmpReal(value, lastvalue) != 1) { // hist->Insert(currentBin);
          continue;
        } // If only a few values are left, merge the last bins.
        else if ((count + noTuples - tupleCount) < noBins * 1.1) {
          // hist->Insert(currentBin);
...
template int Insert2dFun<false>(Word *args, Word &result, int message,
                                Word &local, Supplier s);

template int Insert2dFun<true>(Word *args, Word &result, int message,
                               Word &local, Supplier s);

template ListExpr Insert2dTypeMap<true>(ListExpr args);

template ListExpr Insert2dTypeMap<false>(ListExpr args);

template int FindBin2dFun<true>(Word *args, Word &result, int message,
                                Word &local, Supplier s);

template int FindBin2dFun<false>(Word *args, Word &result, int message,
                                 Word &local, Supplier s);

template int FindMinMaxBinFun2d<true>(Word *args, Word &result, int message,
                                      Word &local, Supplier s);

template int FindMinMaxBinFun2d<false>(Word *args, Word &result, int message,
                                       Word &local, Supplier s);
