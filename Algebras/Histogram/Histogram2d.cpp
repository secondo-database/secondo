...
*/
  ...
  ListExpr CreateHistogram2dEquicountTypeMap(ListExpr args)
  {
    ...
    if(!foundX) return listutils::typeError("Attribute x not found");
    if(!foundY) return listutils::typeError("Attribute y not found");

    if(!attrTypeX.isSymbol(REAL))
      return listutils::typeError("Attribute x not of type real");

    if(!maxCategoriesX.isSymbol(INT))
      return listutils::typeError("Number of categories for x is not an int");

    if(!attrTypeY.isSymbol(REAL))
      return listutils::typeError("Attribute y not of type real");

    if(maxCategoriesY.isSymbol(INT))
      return listutils::typeError("Number of categories for y is not an int");
