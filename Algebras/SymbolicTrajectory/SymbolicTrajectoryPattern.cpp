...
bool PatParser::checkForUniqueVariables() { // not used
  return true;
}
  ...
  for (size_t i = 0; i < conds_.size(); ++i) {
    ...
    for ( size_t j = 0; j < pats_.size(); ++j ) {
      ...
      if ( pats_[j].variable == cond.variable ) {
        if ( pats_[j].addCondition(cond, errMsg) ) 
          return 1;
      }
    } // end for j
  } // end for i  
...
int PatParser::getConds(string const &text, string &errMsg) {
  ...
  for (size_t i = 0; i < conds.size(); ++i) {
    ...
    if ( (pos = conds[i].find_first_of("<>=")) != string::npos ) {
      switch ( conds[i][pos] ) {
        case '<' :  
          if ((conds[i].length() > pos) && (conds[i][pos+1] == '=')) {
            // operator: <=
            op = 2 + 1;
            if (cond.set(conds[i], pos, op, errMsg))
              return 1;
        ...
        default :  
          errMsg = "Cannot resolve condition: " + conds[i].substr(0,15) + "...";
          return 1;
          break; 
      } // end switch
    } // end if
...
int Condition::set(string cond, int op_pos, int op, string &errMsg) {  
  vector<string> lhs;
  Condition::op = op;
  Condition::value = trim( (op == 1 || op == 2 || op == 4) ? 
                     cond.substr(op_pos + 1) : cond.substr(op_pos + 2) );  
...
string SinglePattern::toString() 
{
   ...
   {
      result += "key=" + int2Str( conditions[i].key ) + " ,op="  + int2Str(
            conditions[i].op ) + " ,value=" + conditions[i].value + "; ";
   }
   result += "\n";
...
int SinglePattern::set(PrePat const &prePat, string &errMsg)
{
   ...
   if (!prePat.right.empty() && prePat.right != "_" )
     lbs = getElementsFromSet(prePat.right);
    ...
    for (size_t i = 0; i < elements.size(); ++i)
    {
      ...
      switch (type) 
      {
         ...
         case 5:  semt.type = type;
                  ...
                  sts.push_back( semt );
                  break;
         case 6:  dtrs.push_back(datetime); 
                  break;
