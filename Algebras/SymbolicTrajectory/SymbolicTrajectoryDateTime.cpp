...
public:
   ...
   int getWeekdayNr(string weekday) {return weekdays_map[weekday];} ;
   string getWeekday(int weekday) { return (weekday < 1 || weekday > countWeekdays()) ? "" : weekdays_vector[weekday - 1]; } ;
   int countWeekdays() {return weekdays_map.size(); };

   int getDaytimeNr(string daytime) {return daytimes_map[daytime];};
   string getDaytime(int daytime) { return (daytime < 1 || daytime > countDaytimes()) ? "" : daytimes_vector[daytime - 1]; };
   int countDaytimes() { return daytimes_map.size(); };

   int getMonthNr(string month) {return months_map[month];};
   string getMonth(int month) { return (month < 1 || month > countMonths()) ? "" : months_vector[month - 1]; };
   int countMonths() { return months_map.size(); };
...
int SymbolicTimeRange::getDaysOfMonth(int month, int year) 
{
   ...
   switch (month) 
   {
	  ...
	  case 2 :  days = ( year % 4 == 0 && year % 200 != 0) ? 29 : 28; break;
...
string getFullDate(string const &text, bool const startDate)
{
   ...
   {
      fullDate = setira.getDaysOfMonth(text.substr(0,2), text.substr(3)) + "." + text; 
   }
   else
   {
      fullDate = dateDefault.substr(0, dateDefault.length() - text.length()) + text;
   }  
...
string getFullDateTime(string const &text, bool const startDateTime)
{
   ...
   if (elements.empty()) 
   {
      fullDateTime = getFullDate("", startDateTime) + "#" + getFullTime("", startDateTime);
   }
   else if (elements.size() == 1) 
   {
      fullDateTime = getFullDate(elements[0], startDateTime) + "#" + getFullTime("", startDateTime);
   }
   else if (elements.size() == 2) 
   {
      fullDateTime = getFullDate(elements[0], startDateTime) + "#" + getFullTime(elements[1], startDateTime);
   }   
...
string getFullRange(string const &text)
{
   ...
   if (dates.size() == 1) 
   {
      resultRange = getFullDateTime( dates[0], true ) + "-" + getFullDateTime( "", false );	  
   } 
...
string getFullTimeRange(string const &text)
{
   ...
   {
      resultRange = getFullTime( times[0], true ) + "-" + getFullTime( times[1], true );      
   }
...
int getType(string text, string &datetime)
{
   ...
   if (mask.empty()) return type; // = 0, cannot resolve element
 ...
 string getMask(string const &text)
{
   ...
   for (size_t i = 0; i < text.length(); ++i)
   {
       ...
       if  ( (char_cur >= 65 && char_cur <= 90) || (char_cur >= 97 && char_cur <= 122) ) 
	   {
	      mask += 'A'; 
       ...
       else if  ( (char_cur == '.' || char_cur == '#' || char_cur == ':' || char_cur == '/')  ) 
	   {
	      mask += char_cur;
		  onlyAlphaChars = false;		  
		  onlyNumericChars = false;	
