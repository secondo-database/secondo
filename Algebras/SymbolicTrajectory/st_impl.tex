...
The next step for \texttt{rewrite} is to scan $S$, eliminating the sequences which do not fulfill the set of specified conditions in order to obtain the final set of sequences for rewriting the pattern.

However, we suppose the operator \texttt{matches} is invoked, with the consequence that it suffices to verify whether there is a sequence that complies with every condition, which is performed in algorithm \ref{alg:conditionsmatch}. Hence, it is not necessary to compute $S$ as seen in algorithm \ref{alg:buildsequences}, and with slight modifications we can use the latter to create only one sequence at a time.
