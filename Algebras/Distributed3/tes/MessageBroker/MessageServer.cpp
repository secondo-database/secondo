...
MessageServer::~MessageServer() {
 ...
 void MessageServer::processMessage() {
  //boost::this_thread::interruption_point();
  // Checks whether the socket is correctly initialized 
  // and ready for operation (SocketIO.h)
  if(!socket->IsOk()) { 
  ...
  while ((size_t)lengthRead < Message::HEADER_SIZE) {
    ...
    lengthRead = socket->Read((void *) offset, Message::HEADER_SIZE,
                             Message::HEADER_SIZE, 
                             1); // wait forever führt dazu, 
                             // dass von insges. 3212 nur 649 Tupel 
                             // gelesen werden 
                             // timeout bedeutet, dass Datenstrom für max 
                             // n Sekunden unterbrochen sein darf.
    // !!!!! bool Read( void* buf, size_t size ); 
    // !!!!! lengthRead = socket->Read((void *) offset, 
    // Message::HEADER_SIZE) true -> 1 !!!
    if (lengthRead < 0) { // When the return value is less 
    // than zero, an error has occurred. Usually it
 ...
 void MessageServer::handleFinishedMessage(const int eid) {
   ...
   for (int slot = 0; 
              slot < MessageBroker::get().getNumberOfSlots(eid); ++slot) {
 ...
 Tuple* MessageServer::getTuple(const int eid, const int slot) {
   ...
   // release lock as soon as possible. Messages can be 
   // pushed while processing the popped message
   boost::mutex* m = lock.release(); 
   m->unlock();
