...
namespace distributed3 {
 ...
 template<typename T>
 void MessageBroker::clear(std::map<int,
                               std::map<int, std::queue<T>*>>& inbox) {
 ...
 Tuple* MessageBroker::getTuple(const int eid, const int slot) {
   ...
   */ // alternative end
   ...
   while(true) {
     assert(!servers2[eid][slot].empty()); // addServer has 
     // been called earlier
     ...
     if (ms->hasTuple(eid,slot)) {
       (serversIterator[eid][slot])++; // it is not possible to 
       // move it to the beginning of the while-loop.
 ...
 void MessageBroker::addServer(const int eid, 
                               const int slot, 
                               MessageServer* server) { 
  
   // alternative: if getTuple() uses a MessageServer who has 
   // at least one tuple, we possibly avoid erasing a MessageServer 
   //             from servers2[eid][slot] who has not yet received 
   // its next tuple.
   boost::unique_lock<boost::mutex> lock {servers2mtx[eid][slot]};
   ...
   // thesis two: in the naive implementation servers[eid][slot] 
   // is locked for the shortest possible duration.
 ...
 void MessageBroker::pushTuple(const int eid, const int slot, Tuple* tuple) {
   // there is only one LoopbackProxy. And inserting tuples is 
   // finished before tuples are fetched. So no lock is needed
 ...
 bool MessageBroker::startClient(const int worker, 
                                 const RemoteEndpoint host) {
  try {
 ...
 void MessageBroker::stopServers() {
  if (tcpListenerRunning()) {
   ...
   int messageServerPort = TESContext::get().getMessageServerPort();
   // kann nicht einfach globalSocket zerstört werden?
 ...
 void MessageBroker::sendTuple(const int eid, 
                               const int slot, 
                               const int workerNumber, 
                               Tuple* tuple) {
  if (workerToClient.find(workerNumber) == workerToClient.end()) { // TODO 
 ...
 bool MessageBroker::tcpListenerRunning() {
  ...
  // wenn interrupt() aufgerufen, wird er jetzt unterbrochen
