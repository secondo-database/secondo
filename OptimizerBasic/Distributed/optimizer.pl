...
file ~opSyntax~. There are rules for
  ...
  * postfix followed by two arguments in square brackets, in total 3 or 4 arguments
  
  * postfix followed by three arguments in square brackets, in total 4 or 5 arguments
  
  * postfix followed by four arguments in square brackets, in total 5 arguments

  * postfix followed by five arguments in square brackets, in total 6 arguments

  * prefix, 2 or 3 arguments
...
% fapra 2015/16
/* Additional plan_to_atom rules to map Distributed2- Operators */ 
plan_to_atom(Term, Result) :-
  functor(Term, Op, 3),
...
plan_to_atom(Term, Result) :-
  ...
  concat_atom([Res1, ' ', Op, '[', Res2, ' ', Res3,' ',Res4, '] '], '', Result),
  !.
...
%fapra 2015/16
select(arg(N), Y) => X :-
  argument(N, rel(Name, _, _)),
  dSysRel(Name, ObjName, _, _),
  !, /* Operand is distributed. Do not translate into local selection. */
  distributedselect(ObjName, Y) => X.
...
distributedselect(ObjName, pr(Y = attr(AttrName, Arg, Case), Rel)) => X :-
  distributedselect(ObjName, pr(attr(AttrName, Arg, Case) = Y, Rel)) => X.

distributedselect(ObjName, pr(attr(AttrName, Arg, Case) = Y, _)) =>
  dsummarize(dmap(ObjName,"Result", filter(feed(rel(.,*, _)),
    Y = attr(AttrName, Arg, Case)))).
...
translate(Select from Rel, feed(Rel), Select, 0) :-
  not(dSysRel(Rel, _, _, _)),
  not(is_list(Rel)),
  !.
  
translate(Select from Rel, dsummarize(ObjName),Select, 0) :-
  dSysRel(Rel, ObjName, _, _), 
  not(is_list(Rel)),
  !.
%end fapra 2015/16
