...
secondo_detect() {
  ...
  # readline and jpeg are keg-only, i.e. Homebrew does NOT symlink their
  # headers/libs into $(brew --prefix)/include|lib.  The SDK used to ship
  # these; without it we point the makefiles at the formula prefixes so the
  # optimizer (libreadline) and the jpeg-using algebras link.
  if [ "$os" = Darwin ]; then
    if [ -z "${SECONDO_READLINE_DIR:-}" ]; then
      for cand in "$(brew --prefix readline 2>/dev/null)" \
                  /opt/homebrew/opt/readline /usr/local/opt/readline; do
        [ -n "$cand" ] && [ -f "$cand/include/readline/readline.h" ] \
          && { SECONDO_READLINE_DIR=$cand; break; }
      done
    fi
    if [ -z "${SECONDO_JPEG_DIR:-}" ]; then
      for cand in "$(brew --prefix jpeg-turbo 2>/dev/null)" \
                  "$(brew --prefix jpeg 2>/dev/null)" \
                  /opt/homebrew/opt/jpeg-turbo /usr/local/opt/jpeg-turbo; do
        [ -n "$cand" ] && [ -f "$cand/include/jpeglib.h" ] \
          && { SECONDO_JPEG_DIR=$cand; break; }
      done
    fi
    # flex is keg-only too. The C++ scanners (NLLex, SecLex) are generated by
    # the flex binary on PATH and then include <FlexLexer.h>; that header MUST
    # come from the same flex or the LexerInput/LexerOutput signatures mismatch.
    # macOS ships an old flex in the SDK, so point at the Homebrew one.
    if [ -z "${SECONDO_FLEX_DIR:-}" ]; then
      for cand in "$(brew --prefix flex 2>/dev/null)" \
                  /opt/homebrew/opt/flex /usr/local/opt/flex; do
        [ -n "$cand" ] && [ -f "$cand/include/FlexLexer.h" ] \
          && { SECONDO_FLEX_DIR=$cand; break; }
      done
    fi
    # libxml2 is keg-only; its headers live under <prefix>/include/libxml2.
    if [ -z "${SECONDO_LIBXML2_DIR:-}" ]; then
      for cand in "$(brew --prefix libxml2 2>/dev/null)" \
                  /opt/homebrew/opt/libxml2 /usr/local/opt/libxml2; do
        [ -n "$cand" ] && [ -f "$cand/include/libxml2/libxml/parser.h" ] \
          && { SECONDO_LIBXML2_DIR=$cand; break; }
      done
    fi
  fi
  [ -n "${SECONDO_READLINE_DIR:-}" ] && export SECONDO_READLINE_DIR
  [ -n "${SECONDO_JPEG_DIR:-}" ]     && export SECONDO_JPEG_DIR
  [ -n "${SECONDO_FLEX_DIR:-}" ]     && export SECONDO_FLEX_DIR
  [ -n "${SECONDO_LIBXML2_DIR:-}" ]  && export SECONDO_LIBXML2_DIR

  # --- link-time library search path (macOS) -------------------------------
  # Homebrew installs its libraries (boost, ...) under $(brew --prefix)/lib,
  # which is not a default linker path. Many tool makefiles link without the
  # platform LDFLAGS, so instead of patching each one, put the Homebrew lib
  # dirs on LIBRARY_PATH -- the compiler driver adds them to every link.
  if [ "$os" = Darwin ]; then
    for cand in "$(brew --prefix 2>/dev/null)/lib" \
                "${SECONDO_READLINE_DIR:+$SECONDO_READLINE_DIR/lib}" \
                "${SECONDO_JPEG_DIR:+$SECONDO_JPEG_DIR/lib}" \
                "${SECONDO_LIBXML2_DIR:+$SECONDO_LIBXML2_DIR/lib}" \
                "${SECONDO_FLEX_DIR:+$SECONDO_FLEX_DIR/lib}"; do
      [ -n "$cand" ] && [ -d "$cand" ] || continue
      case ":${LIBRARY_PATH:-}:" in
        *":$cand:"*) ;;
        *) LIBRARY_PATH="$cand${LIBRARY_PATH:+:$LIBRARY_PATH}" ;;
      esac
    done
