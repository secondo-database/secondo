 ...
 * secondo query, searches for implemented datatypes and creates objects for all
 * datatypes that have been found. All objects are put into a new resultlist of
 * datatypes.
 * 
...
public class DataTypeConstructor {
	 ...
	 * 
	 * @param le
	 *            The secondo result in nested list format
	...
	/**
	 * Creates a new DataType of Type Point and adds the information from the
	 * given listexpression
	 * 
	 * @param resultList
	 *            The resultlist of the secondo query
	...
	/**
	 * Creates a new DataType of Type Line and adds the information from the
	 * given points
	 * 
	 * @param a
	...
	/**
	 * Creates a new DataType of Type Polygon and adds the information from the
	 * given pointlist
	 * 
	 * @param pointlist
	 *            The list of points representing the outline of the polygon
	...
	/**
	 * Creates a new DataType of Type Polyline and adds the information from the
	 * given linelist
	 * 
	 * @param linelist
	 *            The list of lines that represent the path of the polyline
	...
	/**
	 * Creates a new DataType of Type MPoint and adds the information from the
	 * given elements
	 * 
	 * @param linelist
	 *            The list of lines representing the path of the moving point
	 * @param timelist
	 *            The list of dates respresenting the time that the moving point
	 *            takes to move along the path
	 * */
	public MPoint addMPoint(ArrayList<Line> linelist,
	...
	/**
	 * Analyzes the relation result in the secondo resultlist for implemented
	 * datatypes and puts the found objects into the resultTypeList
	 * 
	 * @param le
	 *            The resultlist of the secondo query
	 * */
	public void analyzeRelation(ListExpr le) {
		...
		while (!values.isEmpty()) {
			...
			while (!listentry.isEmpty()) {
				...
				if (tuplelist.first().second().stringValue().equals("point")) {
					...
					// add location
					Double x = listentry.first().first().realValue(); // longitude
																		// = X
					point.setX(x);
					Double y = listentry.first().second().realValue();// latitude
																		// = Y
					point.setY(y);
				...
				if (tuplelist.first().second().stringValue().equals("line")) {
					...
					while (!lineList.isEmpty()) {
						...
						// add location
						Double xA = lineList.first().first().realValue(); // longitude
																			// =
																			// X
						pointA.setX(xA);
						Double yA = lineList.first().second().realValue();// latitude
																			// =
																			// Y
						pointA.setY(yA);

						Double xB = lineList.first().third().realValue(); // longitude
																			// =
																			// X
						pointB.setX(xB);
						Double yB = lineList.first().fourth().realValue();// latitude
																			// =
																			// Y
						pointB.setY(yB);
					...
					while (!outerlineList.isEmpty()) {
						...
						while (!innerlineList.isEmpty()) {
							...
							while (!lineList.isEmpty()) {
								...
								// add location
								Double x = lineList.first().first().realValue(); // longitude
																					// =
																					// X
								point.setX(x);
								Double y = lineList.first().second()
										.realValue();// latitude = Y
								point.setY(y);
				...
				if (tuplelist.first().second().stringValue().equals("mpoint")) {
					...
					while (!valueList.isEmpty()) {
						...
						// get time
						Date timeA = parseStringToDate(valueList.first()
								.first().first().writeListExprToString());
						Date timeB = parseStringToDate(valueList.first()
								.first().second().writeListExprToString());
						...
						Point pointA = new Point();
						Point pointB = new Point();
						Double xA = valueList.first().second().first()
								.realValue(); // longitude = X1
						pointA.setX(xA);
						Double yA = valueList.first().second().second()
								.realValue();// latitude = Y1
						pointA.setY(yA);

						Double xB = valueList.first().second().third()
								.realValue(); // longitude = X2
						pointB.setX(xB);
						Double yB = valueList.first().second().fourth()
								.realValue();// latitude = Y2
						pointB.setY(yB);
					...
					mpoint.setAttributeNameInRelation(tuplelist.first().first()
							.stringValue());
				}
				...
				if (tuplelist.first().second().stringValue().equals("mlabel")) {
					...
					// name - attribute name of type mlabel in the relation
					mlabel.setAttributeNameInRelation(tuplelist.first().first()
							.stringValue());
		...
		System.out.println("############## Size of resultTypeList: "
				+ resultTypeList.size());
	}

	/**
	 * Gets the line-result from secondo, builds a list of lines and puts it
	 * into the resultTypeList
	 * 
	 * @param le
	 *            The secondo result containing a line object
	 * */
	...
	/**
	 * Gets the region-result from secondo, builds a polyline and puts it into
	 * the resultTypeList
	 * 
	 * @param le
	 *            The secondo resultlist containing a region
	 * */
	...
	/**
	 * Gets the mpoint-result from secondo, builds a pointlist and a timelist
	 * and puts it into the resultTypeList
	 * 
	 * @param le
	 *            The secondo resultlist containing a moving point
	 * */
	public void analyzeMPoint(ListExpr le) {
		...
		while (!values.isEmpty()) {
			...
			// get time
			Date timeA = parseStringToDate(values.first().first().first()
					.writeListExprToString());
			Date timeB = parseStringToDate(values.first().first().second()
					.writeListExprToString());
			...
			Point pointA = new Point();
			Point pointB = new Point();
			Double xA = values.first().second().first().realValue(); // longitude
																		// = X1
			pointA.setX(xA);
			Double yA = values.first().second().second().realValue();// latitude
																		// = Y1
			pointA.setY(yA);

			Double xB = values.first().second().third().realValue(); // longitude
																		// = X2
			pointB.setX(xB);
			Double yB = values.first().second().fourth().realValue();// latitude
																		// = Y2
			pointB.setY(yB);
	...
	/**
	 * Gets a time string from the secondo result and parses it into a java date
	 * object
	 * 
	 * @param time
	 *            The time string from the secondo result
	 * @return The date object
	 * */
	public Date parseStringToDate(String time) {

		SimpleDateFormat formatterMinutes = new SimpleDateFormat(
				"yyyy-MM-dd-HH:mm", Locale.GERMAN); // length with minutes only:
													// 16
		SimpleDateFormat formatterSeconds = new SimpleDateFormat(
				"yyyy-MM-dd-HH:mm:ss", Locale.GERMAN); // length with seconds:
														// 19
		SimpleDateFormat formatterMilliseconds = new SimpleDateFormat(
				"yyyy-MM-dd-HH:mm:ss.SSS", Locale.GERMAN); // length: 21-23
	...
	/**
	 * Adds the list of attributes to the given relation datatype from the
	 * current result
	 * 
	 * @param le
	 *            The current secondo result list
	 * */
		...
		if (le.first().atomType() != ListExpr.SYMBOL_ATOM
			...
			while (!values.isEmpty()) {
				...
				while (!listentry.isEmpty()) {
					...
					// if
					// (tuplelist.first().second().stringValue().equals("line")
					// ||
					// tuplelist.first().second().stringValue().equals("region")
					// ||
					// tuplelist.first().second().stringValue().equals("mpoint")){
					// datatype.getAttributeList().add(tuplelist.first().first().stringValue().trim()
					// + " : " + "(geometry) \n \n");
					// }
					//
					// else{
					datatype.getAttributeList().add(
							tuplelist.first().first().stringValue().trim()
									+ " : "
									+ listentry.first().writeListExprToString()
											.trim()); // Ort : (8.798653
														// 53.08336)
					// }
