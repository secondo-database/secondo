...
Feb. 2006 M. Spiekermann. The TestRunner is now a command option for SecondoTTY
and not an stand alone program. This saves time for linking and saves diskspace,
since both applications hav minimal differences.
...
Main routine for ~SecondoBDB~ and ~SecondoCS~. This source file will be compiled
twice. The resulting object files will be used to link together ~SecondoBDB~ and
~SecondoCS~. There will be some shell scripts which invoke the above programs
...
public:
  virtual bool handleMsg(NestedList *nl, ListExpr list,
    ...
    if (nl->HasLength(list, 1)) {
      ...
      case TextType:
        std::cout << nl->TextValue(list);
        break;
      case IntType:
        std::cout << nl->IntValue(list);
        break;
      case RealType:
        std::cout << nl->RealValue(list);
        break;
      default:
