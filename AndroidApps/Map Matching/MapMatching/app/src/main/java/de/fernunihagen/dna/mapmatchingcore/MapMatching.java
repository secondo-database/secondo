...
//    private long completeTimeProcessingLocationPoints = 0;
    ...
    private boolean checkExtensionBasedOnPastLocations(Point locationPoint, MapMatchingPath path, NetworkEdge edge) {
        int indexOfLastEdge = path.getEdges().size() - 1;
        if (path.getHistoryPerEdge().get(indexOfLastEdge).size() != 0) {
            boolean distanceOfEdgeComparedToPoints = util.distanceOfEdge(edge) < util.distanceOfPointsMappedToLastEdge(locationPoint, path);
            if (distanceOfEdgeComparedToPoints || util.percentalSpanOfProjectedPointsMappedToLastEdge(path) > extensionPercentalSpan || util.angleOfPointsComparedToAngleOfEdge(locationPoint,path)) {
                return true;
            }
        }
        return false;
