...
public class UpdateViewerController implements ActionListener
{
	...
	/*	
	 * Executes deletecommands generated for all relations in one transaction. 
	...
	private boolean executeDelete()
	{
		...
		// reload changed relations
		for (RelationPanel rp : this.viewer.getRelationPanels())
        {
			if (rp.getDeleteTuples() != null || !rp.getDeleteTuples().isEmpty())
			{
				this.loadRelation(rp.getName());
			}
		}
