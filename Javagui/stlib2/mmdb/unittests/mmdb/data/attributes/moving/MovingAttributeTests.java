...
public class MovingAttributeTests {
   ...
   public static void init() throws ParseException {
      ...
      lists.add(ListExpr.twoElemList(TestUtilAttributes.getUboolList(), TestUtilAttributes.getUboolList()));
      lists.add(ListExpr.twoElemList(TestUtilAttributes.getUintList(), TestUtilAttributes.getUintList()));
      lists.add(ListExpr.twoElemList(TestUtilAttributes.getUpointList(), TestUtilAttributes.getUpointList()));
      lists.add(ListExpr.twoElemList(TestUtilAttributes.getUrealList(), TestUtilAttributes.getUrealList()));
      lists.add(ListExpr.twoElemList(TestUtilAttributes.getUregionList(), TestUtilAttributes.getUregionList()));
      lists.add(ListExpr.twoElemList(TestUtilAttributes.getUstringList(), TestUtilAttributes.getUstringList()));
   }
   ...
   public void testFromList() throws ConversionException {
      for (int i = 0; i < attributes.size(); i++) {
         ...
         assertEquals(2, attribute.getUnitsMMDB().size());
      }
   }
   ...
   public void testGetPeriods() {
      for (int i = 0; i < attributes.size(); i++) {
         MovingAttribute attribute = attributes.get(i);
         List<Period> periods = attribute.getPeriodList();
