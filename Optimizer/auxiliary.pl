% This file constains the pretty-printing predicate
% pretty_print and various auxiliary predicates for
% pretty_print and a secondo predicate which uses just
...
% write a tuple in one single line
write_element(X, N) :-
  is_atomic_list(X),
  !,
  write_tabs(N),
...
% nl_pretty prints a list L which is assumed to
