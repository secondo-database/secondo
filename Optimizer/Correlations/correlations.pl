...
findRelation(SearchFor, [ Relation | _ ]):- % Relation found
  ...
  debug_writeln(findRelation/2, 71, 
      [ 'found/equals ', SearchFor, ' in/and ', Relation ]).
...
% just for debugging issues
set_equal(Set1, Set2):-
  debug_writeln(set_equal/2, 80, [ 'sets ', 
...
add_relation(Relation):-
  ...
  debug_writeln(add_relation/1, 62, [ Relation, 
            ' is still a part of listOfRelations ', 
            ListRelations, ' (do not apped)' ]),
...
%Q%  listOfRelations(OldList),
  ...
  debug_writeln(add_relation/1, 61, [ 
        ' append listOfRelations ', OldList, ' by ', Relation ]),
  !.
% unexpected call
add_relation(A):-
...
%Q%  listOfPredicates(OldList),
  ...
  debug_writeln(add_predicate/1, 61, 
      [ ' append listOfPredicates ', OldList, ' by ', Predicate ]),
  !.
% unexpected call
add_predicate(A):-
...
%?%  !, kann dann "unexpected call" noch erreicht werden?
  ...
  debug_writeln(build_list_of_predicates/0, 31, 
      [ 'predicates found ', PredList]),
...
build_list_of_predicates2([ [ PredIdent,Pr ] | Rest ]):-
  ...
  debug_writeln(build_list_of_predicates2/1, 41, 
        [ 'predicate tuple ', [PredIdent,Pr], ' evaluated' ]),
  build_list_of_predicates2(Rest).

build_list_of_predicates2([ [ PredIdent,Pr ] | Rest ]):-
  debug_writeln(build_list_of_predicates2/1, 40, 
       [ 'predicate tuple ', [PredIdent,Pr], ' not supported - ignore it']),
  build_list_of_predicates2(Rest).

% unexpected call
build_list_of_predicates2(A):-
  error_exit(build_list_of_predicates2/1, 
        [ 'unexpected call with argument ', A ]).
...
build_list_of_predicates3(PredIdent, PredExpr, Relation):-
  ...
  debug_writeln(build_list_of_predicates3/3, 59, 
         [ 'predicate id ', PredIdent, ' is transformed to ', PredIdent2]),
  (
    ...
    add_predicate(Predicate),
    debug_writeln(build_list_of_predicates3/2, 51, [ 'relation ', 
                     Relation, ' and predicate ', Predicate, ' was added'])
  ),
  !.

% unexpected call
build_list_of_predicates3(A, B):-
  error_exit(build_list_of_predicates3/2, 
          [ 'unexpected call with arguments ', [ A, B ] ]).
...
get_predicate_contents(Predicate, PredExpr, Relation):-
  ...
  debug_writeln(get_predicate_contents/3, 61, 
        [ 'select predicate found ', Predicate, ' => ', [PredExpr, Relation]]).
...
get_predicate_contents(Predicate, PredExpr, Relation):-
  ...
  debug_writeln(get_predicate_contents/3, 62, 
        [ 'join predicate found ', Predicate, ' => ', [PredExpr, Relation]]).
...
get_predicate_contents(Predicate, PredExpr, Relation):-
  ...
  Relation = [ Relation1, Relation2 ],
  debug_writeln(get_predicate_contents/3, 62, 
  [ 'join predicate evaluated ', Predicate, ' => ', [PredExpr, Relation]]).
...
get_predicate_contents(Predicate, PredExpr, Relation):-
  ...
  Relation = [ Relation1, Relation2 ],
  debug_writeln(get_predicate_contents/3, 62, 
        [ 'join predicate evaluated ', Predicate, ' => ',
        [PredExpr, Relation]]).

% other unsupported type of predicate
get_predicate_contents(Predicate, _, _):-
  info_continue(get_predicate_contents/3, 
        [ 'unsupported type of predicate ', Predicate, ' - ignore it']),
  fail.
...
remove_obsolete_queries([], _, QueryList, QueryList):-
  debug_writeln(remove_obsolete_queries/4, 41, 
       [ 'end of recursion ', [[], 'unknown', QueryList, QueryList] ]).

% obsolete query -> remove query
remove_obsolete_queries([ QueryDesc | RQuery ], Keep, RQueryRev, QueryList):-
  QueryDesc = [ QueryPreds, _, _],
  debug_writeln(remove_obsolete_queries/4, 42, [ 'is ', 
         QueryPreds, 'a subset of ', Keep, '?' ]),
  %VJO falsche REihenfolge: subset(QueryPreds, Keep),
  subset(Keep, QueryPreds),
  debug_writeln(remove_obsolete_queries/4, 42, 
         [ 'query ', QueryDesc, ' removed' ]),
...
remove_obsolete_queries([ QueryDesc | RQuery ], Keep, RQueryRev, QueryList):-
  ...
  debug_writeln(remove_obsolete_queries/4, 43, 
     [ 'query ', QueryDesc, ' kept' ]),
  remove_obsolete_queries(RQuery, KeepNew,
            [ QueryDesc | RQueryRev ], QueryList).

% unexpected call
remove_obsolete_queries(A, B, C, D):-
  error_exit(remove_obsolete_queries/4, 
         ['unexpected call with arguments ',[A, B, C, D]]).
...
% current element isn't a identifier of a join predicate -> 
...
build_queries([ Relation | RRest], [ Query | QRest]):-
  ...
  debug_writeln(build_queries/2, 41, [ 'query', 
           Query, ' built for relation ', Relation ]),
...
storeCorrPredicates([ [ PredID, PredRel, PredExpr ] | ListOfPredicates ]):-
  ...
  debug_writeln(storeCorrPredicates/1, 51,
        [ 'fact ', corrPredicates(PredID, PredRel, PredExpr), 'stored']),
...
build_query(Relation, [ PredIdentList , Query , Relation ]):-
  ...
  debug_writeln(build_query/2, 59, [ 'relation expression ', 
           RelationExpr, ' built for relation ', Relation, 
           ' using join predicates ', UsedJoinPredicateIDs]),
  !,
  ...
  debug_writeln(build_query/2, 59,
           ['final tanslation list is ', PredIdentList]),
  !,
  % concatinate string
...
%  Query , RelPred ], ' built for relation ', Relation]).
  debug_writeln(build_query/2, 51, [ 'query tuple ', [ PredIdentList , 
         Query , Relation ], ' built for relation ', Relation]),
...
% if Option correlationsJoin is set - applicate operator predcount
%  on a derivated relation
build_relation_expr(Relation, JoinPredicates, RelationExpr, 
  ...
  CountSelPreds > 0,
%?%  is_list(Relation), wie sollte ein join pred definiert 
        % werden ueber eine relationsinstanz
  %!, no cut here - it is possible, that no set of join predicates 
        % joins all relations
  % in that case get_all_join_sets/3 fails (that's correct) and 
        %a product of relations should be used
  (
    get_all_join_sets(Relation, JoinPredicates, JoinSets),
    not(JoinSets=[])
    ->
    debug_writeln(build_relation_expr/5, 69, 
                   [ 'set of possible sets of join predicates for relation ', 
                   Relation, ' is ', JoinSets]);
    warn_continue(build_relation_expr/5, 
                  [ 'no set sufficient join predicates for relation ', 
                     Relation, ' found - use product']),
    fail
  ),
  %!,  if its not possible to construct a sample query
  % repeat until a joinset and path is found which is translatable
        %  to a sample query
  get_smallest_join(JoinSets, SmallestJoinSet, _),
  debug_writeln(build_relation_expr/5, 69, 
             [ 'smallest set of join predicates for relation ', 
                Relation, ' is ', SmallestJoinSet]),
  %!, if its not possible to construct a sample query
...
build_relation_expr(A, B, C, D, E):-
  error_exit(build_relation_expr/5,
         ['unexpected call with arguments ',[A, B, C, D, E]]).

% if Option correlationsProduct is set or
% if Option correlationsJoin is set and no sufficient set of join
% predicates were found to build the derivated relation
...
build_relation_expr2([ LastRelation | [] ], _, RelationExpr, []):-
  ...
  debug_writeln(build_relation_expr2/4, 69, ['use sample ',
            NameSample,' instead of relation ',LastRelation]),
...
%?%  !, % keine Wirkung von unexpected call
  ...
  debug_writeln(build_relation_expr2/4, 62, 
                [ '(Option joinCorrelations* set) last relation ', 
                LastRelation, ' added as ', RelName, ' with alias ', 
                RelAliasString, ' to product']).

% recursion
build_relation_expr2([ Relation | OtherRelations ], 
                    JoinPredicates, RelationExpr, JoinPredicates2):-
  (
    optimizerOption(joinCorrelations);
    optimizerOption(joinCorrelations2)
  ),
  ...
  debug_writeln(build_relation_expr2/4, 69, 
                ['use sample ',NameSample,' instead of relation ',Relation]),
  Sample = rel(NameSample, RelAlias),
%?%  !, % keine Wirkung von unexpected call
  plan_to_atom(Sample, RelName), % determine relation
  % build alias if necessary
  build_alias_string(Sample, RelAliasString),
  build_relation_expr2(OtherRelations, JoinPredicates, 
                             RelationExprTmp, JoinPredicates2),
  atom_concat(RelationExprTmp, RelName, ' feed ', RelAliasString,
    ' product ', RelationExpr), % concatinate string
  debug_writeln(build_relation_expr2/4, 62, 
                   [ '(Option joinCorrelations* set) relation ', 
                     Relation, ' added as ', RelName, ' with alias ', 
                     RelAliasString, ' to product']).
...
%?%  !, % keine Wirkung von unexpected call
  ...
  debug_writeln(build_relation_expr2/4, 63, [ 'singleton relation ', 
             Relation, ' used as ', RelName, ' with alias ', RelAliasString]).

% unexpected call
build_relation_expr2(A, B, C, D):-
  error_exit(build_relation_expr2/4, 
                 ['unexpected call with arguments ',[A, B, C, D]]).
...
build_alias_string(rel(_,*), ''):-
  Relation = rel(_,*),
  debug_writeln(build_alias_string/2, 71, 
                [ 'no alias given for relation ', 
...
build_alias_string(rel(_,RelAlias), AliasString):-
  ...
  debug_writeln(build_alias_string/2, 72, 
                [ ' alias given for relation ', Relation, ' is ', RelAlias]).
...
build_predcount_exprs(Predicates, PredcountString, PredIdentList):-
  ...
  debug_writeln(build_predcount_exprs/3, 69, 
             [ 'list of predicates ', ListOfPredicates, 
               ' for predcount expression ']),
%?%  !, wegen unexpected call raus
  ...
  atom_concat(' predcounts [ ', PredcountExpr, ' ] ', '', '',
    PredcountString),
  debug_writeln(build_predcount_exprs/3, 61,
             [ 'predcount expression ', PredcountString, 
               ' and translation list ', PredIdentList, 
               ' for list of predicates ',ListOfPredicates]).

% unexpected call
build_predcount_exprs(A, B, C):-
  error_exit(build_predcount_exprs/3, 
...
get_join_predicates2(Relation, [ Predicate | Predicates ], 
             [ [PredID] | JoinPredicateIDs ], [ Predicate | JoinPredicates ]):-
  debug_writeln(get_join_predicates2/4, 9999, 
              [ 'check predicate ', Predicate, ' and relation ', Relation]),
  Predicate = [ PredID, PredRelation, pr(_, _, _) ],
  debug_writeln(get_join_predicates2/4, 9999, [ 'is ', 
            Relation, '  a part of ', PredRelation, ' ?']),
  % korrekte Reihenfolge der PArameter - wenn alle RElationen im Praedikat
       %  bettrachtet werden, verbindet das PRaedikat die Relationen
  subset(PredRelation, Relation),
  !,
  get_join_predicates2(Relation, Predicates, JoinPredicateIDs, JoinPredicates).
get_join_predicates2(Relation, [ _ | Predicates ], 
                     JoinPredicateIDs, JoinPredicates):-
  get_join_predicates2(Relation, Predicates, JoinPredicateIDs, JoinPredicates).
...
get_sel_predicates2(Relation, [ Predicate | Predicates ], 
                    [ PredID | SelPredicateIDs ],
                    [ Predicate | SelPredicates ]):-
  ...
  !,
  get_sel_predicates2(Relation, Predicates, SelPredicateIDs, SelPredicates).
get_sel_predicates2(Relation, [ _ | Predicates ], 
                    SelPredicateIDs, SelPredicates):-
  get_sel_predicates2(Relation, Predicates, SelPredicateIDs, SelPredicates).
...
get_all_join_sets(Relation, JoinPredicates, JoinSets):-
  debug_writeln(get_all_join_sets/3, 9999, 
              ['actual params: ',[Relation, JoinPredicates, JoinSets]]),
...
get_join_set0(Relation, JoinPredicates, [], JoinSet):-
  debug_writeln(get_join_set0/4, 9999, ['actual params: ',
             [Relation, JoinPredicates, [], JoinSet]]),
  get_join_set(Relation, JoinPredicates, [], JoinSet).

get_join_set(Relations, _, JointRelations, []):-
  set_equal(Relations, JointRelations),
  debug_writeln(get_join_set/4, 9999, ['all relations involved: ',
              [Relations, JointRelations]]),
  !.
get_join_set(Relations, [ JoinPredicate | JoinPredicates ], JointRelations, 
              [ JoinPredicateID | JoinSet ]):-
  ...
  debug_writeln(get_join_set/4, 9999, ['add PredRelations ', 
             PredRelations, ' to JointRelations ', JointRelations]),
...
6.5.4.5 get\_cheapest\_path/2
 ...
 The implementation using get\_first\_path/2 supports searching of the cheapest 
 path which is translateable to a sample query.
...
get_path_cost(NodeID2, NodeID, [], [], 0):-
  NodeID2 =:= NodeID,
  debug_writeln(get_path_cost/5, 9999, ['cost of path from node ',
                      NodeID2, ' to ', NodeID, ' is 0']).
get_path_cost(LastNodeID, TargetNodeID, [ CostEdge | CostEdges ], 
  ...
  debug_writeln(get_path_cost/5, 9999,
               ['estimate cost for planEdge from ', LastNodeID, 
               ' to ', NewNodeID]),
...
setSampleToBeUsed(Relation, 2):-
  ...
  % if neccesary create sample (the name of created sample have 
        % to be equal to NameSample !!!)
...
setSampleToBeUsed(Relation, 1):-
  ...
  debug_writeln(setSampleToBeUsed/2, 9000, ['register sample ',
              DCNameSample,' for relation ',Relation,
              ' (stored as sampleToBeUsed/2)']).
% other usages of relation - use relation itself (another implementation: 
% create new samples)
...
construct_sample_plan(Path, Plan):-
  debug_writeln(construct_sample_plan/2, 9999,
        [ 'construct plan of path: ', Path]),
  plan(Path, TmpPlan),
  debug_writeln(construct_sample_plan/2, 9999, 
             [ 'temporary plan contructed: ', TmpPlan]),
  ...
  debug_writeln(construct_sample_plan/2, 9999, 
          [ 'final plan constructed: ', Plan]).

replace_relations_by_samples(TmpPlan, Plan):-
  prepare_sample_query(TmpPlan, Plan).

replace_relations_by_samples(Plan, Plan):-
  warn_continue(replace_relations_by_samples/2, 
            [ 'unable to use samples for ', Plan ]).

prepare_sample_query(In, Out):-
  query_sample(In, Out),
  !.
prepare_sample_query(In, Out):-
  warn_continue(prepare_sample_query/2, 
           ['unable to build sample query ', [In, Out]]),
  !,
  fail.
...
query_sample(IndexName, _) :-
  ...
  warn_continue(query_sample/2,['found Index ', DCindexName, 
...
query_sample(rel(Name, V), rel(NameSample, V)) :-
  ...
  debug_writeln(query_sample/2, 9000, ['use sample ',
             NameSample,' instead of relation ',rel(Name, V)]),
  !.

query_sample( rel(Name, V), rel(Name, V) ) :-
  atomic(rel(Name, V)),
  warn_continue(query_sample/2, ['no sample of relation ',
               rel(Name, V),' usable - use relation itself']),
  !.
...
calculate_predcounts([[PredIDs , Query , Relation ] | QRest],
  ...
  (
    ...
    ->
    debug_writeln(calculate_predcounts/2, 39, 
                [ 'add number of tuples of product of ', 
                Relation, ', which do not meet the join predicate']),
    get_card_product(Relation, CardProduct),
    debug_writeln(calculate_predcounts/2, 39, [ 
                 'cardinality of product of all relations ', 
                  Relation, ' is ', CardProduct]),
    get_sum_tuples(PredCount, SumTuples),
    debug_writeln(calculate_predcounts/2, 39,
                [ 'sum of all predcount tupels ', PredCount,
                   ' is ', SumTuples]),
...
calculate_predcounts(A, B):-
  error_exit(calculate_predcounts/2, 
       ['unexpected call with arguments ',[A, B]]).
...
get_sum_tuples([ [ _ , Count] | Rest ], SumTuplesNew):-
  ...
  debug_writeln(get_sum_tuples/2, 41, [Count, ' added to ', 
             SumTuplesOld, ' is ', SumTuplesNew]).
% unexpected call
get_sum_tuples(A, B):-
...
get_card_product(RelationList, CP):-
  ...
  debug_writeln(get_card_product/2, 41, 
            ['product of cardinalities of ', RelationList, ' is ', CP]).
get_card_product(Relation, Card):-
  get_card_relation(Relation, Card),
  debug_writeln(get_card_product/2, 43, ['cardinality of ', 
             Relation, ' is ', Card]).
% unexpected call
...
calculate_pog_ident(InAtom, [ [ JoinId ] | TRest ], OutAtom):-
  ...
  debug_writeln(calculate_pog_ident/3, 53, ['add join id ', 
               JoinId, ' to id => new id ', OutAtom]).
...
equals_or_contains(Relation, Relation2):-
  ...
  debug_outln('equals_or_contains: Relation of Predicate \c
equals Relation of query to build').
...
addCorrelationSizes:-
  ...
  debug_writeln(addCorrelationSizes/0, 9, 
            ['predcount queries built ', Queries]),
  !,
  ...
  debug_writeln(addCorrelationSizes/0, 9, 
            ['node ids corrected ', PCResultsNew]),
  !,
  ...
  debug_writeln(addCorrelationSizes/0, 9,
             ['facts corrCummSel/2 created: ', DEBUG]),
  !,
...
storeCorrCummSel([ TupleList | RList ]):-
  ...
  debug_writeln(storeCorrCummSel/1, 39, ['predcount results ', TupleList, 
    ' stored as facts']),
  !,
  % aus PCResults fuer Option entropy die kummulierten Sels berrechnen und 
  % als corrCummSel/2 speichern
  storeCummSel,
...
storeCummSel3(NodeID, NodeCard, BaseCard):-
  ...
  debug_writeln(storeCummSel3/3, 52, ['corrCummSel(',
       [NodeID, CummSel], ') asserted']).
  %format(atom(DEBUG),'~20f',CummSel),
  %debug_writeln(storeCummSel3/3, 52, ['corrCummSel(',
  % [NodeID, DEBUG], ') asserted']).
...
storePCResults([ [ NodeID, _ ] | RList ], PCList):-
  ...
  debug_writeln(storePCResults/2, 41,['corrected node cardinality ', 
      CorrectCard, ' asserted for node ', NodeID, ' as fact storedPCTuple/2']),
  !,
...
resetPOGSizes :-
  findall([Source, Target, Term, Result], 
          edge(Source, Target, Term, Result, _, _), Edges),
...
resetNodeSize(NodeID, Size) :-
  ...
  debug_writeln(resetNodeSize/2, 51, ['size of node ', 
      Node, ' updated to ', Size]).
resetNodeSize(NodeID, Size) :-
  Node is NodeID + 0.0, % NodeID has to be real
  setNodeSize(Node, Size),
  debug_writeln(resetNodeSize/2, 53, ['size ', Size, 
     ' of node ', Node, ' inserted']).
resetNodeSize(NodeID, Size) :-
...
6.5.4
  ...
  The predicate checks the known relative node cardinalities 
  stored as facts corrCummSel/2.
  If the set of cardinalities not consistent the predicate 
  generates a consistent set.
...
correctNodeCards(CurrentNode):-
  ...
  debug_writeln(correctNodeCards/1, 59, 
      ['node ',Curr,': card lists of successors: ',SuccCardLists]),
  !,
  % block 4 (lines 040 - 060)
  mergeCardLists(SuccCardLists, MergedList),
  debug_writeln(correctNodeCards/1, 59, ['node ',
             Curr,': merged card lists: ',MergedList]),
  !,
  ...
  debug_writeln(correctNodeCards/1, 59, ['node ',
      Curr,': calculated lower limit of card is: ',DEBUG]),
  !,
  ...
  debug_writeln(correctNodeCards/1, 59, ['node ',
                Curr,': set card list: ',[ Tuple | MergedList]]),
  !,
  NextNode is CurrentNode - 1.0,
...
mergeCardLists(SuccCardLists, MergedList):-
  ...
  debug_writeln(mergeCardLists/2, 69, ['merge list ', 
             SuccCardLists,' with ', CardListLen,' levels']),
...
mergeCardLists(SuccCardLists, [ MergedTuples | MergedList ],
               Level, HighestLevel):-
...
sumLimits([ SuccCardList | SuccCardLists ], Level, 
          SumLowerLimit, SumUpperLimit):-
...
calculateLowerLimit([ [ LowLim, _ ] | MergedList ], LowerLimit):-
  ...
  !,
  % the expression -LowerLimitOld produces the alternating 
  % signs for the odd/even levels
...
checkConsistencyAndCorrectNodeCard(Curr, LowerLimit):-
  ...
  !,
  debug_writeln(checkConsistencyAndCorrectNodeCard/2, 69, 
      ['node ',Curr,' check consistency of card against max of set ',
...
resetNodeCardRange(Curr, NewLowerLimit, StoredUpLim):-
  NewLowerLimit =< StoredUpLim,
   debug_writeln(resetNodeCardRange/3, 79,['node ',Curr,
     ': cardinality range consistent: ', [Curr, NewLowerLimit, StoredUpLim]]).
resetNodeCardRange(Curr, NewLowerLimit, StoredUpLim):-
  NewLowerLimit > StoredUpLim,
