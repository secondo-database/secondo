...
----
   ...
   (5a) sql select s:sname from[plz as p, staedte as s ]where [p:plz=37263, p:ort=s:sname].
      
    (6) An example where an index join is better than standard optimizer expects:
    ...
    query 
      Staedte  pfeed[100]  puse[. project[SName] {s} ] 
      plz pjoin1[ 
           symj: . .. feed[0.1, 1000] <exp1> <sel1> pcount[500] symmjoin[(.SName_s = ..Ort_p)], 
             hj: . .. feed <exp1> <sel1> hashjoin[SName_s, Ort_p, 997],  
             smj: . .. feed <exp1> <sel1> sortmergejoin[SName_s, Ort_p], 
            ilj: . loopjoin[plz_Ort .. exactmatch[.SName_s] <exp1> <sel1>]
      ]  
      pdelete  count
...
which reads in a maximum of 10% of the input relation or at least 1000 tuples
...
registerPossibleIndexJoins(L) :-
  retractall( possibleIndexJoin(_) ),
  findall(P, getJoinPred(P), L),
  writeln(L),
  checklist(checkForIndex, L),
  findall(P2, possibleIndexJoin(P2), L2),
  writeln(L2).
