...
\sql{select * from $R_i$ where $C_n$ $\leq$ (select $max(C_m)$ from $R_j$ where P)}
...
\sql{$C_n$ $\theta$ all(select $C_m$ from $R_i$ where P)} wird für $\theta \in {<,<=,>,>=}$ transformiert in \sql{$C_n$ $\theta$ (select AGGR($C_n$) from $R_i$ where P)}. Wobei hier gilt
\begin{math}
AGGR = 
\begin{cases}
...
\sql{select * from $R_i$ where $C_n$ $\leq$ all(select $C_m$ from $R_j$ where P)}
...
Ein Prädikat \sql{$C_n$ = all(select $C_m$ from $R_i$ where P)} hat keine sinnvolle Semantik mehr, sobald die geschachtelte Abfrage mehr als einen Wert für $C_m$ liefert. Die Verwendung von = all(Q) wird daher nicht unterstützt.
...
entspricht hier dem Anti-Join-Operator
