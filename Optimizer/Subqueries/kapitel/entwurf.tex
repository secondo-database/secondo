...
Da der Operator \sql{in} für Tupel noch nicht implementiert ist, wurde die triviale Umsetzung von \sql{in} in die Disjunktion der angegebenen Konstanten als Übersetzung integriert. Eine Implementierung des \sql{in}-Operators für Tupelströme auf Basis \secondo{in}-Operators der \algebra{Collection}-Algebra wäre 
\begin{itemize}
...
Ziel des Algorithmus ist es, ein geschachteltes Prädikat in eine semantische äquivalente entschachtelte Form zu überführen. Dafür wird die Konjunktion der WHERE-Klauseln gebildet und um ein Join-Prädikat ergänzt, das aus dem ursprünglich geschachtelten Prädikat abgeleitet ist. 
