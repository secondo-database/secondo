...
\bottomrule\end{tabular}
 \caption{Laufzeit bei Skalierungsfaktor 0,0064}
...
Bis auf die in Kapitel \ref{chp:Entwurf} beschriebenen Einschränkungen lassen sich geschachtelte Abfragen jetzt mit dem Optimierer übersetzen. Das Ziel, die Auswahl der Ausführungsstrategie dem Optimierer zu überlassen, musste jedoch schon während der Analyse des Problems fallengelassen werden. Die hierfür notwendige Infrastruktur zu erstellen und die notwendigen mathematischen Grundlagen zu erarbeiten hätten den Rahmen dieser Arbeit gesprengt. Aus den ausgeführten Laufzeitvergleichen kann man jedoch schließen, dass die grundsätzliche Auswahl der Entschachtelungsstrategie wohl nur in wenigen Fällen zu einer deutlichen Verschlechterung der Laufzeit gegenüber der iterativen Ausführung führen wird. 
...
\section{Ausblick}
Der \textsc{Secondo}-Optimierer unterstützt bereits einen Großteil der im SQL-Standard definierten Syntax. Zu den wenigen fehlenden Elementen gehören die \sql{having}-Klausel und Left- sowie Right-Outer-Join-Operatoren. Eine Implementierung dieser Syntaxelemente wäre bedenkenswert, da diese Konstrukte in den meisten relationalen Datenbanksystemen verfügbar sind. 
