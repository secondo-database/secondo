...
Grundsätzlich besteht eine SQL-Abfrage aus den drei Klauseln SELECT, FROM und WHERE. In der SELECT-Klausel werden die Attribute der Ergebnisrelation angegeben. Die FROM-Klausel gibt die Quellrelationen an, aus denen das Ergebnis ermittelt wird. Die optionale WHERE-Klausel gibt schließlich Einschränkungen an, denen die Ergebnistupel genügen müssen.
	\subitem Die SELECT-Klausel entspricht der Projektions-Operation der Relationenalgebra. In ihr werden alle Attribute der Ergebnisrelation angegeben. Um alle Attribute der beteiligten Relationen zu erhalten gibt es die spezielle Notation \enquote{*}. Mit Ihr wird also das Schema der Ergebnisrelation festgelegt.
	\subitem Die FROM-Klausel entspricht dem kartesischen Produkt in der Relationenalgebra. Hier werden alle in der Abfrage verwendeten Relationen aufgeführt.
