...
template<bool expectSorted, bool outerJoin> int
mergeouterjoin_vm(Word* args, Word& result, 
                  int message, Word& local, Supplier s)
{
  typedef MergeOuterjoinLocalInfo<outerJoin> MergeInfo;
  typedef LocalInfo<MergeInfo> LocalType;
  LocalType* li = static_cast<LocalType*>( local.addr );
  ...
  switch(message)
  {
    ...
    case REQUEST: {
      ...
      if ( li->ptr == 0 )	//first request;
				...
				//which are a problem for progress estimation
      {
        li->ptr = new MergeOuterjoinLocalInfo<outerJoin>
          (args[0], args[4], args[1], args[5], expectSorted, s, li);
      }

      MergeOuterjoinLocalInfo<outerJoin>* mli = li->ptr;
      result.addr = mli->NextResultTuple();
      li->returned++;
...
template int
mergeouterjoin_vm<true, false>(Word* args, Word& result, int message, 
                 Word& local, Supplier s);

template int
mergeouterjoin_vm<false, false>(Word* args, Word& result, int message, 
                 Word& local, Supplier s);


template int
mergeouterjoin_vm<false, true>(Word* args, Word& result, int message, 
                 Word& local, Supplier s);
