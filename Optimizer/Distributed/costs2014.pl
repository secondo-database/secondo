...
%B.Huber
% Subsitute the arguments from dot for distributed costs2014 rules
cost(dot(Size, NAttrs, TSize), _, _, _, _, Size, NAttrs, TSize, 0).

% Subsitute the arguments from dotdot for distributed costs2014 rules
cost(dotdot(Size, NAttrs, TSize), _, _, _, _, Size, NAttrs, TSize, 0).
%B.Huber end
...
/*
2 Functions for call normal costs methods

When for a secondo command a costs2014 function is not defined, 
call the existing cost function. 

*/

cost(Term, Sel, Pred, Res, Mem,
      Card, NAttrs, TSize, Cost) :-
  !,
  assertOnce(isCosts2014Active),
  setCosts2014Param(Res, Mem, NAttrs, TSize),
  (   cost(Term, Sel, Pred, Card, CostOutput)
...
%When the normall cost-methods call costs2014 get the last parameter
getCosts2014Param(Result, Memory, NAttrs, TSize) :-
   isCosts2014Active,
   costs2014Param(Result, Memory, NAttrs, TSize).

%Save the necessary parameter for costs2014
setCosts2014Param(Result, Memory, NAttrs, TSize) :-
   retractall(costs2014Param(_, _,  _, _)),
   isCosts2014Active,
   assertOnce(costs2014Param(Result, Memory, NAttrs, TSize)).
...
3.2 Looking up constants determined in kernel system
