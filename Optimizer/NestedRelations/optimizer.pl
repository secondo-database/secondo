...
% not needed:
%rel_to_atom(rel(relsubquery(Query, SQID, TOP)), Atom) :-

% NVK ADDED 
/*
The thing is that we don't have here any clue wherefrom we can obtain this arel.
*/
rel_to_atom(rel(arel(_, _, _, RName), _), Atom) :-
  optimizerOption(nestedRelations),
	!,
	currentAlias(Alias),
	atomic_list_concat(['attr(', Alias,', ', RName, ')'], '', Atom).

rel_to_atom(rel(DCname, _), Atom) :-
  optimizerOption(nestedRelations),
	DCname=RELVAR:AREL,
	ground(RELVAR),
	ground(AREL), % Currently there is no need to support more.
	% then the case is no longer that simple, RELVAR is now a nrel relation
	% pointed by a variable RELVAR, or RELVAR is a variable bouded to a arel 
	% attribute.
	% XXX this is still not correct.
	%currentVariablesAll(_, [variable(RELVAR, rel(REL, RELVAR))]),
	findBinding(RELVAR, variable(RELVAR, rel(REL, RELVAR))),
	% XXX we need the full enviroment with the var bindings.
	appendAttribute(REL, AREL, PATH),
  dcName2externalName(PATH, ExtName), % ExtName is only the attribute name.

	%lastAttribute(DCname, DCLastAtt),

	% It is inevitable to extract a arel from a outer tuple variable that 
	% will be generated by the subqueries extension.
	% XXX RELVAR adds must be done recursivly.
	currentAlias(Alias),
	atomic_list_concat(['attr(', Alias,', ', ExtName, '_', RELVAR,')'], '', Atom).
% NVK ADDED NR END
...
% NVK ADDED NR
/*
plan_to_atom(subquery(Query, Rels), Result) :-
  optimizerOption(nestedRelations),
  subquery_plan_to_atom(subquery(Query, Rels), Result),
  !.
*/
% NVK ADDED NR END
...
% NVK ADDED
/*
Versions for nested relations.
The idea behind this is to allow terms like attr(o:p:bevt, _, _) to reflect the renaming when we process nested relations.
XXX Still to do: lookup generation of o:p:att terms.
*/
plan_to_atom(a(A:B:C, _, l), Result) :-
  optimizerOption(nestedRelations),
  attsToList(A:B:C, LST),
	reverse(LST, RLST),
  concat_atom(RLST, '_', Result),
  !.
% Example:
...
plan_to_atom(a(A:B:C, _, u), Result) :-
  ...
  attsToList(A:B:C, LST),
	reverse(LST, [F|RList]),
  upper(F, FUpper),
  concat_atom([FUpper|RList], '_', Result),
  !.
% NVK ADDED END
...
arg(N) => Stream2 :-
  argument(N, rel(relsubquery(Q, SQID, TOP, PS), Var)),
	PS=sqInfo(Stream, Costs, Size, TupleSize, ResAttrList),
 	addTransformationOperator(Stream, TOP, Stream1),
  (Var = * ->
    Stream2=Stream1
  ;
    Stream2=rename(Stream1, Var)
  ),
  !.%,
  %usedAttrList(rel(Name, Var), AttrNames).
...
arg(N) => feed(rel(Name, *)) :-
  isStarQuery,
  argument(N, rel(Name, *)), 
	atomic(Name), % NVK ADDED to distinguish between the afeed rules.
	!.
...
arg(N) => feedproject(rel(Name, *), AttrNames) :-
  argument(N, rel(Name, *)), %!, NVK MODIFIED
	% NVK ADDED
	atomic(Name), % distinguish between the afeed rules.
...
*/
assignSize(Source, Target, sortby(Arg1, Atts), Result) :-
...
resTupleSize(arg(N), TupleSize) :-
  optimizerOption(nestedRelations),
  argument(N, rel(relsubquery(Q, SQID, TOP, PS), _)),
...
*/
cost(rel(T, X1_), X2_, Pred_, Size, 0) :-
  optimizerOption(nestedRelations),
	T=..[arel|_], 
	!,
  card(T, Size).
...
*/
cost(unnest(X, Attr), Sel, P, S, C) :-
  cost(X, Sel, P, S, C).
cost(nest(X, Attr, NewLabel), Sel, P, S, C) :-
  cost(X, Sel, P, S, C).
% This is now needed, but i don't have any costs models for this operation
% so i just make this work.
cost(groupby(X, AttrNamesGroup, Fields), Sel, P, S, C) :-
  cost(X, Sel, P, S, C).
% NVK ADDED NR END
...
createCostEdge :- % use standard cost functions
  ...
  \+ optimizerOption(memoryAllocation), % NVK ADDED
  planEdge(Source, Target, Term, Result),
  edge(Source, Target, EdgeTerm, _, _, _),
  (   EdgeTerm = select(_, Pred)
    ; EdgeTerm = join(_, _, Pred)
...
successor(node(Source, Distance, Path), node(Target, Distance2, RPath)) :-
  ...
  append(Path, [costEdge(Source, Target, Term, Result, Size, Cost)], Path2),
    ...
    optimizeMemoryInPath(Path2, RPath),
...
:- op(935,  fx,  nonempty).
%:- op(930, xfx,  as).
:- op(930, xfy,  as). % NVK MODIFIED NR see testquery 821
:- op(930, xf ,  asc).
:- op(930, xf ,  desc).
...
/*
NVK ADDED NR
Unformtuatly we can't check if Rel is in the query.
Rel maybe a nested relation, or a variable.
RelVar has always to be a term hat isn't confusing prolog, so e.g. starts not with 
uppercase letters.

RelVar is usually a variable of a surrounding query. 
*/
lookupRel(RelVar:ARel as Var, Y) :-
  optimizerOption(nestedRelations),
  atomicCheck([RelVar, ARel, Var]),
  dcName2externalName(ARelDC, ARel), % get downcase spelling
  attributeToFQN(RelVar:ARelDC, FQN, ModAttr),
	(FQN=queryAttr(attr(Attr, Index, Case)) 
	; FQN=queryAttr(attr(Attr, Index, Case)):_),

	plan_to_atom(a(Attr, Index, Case), Atom),
  atomic_list_concat([Atom, '_', ModAttr], '', RName),

  %Y=rel(RelVar:ARelDC, Var),
  Y=rel(arel(RelVar:ARelDC, FQN, ModAttr, RName), Var),
  checkVarIsFree(Var),
  assert(variable(Var, Y)).

lookupRel(RelVar:ARel, Y) :-
  optimizerOption(nestedRelations),
	atomicCheck([RelVar, ARel]),
  dcName2externalName(ARelDC, ARel), % get downcase spelling
  attributeToFQN(RelVar:ARelDC, FQN, ModAttr),
	FQN=RelDC:_, % The first element is always the relation name.
  relation(RelDC, _), !,
  \+ duplicateAttrs(RelDC), % XXX this dosn't checks the arel attributes
	% attributes. need to be modiefied.
	%Y=rel(RelVar:ARelDC, *),

  findBinding(RelVar, variable(RelVar, rel(REL, RelVar))),
  % XXX we need the full enviroment with the var bindings.
  appendAttribute(REL, ARel, PATH),
  dcName2externalName(PATH, ExtName),
  atomic_list_concat([ExtName, '_', RelVar], '', RName),

	Y=rel(arel(RelVar:ARelDC, FQN, ModAttr, RName), Var),
  assert(queryRel(RelVar:ARelDC, Y)).
...
checkVarIsFree(Var) :-
	variable(Var, _),
	ErrMsg='variable is already used.',
 	throw(error_SQL(optimizer_checkVarIsFree(Var)::malformedExpression:ErrMsg)).
checkVarIsFree(_).
% NVK ADDED NR END
...
% NVK MODIFIED NR
lookupAttrs([A | As], Result) :-
  optimizerOption(nestedRelations),
  lookupAttr(A, A2),
  lookupAttrs(As, A2s), % A2s may now a list. Only allowed within queries.
	extendedAppend(A2, A2s, Result).
...
% NVK ADDED NR
lookupAttr(Var:Attr, attr(Attr2, 0, Case)) :-
  optimizerOption(nestedRelations),
	write('\nlookupAttr: '), write_list([Var, ':', Attr]), nl, %sl,
  atomic(Var),
  atomic(Attr),
	getCurrentAttrList(_, _, AttrDesc),
  downcase_atom(Attr, AttrDC),
	nrSimpleFindAttribute(AttrDC, AttrDesc, AD),
	AD=[_, Attr2, Case, DCFQN, SP, _, _, _],
  !,
  (Attr2=_:RestAttr2;Attr2=RestAttr2),
  % Var may point to a binding from an outer query.
  findBinding(Var, variable(Var, Rel2)),
	VA=0,
  (   usedAttr(Rel2, attr(RestAttr2, VA, Case))
    ; assert(usedAttr(Rel2, attr(RestAttr2, VA, Case)))
  ),
	write('\nfound: '), write_list([Attr2, ' -> ', Attr]), nl, %sl,
	true.

% May use backtracking
getCurrentAttrList(Var, RRel, AttrList) :-
  variable(Var, RRel),
  RRel=rel(relsubquery(Query2, SQID, _, PS), Var),
  PS=sqInfo(Stream, Costs, Size, TupleSize, AttrList).
...
lookupAttr(Var:Attr, attr(Attr2, 0, Case)) :- 
  ...
  atomic(Attr), 
	!,
  downcase_atom(Attr, AttrDC),
  appendAttribute(Var, AttrDC, VarN),
  attributeToFQN(VarN, FQN, ModAttr),
  spelled(FQN, attr(DCFQN, VA, Case)),
  % Modfiy now the attribute identifer to respect renamings in outer queries.
  lastAttribute(DCFQN, LastA),
  % Remember this for the renaming process.
  appendAttribute(ModAttr, LastA, Attr2),
	Attr2=_:RestAttr2,
...
attrSpelling(VarN, FQN, ModAttr, attr(DCFQN, VA, Case)) :-
  attributeToFQN(VarN, FQN, ModAttr),
  spelled(FQN, attr(DCFQN, VA, Case)).
...
lookupAttr(Attr, Attr2) :-
  ...
  atomic(Attr), %% changed code FIXME
  downcase_atom(Attr, AttrDC),
  isAttribute(AttrDC, QRel, Rel, ModAttr), !,
	appendAttribute(Rel, AttrDC, DCFQN),
  spelled(DCFQN, attr(FQN, X, Case)),
  lastAttribute(FQN, LastA),
  % Here we setup Attr2 to 
  appendAttribute(ModAttr, LastA, AttrRenamed),
	Attr2=attr(AttrRenamed, X, Case),

  queryRel(QRel, Rel2), % Without specific variable, the attributes never comes from an outer query.
  (   usedAttr(Rel2, Attr2)
...
*/
lookupAttr(Query, OutQuery) :- 
  optimizerOption(nestedRelations),
	simplifiedIsQuery(Query),
	!,
	lookupSubqueryAttr(Query, OutQuery).
...
lookupSubqueryAttr(Query, OutQuery) :- 
  throw(error_Internal(lookupSubqueryAttr(Query, OutQuery)::failed)).
...
/* 
NVK ADDED NR
Because the normal predicate won't work this way anymore for nested nested relations.
	
Note that now it is a little bit more complicated to retrieve the relation.	For example if we lookup the query:
select * from orteh as o
where [exists(select * from o:subrel where bevt>o:bevth)]
	
Then bevt is in o:subrel that has a fact queryRel(o:subrel, _), but there is no queryRel Fact, we have to look for a variable fact as well.  At the moment the from clause for the arel attributes has always the form
var:_.

	isAttribute(+DCAttr, -DCRel).
*/
isAttribute2(Name, Rel, RelOut, ModAttrIn, ModAttrOut) :-
  Rel=OuterVar:ARel,!, % note that if there is not : within Rel, this is
  findBinding(OuterVar, variable(OuterVar, rel(DCRel, OuterVar))),
  appendAttribute(ARel, Name, X),
  isAttribute2(X, DCRel, RelOut, ModAttrIn, ModAttrOut2),
	appendAttribute(OuterVar, ModAttrOut2, ModAttrOut).
  %atomic_list_concat([ModAttrOut2, '_', OuterVar], ModAttrOut).

isAttribute2(Name, Rel, RelOut, ModAttr, ModAttr) :-
  atomic(Rel), % NVK ADDED, if not atomic, the predicate above should be eval.
  relation(Rel, List),
  member(Name, List), !, 
  removeLastAttribute(Name, AREL),
  appendAttribute(Rel, AREL, RelOut).

isAttribute(Name, Rel, RelOut, ModAttr) :-
  optimizerOption(nestedRelations),
  queryRel(Rel, _), % Hence, outer query attributes can only be accessed with a variable access.
  isAttribute2(Name, Rel, RelOut, [], ModAttr).
	%write_list(['isAttribute(', Name, ',', RelOut, ',', ModAttr, ')']), nl, sleep(5).
	%atomic_list_concat([Name, ModAttrOut], ModAttr).
% NVD ADDED END
...
/*
NVK ADDED NR
Recognize and modifies the access on attributes within arel attributes. E.g. for queries
	select * from orteh as o where [exists(select * from o:subrel as p where p:bevt>o:bevth)]
we have the term p:bevt that leads to o:subrel. So we need to transform this to orteh:subrel:bevt to obtain the correct spelling. We do this by follwing the variable pointer into the directions of the outer queries.
*/
genericAttrLookup(Var:Attr, AT) :-
	write('\n...lookup...\n'), %sl,
  atomic(Var),
  atomic(Attr),
  downcase_atom(Attr, AttrDC),
  ...
  attributeToFQN(VarN, FQN, ModAttr),
  (
    FQN=queryAttr(attr(DCFQN, _, Case))
  ;
    FQN=queryAttr(attr(DCFQN, _, Case)):_
  ),
  % Modfiy now the attribute identifer to respect renamings in outer queries.
  lastAttribute(DCFQN, LastA),
  % Remember this for the renaming process.
  appendAttribute(ModAttr, LastA, Attr2),
	AT=attr(Attr2, _, Case),
	!.
...
lookupPred1(Var:Attr, attr(Attr2, Index, Case), RelsBefore, RelsAfter) :-
  optimizerOption(nestedRelations),
  write('\nlookupPred1: '), write_list([Var, ':', Attr]), nl,% sl,
  atomic(Var),
  atomic(Attr),
  getCurrentAttrList(Var, Rel2, AttrDesc),
  downcase_atom(Attr, AttrDC),
  nrSimpleFindAttribute(AttrDC, AttrDesc, AD),
  AD=[_, Attr2, Case, DCFQN, SP, _, _, _],
  !,
  ...
  write('\nfound Pred1: '), write_list([Attr2, ' -> ', Attr]), nl, %sl,
  !.


lookupPred1(Var:Attr, attr(Attr2, Index, Case), RelsBefore, RelsAfter) :-
  optimizerOption(nestedRelations),
	genericAttrLookup(Var:Attr, attr(Attr2, _, Case)),
  Attr2=_:RestAttr2,
  % Var may point to a binding from an outer query.
  findBinding(Var, variable(Var, Rel2)),
  ( member(Rel2, RelsBefore)
...
lookupPred1(Attr, attr(Attr2, Index, Case), RelsBefore, RelsAfter) :-
	optimizerOption(nestedRelations),
	% arel attribute access
	atomic(Attr),
  downcase_atom(Attr, AttrDC),
  isAttribute(AttrDC, QRel, Rel, ModAttr), !,
	%Rel = NREL:_,!, % otherwise handeld by the default predicate.
  appendAttribute(Rel, AttrDC, VarN),
	%attributeToFQN(VarN, FQN, ModAttr),
	VarN=DCFQN,
  spelled(DCFQN, attr(FQN, X, Case)),
	lastAttribute(FQN, LastA),
  ...
  (   usedAttr(Rel2, attr(Attr2, X, Case))
	...
	!.
/*
Not possible currently, even after fakting the relation where the attr is comming from, then the selectivity is missing.
lookupPred1(Attr, attr(Attr, Index, Case), RelsBefore, RelsAfter) :-
  optimizerOption(nestedRelations), % NVK ADDED
  atomic(Attr), %% changed code FIXME
  downcase_atom(Attr,AttrDC),
	queryAttr(attr(Attr, Index, Case)),
	Attr2=Attr,
  queryRel(QRel, Rel2),
  (member(Rel2, RelsBefore) ->
    RelsAfter = RelsBefore
...
makeStream(ARel, Plan) :-
  optimizerOption(nestedRelations),
  ARel = rel(arel(_, _, _, _), *),
  Plan=afeed(ARel).

makeStream(ARel, Plan) :-
  optimizerOption(nestedRelations),
  ARel = rel(arel(_, _, _, _), Var), 
...
makeStream(rel(relsubquery(Query, SQID, TOP, SQInfo), Var), Stream3) :- 
  optimizerOption(nestedRelations),
...
terms like rel(Var:Name, Var). This is done by adding the facts
...
/*
NVK ADDED NR
If the stream was feed by the afeed operator, or created a new attribute, the relation is created with aconsume instead of consume.
*/
queryToPlan(Query, StreamOut, Cost) :-
  optimizerOption(nestedRelations),
  queryToStream(Query, Stream, Cost), !,
