...
public:
  ...
  ~Win32SocketLibrary() { WSACleanup(); }
  bool initialized;
};

static Win32SocketLibrary ws32Lib;

bool Socket::IsLibraryInitialized() { return (ws32Lib.initialized); }
...
Win32Socket::Win32Socket(const string &addr, const string &port,
                         ...
                         std::ostream *_traceOutStream, bool _destroyTrace) {
  hostAddress = addr;
  hostPort = port;
  lastError = EC_OK;
  s = INVALID_SOCKET;
...
Win32Socket::Win32Socket(SOCKET newSock, std::ostream *_traceInStream,
                         std::ostream *_traceOutStream, bool _destroyTrace) {
  s = newSock;
  hostAddress = "";
  hostPort = "";
...
bool Win32Socket::Open(const int listenQueueSize, const int sockType,
                       const int flags) {
...
bool Win32Socket::Connect(int maxAttempts, const time_t timeout) {
  unsigned short port;
  ...
  // keep the error code of the last attempt (e.g. WSAECONNREFUSED) so that
...
int Win32Socket::Read(void *buf, size_t minSize, size_t maxSize,
                      time_t timeout) {
  size_t size = 0;
  time_t start = 0;
  if (state != SS_OPEN) {
...
bool Win32Socket::Read(void *buf, size_t size) {
  if (state != SS_OPEN) {
    SetStreamState(ios::failbit);
    lastError = EC_NOT_OPENED;
    return (false);
...
bool Win32Socket::Write(void const *buf, size_t size) {
  if (state != SS_OPEN) {
    SetStreamState(ios::failbit);
    lastError = EC_NOT_OPENED;
    return (false);
...
bool Win32Socket::IsOk() { return (lastError == EC_OK); }

string Win32Socket::GetErrorText() {
  string msg;
  int len;
  char msgbuf[256];
  ...
  case EC_MESSAGE_TRUNCATED:
    ...
    break;
  default:
    len = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, lastError, 0, msgbuf,
                        256, NULL);
    if (len == 0) {
...
string Win32Socket::GetSocketAddress() const {
  assert(this);
...
string Win32Socket::GetPeerAddress() const {
  assert(this);
  struct sockaddr_in addr;
  int bytes = sizeof(sockaddr);
...
Socket *Win32Socket::Accept(std::ostream *traceInStream,
                            std::ostream *traceOutStream, bool destroyTrace) {
  if (state != SS_OPEN) {
    lastError = EC_NOT_OPENED;
    return (NULL);
  }
  ...
  if (newSock == INVALID_SOCKET) {
    ...
    int enabled = 1;
    if (setsockopt(newSock, IPPROTO_TCP, TCP_NODELAY, (char *)&enabled,
                   sizeof(enabled)) != 0) {
      lastError = WSAGetLastError();
      closesocket(newSock);
      return (NULL);
    ...
    lastError = EC_OK;
    return (
        new Win32Socket(newSock, traceInStream, traceOutStream, destroyTrace));
  }
}

bool Win32Socket::CancelAccept() {
  bool result = Close();
  // Wakeup listener
...
void Win32Socket::SetStreamState(ios::iostate newState) {
  if (ioSocketStream != 0) {
    ioSocketStream->setstate(newState);
  }
}

SocketDescriptor Win32Socket::GetDescriptor() {
  if (state != SS_OPEN) {
    lastError = EC_NOT_OPENED;
...
Socket *Socket::CreateClient(const SocketDescriptor sd,
                             std::ostream *traceInStream,
                             std::ostream *traceOutStream, bool destroyTrace) {
  Win32Socket *sock =
      new Win32Socket(sd, traceInStream, traceOutStream, destroyTrace);
  return (sock);
}

int Socket::GetIP(const string &address) {
...
string Socket::GetHostname(const string &ipAddress) {
...
Socket *Socket::CreateLocal(const string &address, const int listenQueueSize,
                            std::ostream *traceInStream,
                            std::ostream *traceOutStream, bool destroyTrace) {
...
Socket *Socket::CreateGlobal(const string &address, const string &port,
                             const int listenQueueSize,
                             std::ostream *traceInStream,
                             std::ostream *traceOutStream, bool destroyTrace) {
  Win32Socket *sock = new Win32Socket(address, port, traceInStream,
                                      traceOutStream, destroyTrace);
  sock->Open(listenQueueSize, SOCK_STREAM);
  return (sock);
}

Socket *Socket::Connect(const string &address, const string &port,
                        const SocketDomain domain, const int maxAttempts,
                        const time_t timeout, std::ostream *traceInStream,
                        std::ostream *traceOutStream, bool destroyTrace) {
...
void LocalWin32Socket::SetStreamState(ios::iostate newState) {
  if (ioSocketStream != 0) {
    ioSocketStream->setstate(newState);
  }
}

int LocalWin32Socket::Read(void *buf, size_t minSize, size_t maxSize,
                           time_t timeout) {
  time_t start = 0;
  char *dst = (char *)buf;
  size_t size = 0;
...
bool LocalWin32Socket::Read(void *buf, size_t size) {
  char *dst = (char *)buf;
  lastError = EC_OK;
...
bool LocalWin32Socket::Write(const void *buf, size_t size) {
  char *src = (char *)buf;
  lastError = EC_OK;
  while (size > 0 && state == SS_OPEN) {
...
LocalWin32Socket::LocalWin32Socket(const string &address,
  ...
  ioSocketStream = 0;
  traceInStream = _traceInStream;
  traceOutStream = _traceOutStream, destroyTrace = _destroyTrace;
}

bool LocalWin32Socket::Open(int) {
...
Socket *LocalWin32Socket::Accept(std::ostream *traceInStream,
  ...
  LocalWin32Socket *sock =
      new LocalWin32Socket(traceInStream, traceOutStream, destroyTrace);
...
SocketDescriptor LocalWin32Socket::GetDescriptor() {
  lastError = EC_INVALID_ACCESS_MODE;
  return (INVALID_SOCKET);
}

bool LocalWin32Socket::CancelAccept() {
  state = SS_SHUTDOWN;
...
bool LocalWin32Socket::IsOk() { return (!lastError); }

bool LocalWin32Socket::Close() {
  if (state != SS_CLOSE) {
    state = SS_CLOSE;
    if (mutexHandle) {
...
string LocalWin32Socket::GetSocketAddress() const { return (localName); }

string LocalWin32Socket::GetPeerAddress() const { return (localName); }

string LocalWin32Socket::GetErrorText() {
  string msg;
  int len;
  char msgbuf[256];
...
bool LocalWin32Socket::ShutDown() {
  if (state == SS_OPEN) {
    state = SS_SHUTDOWN;
    SetEvent(signalHandle[RD]);
    SetEvent(signalHandle[RTT]);
...
bool LocalWin32Socket::Connect(int maxAttempts, time_t timeout) {
  char buf[MAX_ADDRESS_LEN];
  int rc, i, error_code;
  HANDLE h[2];
  ...
  rc = WaitForSingleObject(signalHandle[RTT],
                           timeout * maxAttempts * MILLISECOND);
  if (rc != WAIT_OBJECT_0) {
  ...
  if (!DuplicateHandle(hServer, cdp->mutexHandle, hSelf, &mutexHandle, 0, FALSE,
                       DUPLICATE_SAME_ACCESS) ||
      !DuplicateHandle(hSelf, watchDogMutex, hServer,
                       &((ConnectData *)sendBuffer->dataBuf)->mutexHandle, 0,
  ...
  if (!DuplicateHandle(hServer, ad.bufferHandle, hSelf, &bufferHandle, 0, FALSE,
                       DUPLICATE_SAME_ACCESS)) {
    lastError = GetLastError();
    CloseHandle(hServer);
    CloseHandle(mutexHandle);
    ...
    for (i = RD; i <= RTT; i++) {
      if (!DuplicateHandle(hServer, ad.signalHandle[i], hSelf, &signalHandle[i],
