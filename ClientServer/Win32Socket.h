...
public:
  Win32Socket(const std::string &address, const std::string &port,
              ...
              bool destroyTrace);
  Win32Socket(SOCKET newSock, std::ostream *traceInStream,
              std::ostream *traceOutStream, bool destroyTrace);
  ~Win32Socket();
  SocketDescriptor GetDescriptor();
  bool Open(const int listenQueueSize, const int socketType,
            const int flags = 0);
  ...
  Socket *Accept(std::ostream *traceInStream, std::ostream *traceOutStream,
                 bool destryTrace);
  bool CancelAccept();
  bool Close();
  bool ShutDown();
...
public:
  LocalWin32Socket(const std::string &address, std::ostream *traceInStream,
                   std::ostream *traceOutStream, bool destroyStream);
  LocalWin32Socket(std::ostream *traceInStream, std::ostream *traceOutStream,
                   bool destroyStream);
  ~LocalWin32Socket();
  SocketDescriptor GetDescriptor();
